...
/* FROZEN v1.5.3 — preserve readable 34pt Tartil; fix vertical density and footer as one system. */
...
/* All legacy loose footer identities are forbidden by v1.5.3. */
...
function footer(pageNo,qr){
 ...
 return `<div class="qurbata-footer-v153" data-qurbata-footer="${id}"><div class="qf-id">${id}</div><div class="qf-field"><div class="qf-label">Tanggal:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">Nilai:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">TTD:</div><div class="qf-box"></div></div><div class="qf-qr">${qr}</div><div class="qf-brand"><div class="qf-brand-ar" dir="rtl">تَعَلَّمْ – اِعْمَلْ – عَلِّمْ</div><div class="qf-brand-id">Belajar, Mengamalkan, Mengajarkan</div><div class="qf-brand-os">RIQA OS</div></div></div>`;
}
function normalizeArticle(article,pageNo){
 const qr=getQr(article);let one=article;
 const exampleRequired=!/P(?:010|020|040)\b/.test(`P${String(pageNo).padStart(3,'0')}`);
 if(exampleRequired&&!/(?:class=["'][^"']*(?:planting|intro-pair|lesson-heading)[^"']*["'])/.test(one))throw new Error(`P${String(pageNo).padStart(3,'0')}: EXAMPLE_BLOCK_MISSING`);
...
for(const file of files){
 ...
 const footers=(html.match(/class="qurbata-footer-v153"/g)||[]).length;
 const dates=(html.match(/Tanggal:/g)||[]).length,scores=(html.match(/Nilai:/g)||[]).length,signs=(html.match(/TTD:/g)||[]).length;
 if(footers!==10||dates<10||scores<10||signs<10)throw new Error(`${file}: V153_FOOTER_ASSERT_FAIL footers=${footers}`);
 ...
 if(examples<7)throw new Error(`${file}: V153_EXAMPLE_ASSERT_FAIL examples=${examples}`);
 await fs.writeFile(file,html);
}
console.log('Applied J1 FROZEN v1.5.3: reserve mandatory example track first, preserve 34pt reading, distribute remaining height to 8 rows, unified 15mm footer.');