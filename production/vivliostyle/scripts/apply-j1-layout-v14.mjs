...
/* FROZEN v1.5.9 — preserve readable 34pt Tartil; fix vertical density and footer as one system. */
...
article{height:100%!important;min-height:100%!important;display:grid!important;grid-template-rows:auto minmax(16mm,auto) auto minmax(68mm,1fr) 15mm!important;align-content:stretch!important;row-gap:.8mm!important;}
...
.practice-panel{height:100%!important;min-height:68mm!important;margin:0!important;padding:0!important;overflow:visible!important;display:flex!important;flex-direction:column!important;align-self:stretch!important;}
.practice-grid{flex:1 1 auto!important;height:68mm!important;min-height:68mm!important;margin:0!important;display:grid!important;grid-template-rows:repeat(8,8.5mm)!important;gap:0!important;overflow:visible!important;align-self:stretch!important;}
...
/* All legacy loose footer identities are forbidden by v1.5.9. */
...
function footer(pageNo,qr){
 ...
 return `<div class="qurbata-footer-v159" data-qurbata-footer="${id}"><div class="qf-id">${id}</div><div class="qf-field"><div class="qf-label">Tanggal:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">Nilai:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">TTD:</div><div class="qf-box"></div></div><div class="qf-qr">${qr}</div><div class="qf-brand"><div class="qf-brand-ar" dir="rtl">تَعَلَّمْ – اِعْمَلْ – عَلِّمْ</div><div class="qf-brand-id">Belajar, Mengamalkan, Mengajarkan</div><div class="qf-brand-os">RIQA OS</div></div></div>`;
}
...
function ensureExample(one,pageNo){
 ...
 const html='<section class="planting example-source-v159"><div class="arabic" dir="rtl">'+expected+'</div></section>';
...
for(const file of files){
 ...
 const footers=(html.match(/class="qurbata-footer-v159"/g)||[]).length;
 const dates=(html.match(/Tanggal:/g)||[]).length,scores=(html.match(/Nilai:/g)||[]).length,signs=(html.match(/TTD:/g)||[]).length;
 if(footers!==10||dates<10||scores<10||signs<10)throw new Error(`${file}: V159_FOOTER_ASSERT_FAIL footers=${footers}`);
 ...
 if(examples<requiredExamples)throw new Error(`${file}: V159_EXAMPLE_ASSERT_FAIL examples=${examples} required=${requiredExamples}`);
 await fs.writeFile(file,html);
}
console.log('Applied J1 FROZEN v1.5.9: protect heading separately; assert practice 8x3 + Arabic content; inject/verify Page Register example, distribute remaining height to 8 rows, unified 15mm footer.');