...
/* FROZEN v1.5.7 — preserve readable 34pt Tartil; fix vertical density and footer as one system. */
...
article{height:100%!important;min-height:100%!important;display:grid!important;grid-template-rows:auto minmax(18mm,auto) auto minmax(96mm,1fr) 15mm!important;align-content:stretch!important;row-gap:.8mm!important;}
...
.practice-panel{height:100%!important;min-height:96mm!important;margin:0!important;padding:0!important;overflow:visible!important;display:flex!important;flex-direction:column!important;align-self:stretch!important;}
.practice-grid{flex:1 1 auto!important;height:96mm!important;min-height:96mm!important;margin:0!important;display:grid!important;grid-template-rows:repeat(8,12mm)!important;gap:0!important;overflow:visible!important;align-self:stretch!important;}
.practice-row{height:12mm!important;min-height:12mm!important;margin:0!important;overflow:visible!important;display:grid!important;align-items:center!important;}
.practice-cell{font-size:32pt!important;line-height:1!important;padding:.3mm .45mm!important;min-height:12mm!important;height:12mm!important;overflow:visible!important;display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;white-space:nowrap!important;}
/* Official footer: exactly one compact horizontal system. */
.qurbata-footer-v157{display:grid!important;grid-template-columns:9fr 21fr 14fr 23fr 11fr 22fr!important;column-gap:.7mm!important;align-items:center!important;width:100%!important;height:15mm!important;min-height:15mm!important;max-height:15mm!important;margin:0!important;padding:0!important;box-sizing:border-box!important;break-inside:avoid!important;overflow:hidden!important;font-family:Arial,sans-serif!important;color:#12415c!important;align-self:end!important;}
...
/* All legacy loose footer identities are forbidden by v1.5.7. */
...
function footer(pageNo,qr){
 ...
 return `<div class="qurbata-footer-v157" data-qurbata-footer="${id}"><div class="qf-id">${id}</div><div class="qf-field"><div class="qf-label">Tanggal:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">Nilai:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">TTD:</div><div class="qf-box"></div></div><div class="qf-qr">${qr}</div><div class="qf-brand"><div class="qf-brand-ar" dir="rtl">تَعَلَّمْ – اِعْمَلْ – عَلِّمْ</div><div class="qf-brand-id">Belajar, Mengamalkan, Mengajarkan</div><div class="qf-brand-os">RIQA OS</div></div></div>`;
}
...
function ensureExample(one,pageNo){
 ...
 const html='<section class="planting example-source-v157"><div class="arabic" dir="rtl">'+expected+'</div></section>';
...
for(const file of files){
 ...
 const footers=(html.match(/class="qurbata-footer-v157"/g)||[]).length;
 const dates=(html.match(/Tanggal:/g)||[]).length,scores=(html.match(/Nilai:/g)||[]).length,signs=(html.match(/TTD:/g)||[]).length;
 if(footers!==10||dates<10||scores<10||signs<10)throw new Error(`${file}: V157_FOOTER_ASSERT_FAIL footers=${footers}`);
 if(/AUDIT\s*[—-]|PAGE_REGISTER|03_BOOKS/.test(html))throw new Error(`${file}: PRINTED_TECHNICAL_METADATA_FAIL`);
 if(!/font-size:32pt!important/.test(css))throw new Error(`${file}: READING_SIZE_ASSERT_FAIL`);
 ...
 if(examples<requiredExamples)throw new Error(`${file}: V157_EXAMPLE_ASSERT_FAIL examples=${examples} required=${requiredExamples}`);
 await fs.writeFile(file,html);
}
console.log('Applied J1 FROZEN v1.5.7: protect heading separately; assert practice 8x3 + Arabic content; inject/verify Page Register example, distribute remaining height to 8 rows, unified 15mm footer.');