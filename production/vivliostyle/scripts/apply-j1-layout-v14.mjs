...
/* FROZEN v1.5.4 — preserve readable 34pt Tartil; fix vertical density and footer as one system. */
...
.planting,.intro-pair,.lesson-heading{min-height:18mm!important;margin:0!important;overflow:visible!important;display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;}
.planting .arabic,.intro-pair,.lesson-heading .arabic-title{font-size:38pt!important;line-height:1.12!important;padding:1.2mm .8mm!important;overflow:visible!important;min-height:16mm!important;display:flex!important;align-items:center!important;justify-content:center!important;}
...
/* All legacy loose footer identities are forbidden by v1.5.4. */
...
function footer(pageNo,qr){
 ...
 return `<div class="qurbata-footer-v154" data-qurbata-footer="${id}"><div class="qf-id">${id}</div><div class="qf-field"><div class="qf-label">Tanggal:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">Nilai:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">TTD:</div><div class="qf-box"></div></div><div class="qf-qr">${qr}</div><div class="qf-brand"><div class="qf-brand-ar" dir="rtl">تَعَلَّمْ – اِعْمَلْ – عَلِّمْ</div><div class="qf-brand-id">Belajar, Mengamalkan, Mengajarkan</div><div class="qf-brand-os">RIQA OS</div></div></div>`;
}
...
function ensureExample(one,pageNo){
 ...
 const block=one.match(/<[^>]+class=["'][^"']*(?:planting|intro-pair|lesson-heading)[^"']*["'][^>]*>[\s\S]*?<\/[^>]+>/i);
 const hasArabic=block&&/[\u0600-\u06FF]/.test(stripTags(block[0]));
 if(hasArabic)return one;
 const html='<section class="planting example-source-v154"><div class="arabic" dir="rtl">'+expected+'</div></section>';
...
function normalizeArticle(article,pageNo){
 ...
 if(exampleRequired){const b=one.match(/<[^>]+class=["'][^"']*(?:planting|intro-pair|lesson-heading)[^"']*["'][^>]*>[\s\S]*?<\/[^>]+>/i);if(!b||!/[\u0600-\u06FF]/.test(stripTags(b[0])))throw new Error(`P${String(pageNo).padStart(3,'0')}: EXAMPLE_CONTENT_MISSING`);if(pageNo===1&&!['بَ','تَ','ثَ'].every(x=>stripTags(b[0]).includes(x)))throw new Error('P001: EXAMPLE_SENTINEL_FAIL');}
...
for(const file of files){
 ...
 const footers=(html.match(/class="qurbata-footer-v154"/g)||[]).length;
 const dates=(html.match(/Tanggal:/g)||[]).length,scores=(html.match(/Nilai:/g)||[]).length,signs=(html.match(/TTD:/g)||[]).length;
 if(footers!==10||dates<10||scores<10||signs<10)throw new Error(`${file}: V154_FOOTER_ASSERT_FAIL footers=${footers}`);
 ...
 const examples=(html.match(/class=["'][^"']*(?:planting|intro-pair|lesson-heading)[^"']*["']/g)||[]).length;
 if(examples<7)throw new Error(`${file}: V154_EXAMPLE_ASSERT_FAIL examples=${examples}`);
 await fs.writeFile(file,html);
}
console.log('Applied J1 FROZEN v1.5.4: inject/verify real Page Register example content, reserve mandatory example track, preserve 34pt reading, distribute remaining height to 8 rows, unified 15mm footer.');