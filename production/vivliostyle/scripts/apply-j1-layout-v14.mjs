...
/* FROZEN v1.5.2 — preserve readable 34pt Tartil; fix vertical density and footer as one system. */
...
article{height:100%!important;min-height:100%!important;display:grid!important;grid-template-rows:auto auto auto minmax(0,1fr) 15mm!important;align-content:stretch!important;row-gap:.8mm!important;}
.planting,.intro-pair,.lesson-heading{min-height:0!important;margin:0!important;overflow:visible!important;}
.planting .arabic,.intro-pair,.lesson-heading .arabic-title{font-size:38pt!important;line-height:1.12!important;padding:.8mm .6mm!important;overflow:visible!important;}
...
/* Practice receives all remaining height. Keep reading size; enlarge ROWS, not font. */
...
/* All legacy loose footer identities are forbidden by v1.5.2. */
...
function footer(pageNo,qr){
 ...
 return `<div class="qurbata-footer-v152" data-qurbata-footer="${id}"><div class="qf-id">${id}</div><div class="qf-field"><div class="qf-label">Tanggal:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">Nilai:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">TTD:</div><div class="qf-box"></div></div><div class="qf-qr">${qr}</div><div class="qf-brand"><div class="qf-brand-ar" dir="rtl">تَعَلَّمْ – اِعْمَلْ – عَلِّمْ</div><div class="qf-brand-id">Belajar, Mengamalkan, Mengajarkan</div><div class="qf-brand-os">RIQA OS</div></div></div>`;
}
...
for(const file of files){
 ...
 const footers=(html.match(/class="qurbata-footer-v152"/g)||[]).length;
 const dates=(html.match(/Tanggal:/g)||[]).length,scores=(html.match(/Nilai:/g)||[]).length,signs=(html.match(/TTD:/g)||[]).length;
 if(footers!==10||dates<10||scores<10||signs<10)throw new Error(`${file}: V152_FOOTER_ASSERT_FAIL footers=${footers}`);
...
console.log('Applied J1 FROZEN v1.5.2: preserve 34pt reading, distribute remaining height to 8 rows, unified 15mm footer.');