...
/* FROZEN v1.5.1 — large, clear, proportional reading at A5 100%. */
...
/* Controlled vertical density: example + practice receive the page, not dead space. */
article{display:grid!important;grid-template-rows:auto minmax(20mm,auto) auto minmax(0,1fr) 15mm!important;align-content:stretch!important;row-gap:.8mm!important;}
.planting,.intro-pair,.lesson-heading{min-height:0!important;margin:0!important;overflow:visible!important;}
.planting .arabic,.intro-pair,.lesson-heading .arabic-title{font-size:38pt!important;line-height:1.12!important;padding:1mm .6mm!important;overflow:visible!important;}
/* Tartil: 8 rows fill the flexible area evenly. */
...
.practice-grid{flex:1 1 auto!important;height:100%!important;min-height:0!important;display:grid!important;grid-template-rows:repeat(8,minmax(0,1fr))!important;gap:.65mm!important;overflow:visible!important;}
.practice-row{height:100%!important;min-height:0!important;overflow:visible!important;}
.practice-cell{font-size:34pt!important;line-height:1.14!important;padding:1.15mm .55mm!important;min-height:0!important;overflow:visible!important;display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;}
.integration-panel,.integration-grid{margin:0!important;min-height:0!important;}
.integration-card{padding:.45mm .65mm!important;min-height:0!important;}
.integration-arabic{line-height:1.12!important;padding:.3mm .45mm!important;}
.integration-meaning,.integration-instruction{line-height:1.08!important;padding:.2mm .4mm!important;}
/* Official compact footer, single brand only. */
.qurbata-footer-v151{display:grid!important;grid-template-columns:9fr 21fr 14fr 23fr 11fr 22fr!important;column-gap:.7mm!important;align-items:center!important;width:100%!important;height:15mm!important;min-height:15mm!important;max-height:15mm!important;margin:0!important;padding:0!important;box-sizing:border-box!important;break-inside:avoid!important;overflow:hidden!important;font-family:Arial,sans-serif!important;color:#12415c!important;}
...
/* Kill legacy loose brand/motto elements outside official footer. */
article>.motto,article>.footer-motto,article>.riqa-os-brand,article>.brand-footer{display:none!important;}
</style>`;
...
function footer(pageNo,qr){const id=`QJ1-P${String(pageNo).padStart(3,'0')}`;return `<div class="qurbata-footer-v151" data-qurbata-footer="${id}"><div class="qf-id">${id}</div><div class="qf-field"><div class="qf-label">Tanggal:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">Nilai:</div><div class="qf-box"></div></div><div class="qf-field"><div class="qf-label">TTD:</div><div class="qf-box"></div></div><div class="qf-qr">${qr}</div><div class="qf-brand"><div class="qf-brand-ar" dir="rtl">تَعَلَّمْ – اِعْمَلْ – عَلِّمْ</div><div class="qf-brand-id">Belajar, Mengamalkan, Mengajarkan</div><div class="qf-brand-os">RIQA OS</div></div></div>`;}
function normalizeArticle(article,pageNo){
 ...
 one=one.replace(/<div class=["']qurbata-footer-v(?:142|143|15|151)["'][^>]*>[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/gi,'');
...
for(const file of files){
 ...
 const footers=(html.match(/class="qurbata-footer-v151"/g)||[]).length;
 const dates=(html.match(/Tanggal:/g)||[]).length,scores=(html.match(/Nilai:/g)||[]).length,signs=(html.match(/TTD:/g)||[]).length;
 if(footers!==10||dates<10||scores<10||signs<10)throw new Error(`${file}: V151_FOOTER_ASSERT_FAIL`);
...
console.log('Applied J1 FROZEN v1.5.1: 38pt planting, 34pt Tartil, evenly distributed 8-row practice, compact 15mm footer.');