...
const assertUniquePage=(n,items)=>{const keys=items.map(groupKey);if(items.length!==24)throw new Error('P'+String(n).padStart(3,'0')+' GROUP_COUNT_FAIL '+items.length+'/24');if(new Set(keys).size!==items.length)throw new Error('P'+String(n).padStart(3,'0')+' DUPLICATE_GROUP_FAIL');if(items.some(x=>groupLen(x)===1))throw new Error('P'+String(n).padStart(3,'0')+' SINGLE_LETTER_GROUP_FAIL');return items;};
const assertRegular=(n,items,target,review)=>{
 assertUniquePage(n,items);
...
const PAGE_REGISTER={
 3:{target:['جَ','حَ','خَ'],review:['ءَ','أَ','بَ','تَ','ثَ']}
};
...
function exercisesFor(n){
 // P003 is the curriculum sentinel: no random cycling, no fallback, exact register mapping.
 if(n===3){const r=PAGE_REGISTER[3];return assertRegular(3,[...P003_PLANTING,...P003_PRACTICE],r.target,r.review);}
 // Until each page is migrated to the frozen Page Register, do not silently apply the new regular-page algorithm.
 // Existing pages remain legacy output and may not be declared curriculum-final.
 ...
 // Legacy filler is retained only to keep non-sentinel pages buildable during migration; it is NOT curriculum-final.
