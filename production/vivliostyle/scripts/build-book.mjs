...
const pages = await Promise.all(sourcePages.map(async (page) => {
  ...
  <article class="book-page ${page.intro ? "book-page--concept" : ""} ${page.review ? "book-page--review" : ""}" data-page-id="${escapeHtml(page.pageId)}">
    ...
    ${page.intro ? `<section class="concept-intro concept-intro--practice"><div class="concept-label"><h2>${escapeHtml(page.intro.title)}</h2></div><div class="concept-items arabic" lang="ar" dir="ltr">${page.intro.items.map(([a,b])=>`<span><b>${escapeHtml(a)}</b><i>→</i><strong>${escapeHtml(b)}</strong></span>`).join("")}</div></section>` : ""}
