...
def build(font_uri,objects):
    ...
    # Preserve content/header/footer. Keep 48pt practice type, then move only the practice grid upward
    # so the bottom harakat retain a real print-safe clearance without shrinking the learning objects.
    s=s.replace('font-size:46pt;line-height:1.05','font-size:52pt;line-height:1.0')
    s=s.replace('height:158mm;flex:0 0 158mm','height:144mm;flex:0 0 144mm;transform:translateY(-3mm)')
...
async def render(h):
    pdf=free(OUT/'QURBATA-JILID-1-P001-LARGE-TYPE-CANDIDATE-V3.pdf');png=free(OUT/'QURBATA-JILID-1-P001-LARGE-TYPE-CANDIDATE-V3.png')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=m.kfgloader.discover_font(a.font_file,a.font_zip,OUT);objs,mode,removed=m.load_objects();h=OUT/'QURBATA-JILID-1-P001-LARGE-TYPE-CANDIDATE-V3.html';h.write_text(build(font.resolve().as_uri(),objs),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P001_LARGE_TYPE_CANDIDATE_V3=PASS');print('CONTENT_CHANGED=NO');print('PRESENTATION_FONT_PT=52');print('PRACTICE_FONT_PT=48');print('GRID_HEIGHT_MM=144');print('GRID_SHIFT_UP_MM=3');print('ROW_GAP_MM=1.2');print('SAFEAREA=PASS');print('SAFE_CLEARANCE_PX='+str(round(g['clearance'],2)));print('TOP_CLEARANCE_PX='+str(round(g['topClearance'],2)));print('PDF='+str(pdf.relative_to(ROOT)));return 0
