...
def build(font_uri,objects):
    ...
    # There is unused vertical space below the fixed-height content stack. Use part of it
    # to lower the footer instead of shrinking the 48pt learning objects.
    s=s.replace('.footer{height:12mm;flex:0 0 12mm;', '.footer{height:12mm;flex:0 0 12mm;margin-top:4mm;')
    return s
...
async def render(h):
    pdf=free(OUT/'QURBATA-JILID-1-P001-LARGE-TYPE-CANDIDATE-V5.pdf');png=free(OUT/'QURBATA-JILID-1-P001-LARGE-TYPE-CANDIDATE-V5.png')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=m.kfgloader.discover_font(a.font_file,a.font_zip,OUT);objs,mode,removed=m.load_objects();h=OUT/'QURBATA-JILID-1-P001-LARGE-TYPE-CANDIDATE-V5.html';h.write_text(build(font.resolve().as_uri(),objs),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P001_LARGE_TYPE_CANDIDATE_V5=PASS');print('CONTENT_CHANGED=NO');print('PRESENTATION_FONT_PT=52');print('PRACTICE_FONT_PT=48');print('GRID_HEIGHT_MM=140');print('ROW_GAP_MM=0.9');print('FOOTER_MARGIN_TOP_MM=4');print('SAFEAREA=PASS');print('SAFE_CLEARANCE_PX='+str(round(g['clearance'],2)));print('TOP_CLEARANCE_PX='+str(round(g['topClearance'],2)));print('FOOTER_BOTTOM_CLEARANCE_PX='+str(round(g['footerBottomClearance'],2)));print('PDF='+str(pdf.relative_to(ROOT)));return 0
