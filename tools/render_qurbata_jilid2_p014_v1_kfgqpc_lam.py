#!/usr/bin/env python3
"""QURBATA Jilid 2 P014 — acquisition of ل with cumulative P001-P013 review."""
...
MAP=ROOT/'content/qwo/registry/JILID-2-P014-COMPETENCY-MAP-V1.csv'; MICRO=ROOT/'content/qwo/registry/JILID-2-MICRO-COMPETENCY-P014-V1.csv'; LEX=ROOT/'content/qwo/registry/JILID-2-P014-LEXICAL-FOUNDATION-V1.csv'
...
if len(stairs)!=10 or len(lex)!=32: raise ValueError('P014_REGISTRY_INVALID')
...
for r in lex:
    if len(bases(r['word']))!=3 or not r['meaning_id'].strip() or r['lexical_status']!='CURATED' or r['competency_status']!='ALLOWED': raise ValueError('P014_SEMANTIC_GATE_FAIL='+repr(r))
p001.MICRO=MICRO
# Future joining letters after lam: م ن ه ي. ظ is also not yet acquired.
p001.P001_BANNED_JOINING=set('ظمنهي')
...
def build(debug):
    h=orig(debug).replace('<div class="page-number">01</div>','<div class="page-number">14</div>',1)
    s=h.index('<section class="presentation">'); e=h.index('</section>',s)+len('</section>')
    pres=f'''<section class="presentation"><div class="presentation-object-wrap"><div class="presentation-object"><span class="arabic-part" lang="ar">{p001.arabic_html('لَعِبَ')}</span><span class="arrow">←</span><span class="arabic-part" lang="ar">{p001.arabic_html('عِ')}</span><span class="arrow">←</span><span class="arabic-part" lang="ar">{p001.arabic_html('لَ')}</span></div></div></section>'''
    h=h[:s]+pres+h[e:]
...
async def render(h,out,debug):
    ...
    async with async_playwright() as pw:
        ...
        if count!=32: raise RuntimeError(f'P014_OBJECT_COUNT_INVALID actual={count} expected=32')
        ...
        if issues: raise RuntimeError('P014_LAYOUT_ISSUES='+str(len(issues))+' REPORT='+str(report))
        page_no=(await page.locator('.page-number').inner_text()).strip()
        if page_no!='14': raise RuntimeError('P014_PAGE_IDENTITY_FAIL actual='+repr(page_no))
        await page.screenshot(path=str(png/'page-014.png'),full_page=True)
        pdf=out/'QURBATA-JILID-2-P014-V1-KFGQPC-LAM-CUMULATIVE.pdf'
...
def main():
    ...
    if leaks: raise ValueError('P014_COMPETENCY_LEAKAGE='+repr(leaks))
    current=[r for r in lex if r['function']=='CURRENT']; missing=[r['word'] for r in current if 'ل' not in bases(r['word'])]
    if missing: raise ValueError('P014_CURRENT_OBJECT_MISSING_LAM='+repr(missing))
    text=''.join(r['word'] for r in lex); counts={'FATHA':text.count('َ'),'KASRA':text.count('ِ'),'DAMMA':text.count('ُ')}
    if counts['KASRA']<10 or counts['DAMMA']<10: raise ValueError('P014_HARAKAT_BALANCE_FAIL='+repr(counts))
    rc=v22.main()
    print('JILID2_P014_RENDERER_V1=PASS'); print('PAGE=14'); print('PAGE_IDENTITY_GATE=14'); print(f"COMPETENCY={meta['CompetencyCode']}|{meta['Competency']}"); print(f"UNIT_COMPETENCY={meta['UnitCompetencyCode']}|{meta['UnitCompetency']}"); print(f"UNIT_MUROJAAH={meta['UnitMurojaahCode']}|{meta['UnitMurojaah']}"); print(f"STAIR_RANGE={stairs[0]['StairCode']}-{stairs[-1]['StairCode']}"); print('ACQUISITION_LETTERS=ل'); print('CUMULATIVE_HARAKAT=FATHA|KASRA|DAMMA'); print('HARAKAT_FATHA_COUNT='+str(counts['FATHA'])); print('HARAKAT_KASRA_COUNT='+str(counts['KASRA'])); print('HARAKAT_DAMMA_COUNT='+str(counts['DAMMA'])); print('HARAKAT_BALANCE_GATE=KASRA>=10|DAMMA>=10'); print('CUMULATIVE_COMPETENCY_P001_P013=PRESERVED'); print('PRACTICE_OBJECTS=32'); print('CURRENT_LEXICAL_OBJECTS='+str(len(current))); print('MUROJAAH_LEXICAL_OBJECTS='+str(32-len(current))); print('THREE_LETTER_WITH_MEANING=32'); print('MEANINGLESS_THREE_LETTER_OBJECTS=0'); print('COMPETENCY_LEAKAGE=0'); print('ARABIC_FONT_PRIMARY=KFGQPC Uthman Taha Naskh'); print('STATUS=P014_CUMULATIVE_CANDIDATE_NOT_FROZEN'); return rc
