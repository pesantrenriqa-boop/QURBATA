...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P042';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'فُ','قُ','كُ','لُ'}
# 61 tokens = 31 focus + 30 cumulative review.
EX=['فُ بَ','قُ بِ','كُ تَ','لُ تِ','فُ ثَ','قُ ثِ','كُ جَ','لُ جِ',
    'فُ حَ قُ','قُ خِ كُ','كُ دَ لُ','لُ ذِ فُ','فُ رَ قُ','قُ زِ كُ','كُ سَ لُ','لُ شِ فُ',
    'فُ صَ ضِ','قُ طَ ظِ','كُ عَ غِ','لُ فَ قِ','فُ مَ نِ','قُ وَ يِ','كُ ءَ بِ']
ARABIC_NUMERALS=['١','٢','٣','٤','٥','٦','٧','٨','٩','١٠'];NAME_REVIEW=['ف','ق','ك','ل']
...
def audit():
 ...
 if len(ts)!=61:raise RuntimeError(f'P042_TOKEN_COUNT_FAIL {len(ts)}')
 if (focus,review)!=(31,30):raise RuntimeError(f'P042_BALANCE_FAIL focus={focus} review={review}')
 if 'إُ' in ts:raise RuntimeError('P042_HAMZAH_BELOW_ALIF_FORBIDDEN')
 if any(x in {'ه','هَ','هِ','هُ'} for x in ts+NAME_REVIEW):raise RuntimeError('P042_ONE_HOLE_HA_FORBIDDEN')
 if ARABIC_NUMERALS!=['١','٢','٣','٤','٥','٦','٧','٨','٩','١٠']:raise RuntimeError('P042_ARABIC_NUMERAL_SEQUENCE_FAIL')
def rows():
 o=[]
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">42</div></header><section class="presentation"><span>فُ</span><span>قُ</span><span>كُ</span><span>لُ</span></section><section class="grid">{rows()}</section><div></div>{bottom_strip()}<div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P042-DAMMAH-06-CANDIDATE-V1-ARABIC-NUMERALS.pdf'
 async with async_playwright() as pw:
  ...
  if g['lastRowBottom']>g['gridBottom']+1:raise RuntimeError('P042_GRID_OVERFLOW '+str(g))
  if g['lastGlyphToStrip']<5:raise RuntimeError('P042_STRIP_COLLISION '+str(g))
  if g['stripBottom']>g['footerTop'] or g['footerBottom']>g['pageBottom']+1:raise RuntimeError('P042_BOTTOM_FLOW_FAIL '+str(g))
  if g['nums']!=ARABIC_NUMERALS:raise RuntimeError('P042_ARABIC_NUMERAL_RENDER_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P042-DAMMAH-06-CANDIDATE-V1-ARABIC-NUMERALS.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P042_DAMMAH=PASS');print('DAMMAH_COMPACT_PAGE=2_OF_4');print('MATERIAL_NEW=فُ|قُ|كُ|لُ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('BOTTOM_STRIP_PRIMARY=ARABIC_NUMERALS');print('ARABIC_NUMERALS=١|٢|٣|٤|٥|٦|٧|٨|٩|١٠');print('NUMBER_FONT_PT=25');print('NAME_REVIEW=ف|ق|ك|ل');print('JILID1_TARGET_END=P045');print('BOTTOM_FLOW=SAFE');print('PDF='+str(pdf.relative_to(ROOT)))
