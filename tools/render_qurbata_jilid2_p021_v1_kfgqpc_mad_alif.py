...
MAP=ROOT/'content/qwo/registry/JILID-2-P021-COMPETENCY-MAP-V1.csv'; MICRO=ROOT/'content/qwo/registry/JILID-2-MICRO-COMPETENCY-P021-V1.csv'; LEX=ROOT/'content/qwo/registry/JILID-2-P021-LEXICAL-FOUNDATION-V1.csv'
...
async def render(h,out,debug):
    ...
    async with async_playwright() as pw:
        ...
        if await page.locator('.j2-object').count()!=32: raise RuntimeError('P021_OBJECT_COUNT_INVALID')
        if (await page.locator('.page-number').inner_text()).strip()!='21': raise RuntimeError('P021_PAGE_IDENTITY_FAIL')
        family=await page.locator('.mad-unit').evaluate("e=>getComputedStyle(e).fontFamily")
        if 'QURBATA KFGQPC Uthman Taha Naskh' not in family: raise RuntimeError('P021_FONT_BINDING_FAIL='+repr(family))
