...
# V6: sharpen the native lower-left terminal by clipping the glyph itself; no patch/overlay/raster.
...
/* Sharp terminal: this clips the ORIGINAL glyph only. No extra black shape is drawn. */
.ha-sharp{{display:inline-block;clip-path:polygon(6% 0,100% 0,100% 100%,6% 100%,6% 94%,0 88%,6% 82%)}}
...
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V6-SHARP-NATIVE.pdf');png=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V6-SHARP-NATIVE.png');expected=EXERCISES[:-1]
 ...
 async with async_playwright() as pw:
  ...
  if clip=='none':raise RuntimeError('P014_SHARP_CLIP_NOT_ACTIVE')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V6-SHARP-NATIVE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P014_HA_WAW_SHARP_NATIVE=PASS');print('MATERIAL_NEW=ﮪَ|وَ');print('HA_FORM=NATIVE_ISOLATED_TWO_LOOP');print('HA_TAIL=ORIGINAL_GLYPH_VECTOR_CLIPPED_TO_POINT');print('HA_TAIL_PATCH=NONE');print('CSS_PSEUDO_OVERLAY=NONE');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('PRACTICE_FONT_PT=40');print('GROUP_SPACING=PRESERVED');print('BOTTOM_GLYPH_OVERFLOW=0');print('PDF='+str(pdf.relative_to(ROOT)))
