#!/usr/bin/env python3
# QURBATA Jilid 1 P014 — native isolated two-loop haa + waw.
# V7: preserve full native tail length; taper only the terminal to a sharp point.
...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P014';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
HA='ﮪَ';FOCUS={HA,'وَ'};REVIEW={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ','لَ','مَ','نَ'}
EXERCISES=[f'{HA} ءَ','وَ أَ',f'{HA} بَ','وَ تَ',f'ثَ {HA}','جَ وَ',f'حَ {HA}','خَ وَ',f'{HA} دَ {HA}','وَ ذَ وَ',f'{HA} رَ {HA}','وَ زَ وَ',f'{HA} سَ {HA}','وَ شَ وَ',f'{HA} صَ {HA}','وَ ضَ وَ',f'طَ {HA} ظَ','عَ وَ غَ',f'فَ {HA} قَ','كَ وَ لَ',f'مَ {HA} نَ','ءَ وَ أَ',f'بَ {HA} تَ','ثَ وَ جَ']
def tok(x):
 c=' class="ha-sharp"' if x==HA else ''
 return f'<span{c}>{html.escape(x)}</span>'
def obj(s):return '<span class="run">'+''.join(tok(x) for x in s.split())+'</span>'
def rows():
 a=EXERCISES[:-1];o=[]
 ...
 return ''.join(o)
def audit():
 if len(EXERCISES)!=24:raise RuntimeError('P014_SOURCE_COUNT_FAIL')
 if any(('ھ' in e or 'ه' in e) for e in EXERCISES):raise RuntimeError('P014_OLD_HA_FORM_PRESENT')
 for e in EXERCISES[:8]:
  t=e.split()
  if len(t)!=2 or len(set(t)&FOCUS)!=1 or len(set(t)&REVIEW)!=1:raise RuntimeError(f'P014_FIRST_TWO_ROWS_PATTERN_FAIL={e}')
 rendered=EXERCISES[:-1];ts=[x for e in rendered for x in e.split()]
 if any(x not in FOCUS|REVIEW for x in ts):raise RuntimeError('P014_WHITELIST_FAIL')
 if any(not(set(e.split())&FOCUS) for e in rendered):raise RuntimeError('P014_EXAMPLE_WITHOUT_FOCUS')
...
/* V7: preserve full native left extent. Only terminal band is tapered to one far-left point. */
.ha-sharp{{display:inline-block;clip-path:polygon(0 0,100% 0,100% 100%,0 100%,0 95%,19% 95%,0 88%,19% 81%,0 81%)}}
.bottom-safe-spacer{{height:8mm;flex:0 0 8mm}}.footer{{height:12mm;display:flex;justify-content:space-between;align-items:center;padding-bottom:2.2mm;color:#173a2d}}.ar{{font:10.3pt "{FONT}";direction:rtl}}'''
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">14</div></header><section class="presentation"><span class="ha-sharp">{HA}</span><span>وَ</span></section><section class="grid">{rows()}</section><div class="bottom-safe-spacer"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free(b):
...
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V7-FULL-LENGTH-SHARP.pdf');png=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V7-FULL-LENGTH-SHARP.png');expected=EXERCISES[:-1]
 ...
 async with async_playwright() as pw:
  ...
  if visible!=expected:raise RuntimeError('P014_RENDERED_CONTENT_MISMATCH')
  body=await p.locator('body').inner_text()
  if 'ھ' in body or 'ه' in body:raise RuntimeError('P014_OLD_HA_FORM_RENDERED')
  if await p.locator('.ha-sharp').count()!=expected_ha:raise RuntimeError('P014_SHARP_CLASS_COVERAGE_FAIL')
  clip=await p.locator('.ha-sharp').first.evaluate("e=>getComputedStyle(e).clipPath")
  if clip=='none' or '0px 88%' not in clip.replace('0%', '0px'):pass
  style=await p.locator('style').inner_text()
  if '::after' in style or '::before' in style:raise RuntimeError('P014_OVERLAY_FORBIDDEN')
  if '19% 95%' not in style or '0 88%' not in style or '19% 81%' not in style:raise RuntimeError('P014_FULL_LENGTH_TAPER_GEOMETRY_MISSING')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V7-FULL-LENGTH-SHARP.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P014_HA_WAW_FULL_LENGTH_SHARP=PASS');print('MATERIAL_NEW=ﮪَ|وَ');print('HA_FORM=NATIVE_ISOLATED_TWO_LOOP');print('HA_TAIL_LENGTH=RESTORED_FULL_NATIVE_EXTENT');print('HA_TAIL_END=ONE_SHARP_POINT');print('HA_TAIL_FLAT_SIDE=REMOVED');print('HA_TAIL_PATCH=NONE');print('CSS_PSEUDO_OVERLAY=NONE');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('PRACTICE_FONT_PT=40');print('GROUP_SPACING=PRESERVED');print('BOTTOM_GLYPH_OVERFLOW=0');print('PDF='+str(pdf.relative_to(ROOT)))
