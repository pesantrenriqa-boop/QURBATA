#!/usr/bin/env python3
# QURBATA Jilid 1 P014 — هَ وَ. Same frozen cumulative baseline as P013.
...
FOCUS={'هَ','وَ'};REVIEW={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ','لَ','مَ','نَ'}
EXERCISES=['هَ ءَ','وَ أَ','هَ بَ','وَ تَ','ثَ هَ','جَ وَ','حَ هَ','خَ وَ','هَ دَ هَ','وَ ذَ وَ','هَ رَ هَ','وَ زَ وَ','هَ سَ هَ','وَ شَ وَ','هَ صَ هَ','وَ ضَ وَ','طَ هَ ظَ','عَ وَ غَ','فَ هَ قَ','كَ وَ لَ','مَ هَ نَ','ءَ وَ أَ','بَ هَ تَ','ثَ وَ جَ']
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">14</div></header><section class="presentation"><span>هَ</span><span>وَ</span></section><section class="grid">{rows()}</section><div class="bottom-safe-spacer"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free(b):
...
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V1-CUMULATIVE.pdf');png=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V1-CUMULATIVE.png');expected=EXERCISES[:-1]
 async with async_playwright() as pw:
  ...
  for e in visible[:8]:
   t=e.split()
   if len(t)!=2 or len(set(t)&FOCUS)!=1 or len(set(t)&REVIEW)!=1:raise RuntimeError(f'P014_VISUAL_FIRST_TWO_ROWS_PATTERN_FAIL={e}')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V1-CUMULATIVE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P014_HA_WAW_CUMULATIVE=PASS');print('MATERIAL_NEW=هَ|وَ');print('ROWS_1_2=NEW_PLUS_PREVIOUS_VERIFIED');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('PRACTICE_FONT_PT=40');print('GROUP_SPACING=PRESERVED');print('BOTTOM_GLYPH_OVERFLOW=0');print('PDF='+str(pdf.relative_to(ROOT)))
