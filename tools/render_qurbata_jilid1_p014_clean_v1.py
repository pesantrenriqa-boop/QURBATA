...
# No CSS tail patch, pseudo-element, or raster overlay.
...
# U+FBAA: ARABIC LETTER HEH DOACHASHMEE ISOLATED FORM. Native complete outline only.
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">14</div></header><section class="presentation"><span>{HA}</span><span>وَ</span></section><section class="grid">{rows()}</section><div class="bottom-safe-spacer"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free(b):
...
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V5-NATIVE-ISOLATED.pdf');png=free(OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V5-NATIVE-ISOLATED.png');expected=EXERCISES[:-1]
 async with async_playwright() as pw:
  ...
  # No custom overlays are allowed on the glyph.
  if '::after' in await p.locator('style').inner_text():raise RuntimeError('P014_CUSTOM_TAIL_OVERLAY_PRESENT')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P014-HA-WAW-CANDIDATE-V5-NATIVE-ISOLATED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P014_HA_WAW_NATIVE_ISOLATED=PASS');print('MATERIAL_NEW=ﮪَ|وَ');print('HA_FORM=NATIVE_ISOLATED_TWO_LOOP');print('HA_TAIL_PATCH=NONE');print('CSS_PSEUDO_OVERLAY=NONE');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('PRACTICE_FONT_PT=40');print('GROUP_SPACING=PRESERVED');print('BOTTOM_GLYPH_OVERFLOW=0');print('PDF='+str(pdf.relative_to(ROOT)))
