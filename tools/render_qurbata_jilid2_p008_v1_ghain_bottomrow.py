#!/usr/bin/env python3
"""QURBATA Jilid 2 P008 — acquisition of غ; no madd/hamzah/future-letter leakage."""
...
p001.MICRO=MICRO
# غ is active; ظ and later joining families remain forbidden.
p001.P001_BANNED_JOINING=set('ظفقكلمنيه')
...
.p008-enrichment-row{grid-column:1/-1!important;grid-row:8!important;display:grid!important;grid-template-columns:1fr 1fr!important;gap:10mm!important;padding:.55mm 6mm .35mm!important;border-top:.28mm solid #111!important;box-sizing:border-box!important;background:#fff!important;}
.p008-enrichment-row .micro{display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;text-align:center!important;min-width:0!important;}
.p008-enrichment-row .micro-label{font-family:Arial,sans-serif!important;font-size:6.2pt!important;font-weight:700!important;margin:0 0 .7mm!important;white-space:nowrap!important;}
...
def build_p008(debug:bool):
    ...
    pres=f'''<section class="presentation"><div class="presentation-object-wrap"><div class="presentation-object" dir="ltr"><span class="arabic-part" lang="ar" dir="rtl">{p001.arabic_html('غَ')}</span><span class="arrow" dir="ltr">←</span><span class="arabic-part" lang="ar" dir="rtl">{p001.arabic_html('غَرَبَ')}</span></div></div></section>'''
    ...
    enrichment=f'''<div class="p008-enrichment-row" data-enrichment-row="8"><div class="micro"><div class="micro-label">ANGKA ARAB</div><div class="glyph-run num">{nums}</div></div><div class="micro"><div class="micro-label">PEMUTUS SAMBUNGAN</div><div class="glyph-run nonjoin">{njs}</div></div></div>'''
...
async def _write_pdf(page,out:Path):
    names=[out/'QURBATA-JILID-2-P008-V1-GHAIN-BOTTOMROW.pdf']+[out/f'QURBATA-JILID-2-P008-V1-GHAIN-BOTTOMROW-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
    last=None
    for idx,p in enumerate(names):
        try:
            ...
            return p,('DIRECT_P008' if idx==0 else f'LOCK_FALLBACK_P008_{idx:02d}')
        except PermissionError as e:last=e
    raise RuntimeError('P008_NO_AVAILABLE_PDF_NAME') from last
...
async def render_p008(h:Path,out:Path,debug:bool):
    ...
    async with async_playwright() as pw:
        ...
        if await page.locator('.j2-object').count()!=28:raise RuntimeError('P008_CORE_OBJECT_COUNT_INVALID')
        metrics,issues=await p001.fit_and_inspect(page)
        extra=await page.evaluate('''()=>{const e=document.querySelector('.p008-enrichment-row'),g=document.querySelector('.j2-grid'),f=document.querySelector('.footer'),out=[];if(!e||!g)return[{kind:'P008_ENRICHMENT_MISSING'}];const er=e.getBoundingClientRect(),gr=g.getBoundingClientRect();if(er.left<gr.left-1||er.right>gr.right+1||er.bottom>gr.bottom+1)out.push({kind:'P008_ENRICHMENT_OUTSIDE_GRID'});if(f&&er.bottom>f.getBoundingClientRect().top-2)out.push({kind:'P008_ENRICHMENT_FOOTER_COLLISION'});return out}''')
        all_issues=[*issues,*extra];report.write_text(json.dumps(all_issues,ensure_ascii=False,indent=2),encoding='utf-8')
        if all_issues:raise RuntimeError('P008_LAYOUT_ISSUES='+repr(all_issues))
        await page.screenshot(path=str(png/'page-008-v1.png'),full_page=True)
...
def main():
    ...
    current_ghain=[r for r in current if 'غ' in base_letters(r['word'])]
    if len(current_ghain)<8:raise ValueError(f'P008_LEXICAL_GATE_FAIL_GHAIN current={len(current_ghain)} required=8')
    if any(r['lexical_status']!='CURATED' or r['competency_status']!='ALLOWED' for r in current_ghain):raise ValueError('P008_LEXICAL_GATE_STATUS_FAIL')
    missing=[r['word'] for r in current if 'غ' not in base_letters(r['word'])]
    if missing:raise ValueError('P008_CURRENT_MISSING_GHAIN='+repr(missing))
    leaks=[]
    for r in lex:
    ...
    if premature:raise ValueError('P008_PREMATURE_MADD_OR_HAMZAH='+repr(premature))
    if '--output-dir' not in sys.argv[1:]:sys.argv.extend(['--output-dir','dist/qurbata-print-ready/jilid-2/pages/P008'])
    rc=v22.main()
    print('JILID2_P008_RENDERER_V1=PASS');print('PAGE=8');print('ACQUISITION_LETTERS=غ');print('TITLE_VISUAL_RIGHT_TO_LEFT=غَ←غَرَبَ')
    print('LEXICAL_ACQUISITION_GATE=MIN_8_CURRENT_PER_NEW_LETTER');print(f'CURRENT_GHAIN_OBJECTS={len(current_ghain)}');print('PREMATURE_MADD_OR_HAMZAH=0');print('FUTURE_LETTER_LEAKAGE=0');print('ZA_STATUS=DEFERRED_UNTIL_CORPUS_GATE_PASS')
    print('CORE_PRACTICE_ROWS=7');print('CORE_PRACTICE_OBJECTS=28');print('REGISTRY_OBJECTS=32_PRESERVED');print('PRESENTATION_FONT_SIZE=34PT');print('PRACTICE_FONT_SIZE=39PT')
    print('ENRICHMENT_GRID_ROW=8_NATIVE');print('ENRICHMENT_BLOCK_POLICY=CONTINUE_UNTIL_MASTERY');print('ENRICHMENT_CATEGORY=E02|E06');print('ENRICHMENT_LAYOUT=INDIVIDUAL_FIXED_WIDTH_GLYPHS')
    print('OUTPUT_DIR=dist/qurbata-print-ready/jilid-2/pages/P008');return rc
