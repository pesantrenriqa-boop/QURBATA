...
fatha/damma/kasra independently from the base run. This avoids both failure modes
seen in earlier candidates:
- splitting every combining mark into inline spans, which disturbed Arabic shaping;
- relying entirely on the font's native mark anchors, whose vertical placement is
  visually too high for the QURBATA practice layout at 36pt.

V15 strips only the three short-vowel marks from the visible shaping run, keeps all
base letters joined natively, then overlays each mark on the DOM Range box of its
own base character. Horizontal fitting remains scaleX-only; no whole-word Y shift.
"""
...
# Extra CSS is appended to the already-stable P001 V12 shell.
p001.P001_CSS += r'''
...
async def fit_joined_cluster_overlay(page):
    """Horizontal fit base run, then attach marks to per-character DOM Range boxes."""
    return await page.evaluate(r'''()=>{
      ...
      for(const slot of document.querySelectorAll('.j2-object')){
        ...
        // Idempotent cleanup for repeated calls.
        ...
        for(const ch of original){
          if(isMark(ch)){
            if(baseIndex>=0) attached.push({baseIndex,mark:ch});
          }else{
            bases.push(ch);baseIndex++;
          }
        }
        ...
        // Width fitting on the uninterrupted base-letter run only.
        ...
        // Obtain actual browser cluster rectangles after contextual joining + scaleX.
        ...
        if(node && node.nodeType===Node.TEXT_NODE){
          for(const item of attached){
            ...
            range.setStart(node,item.baseIndex);range.setEnd(node,item.baseIndex+1);
            ...
            span.dataset.baseIndex=String(item.baseIndex);
            span.dataset.mark=item.mark;

            // X is the actual contextual base cluster centre.
            const x=(r.left+r.right)/2-s.left;

            // Y policy is tied to the cluster line box, not to another row.
            // Fatha/damma are deliberately closer to the base than Amiri Quran's
            // native anchors; damma is slightly lower than fatha. Kasra sits below.
            const top=r.top-s.top, bottom=r.bottom-s.top, h=r.height;
            let y;
            if(item.mark===FATHA){y=top + h*0.18;fatha++;}
            else if(item.mark===DAMMA){y=top + h*0.25;damma++;}
            else {y=bottom - h*0.06;kasra++;}
...
# Replace legacy/native vertical fitter only. P001 content, competency gates, header,
# footer and page geometry remain owned by the stable P001 renderer.
    ...
    extra = await page.evaluate(r'''()=>{
      ...
      for(const slot of document.querySelectorAll('.j2-object')){
        ...
        for(const mark of slot.querySelectorAll('.q-overlay-mark')){
          ...
          if(r.left<s.left-4||r.right>s.right+4||r.top<s.top-10||r.bottom>s.bottom+10){
            out.push({kind:'OVERLAY_MARK_OUTSIDE_LOGICAL_CELL',slot:slot.dataset.slot,mark:mark.dataset.mark,left:r.left,right:r.right,top:r.top,bottom:r.bottom,slotLeft:s.left,slotRight:s.right,slotTop:s.top,slotBottom:s.bottom});
          }
        }
      }
      return out;
...
def main():
    ...
    print('FATHA_CLUSTER_Y_RATIO=0.18')
    print('DAMMA_CLUSTER_Y_RATIO=0.25')
    print('KASRA_CLUSTER_Y_RATIO=0.94')
