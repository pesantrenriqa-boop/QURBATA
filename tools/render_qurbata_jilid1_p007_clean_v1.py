#!/usr/bin/env python3
# QURBATA Jilid 1 P007 — صَ ضَ. Content-verified production candidate.
...
def doc(font_uri):
    # IMPORTANT: p004.doc builds rows from p004 module globals; inject this page's data before calling it.
    m.EXERCISES=list(EXERCISES);m.FOCUS=set(FOCUS);m.REVIEW=list(REVIEW)
    s=m.doc(font_uri)
...
async def render(h):
    pdf=free(OUT/'QURBATA-JILID-1-P007-SAD-DAD-CANDIDATE-V3-CONTENT-VERIFIED.pdf')
    png=free(OUT/'QURBATA-JILID-1-P007-SAD-DAD-CANDIDATE-V3-CONTENT-VERIFIED.png')
    async with async_playwright() as pw:
        ...
        rendered=[norm(x) for x in await p.locator('.practice').all_inner_texts()]
        expected=[norm(x) for x in EXERCISES[:-1]]
        if rendered != expected: raise RuntimeError(f'P007_RENDERED_CONTENT_MISMATCH rendered={rendered} expected={expected}')
        # First two rows must visibly teach the current focus, never stale Sin/Shin content.
        first8=rendered[:8]
        if any(('صَ' not in x and 'ضَ' not in x) for x in first8): raise RuntimeError(f'P007_FIRST_TWO_ROWS_MISSING_FOCUS={first8}')
...
if __name__=='__main__':
    ...
    font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P007-SAD-DAD-CANDIDATE-V3-CONTENT-VERIFIED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h))
    print('QJ1_P007_SAD_DAD_CONTENT_VERIFIED=PASS');print('MATERIAL_NEW=صَ|ضَ');print('FIRST_TWO_ROWS=صَ|ضَ_FOCUS_VERIFIED');print('RENDERED_CONTENT_MATCH=23_OF_23');print('PRACTICE_FONT_PT=40');print('GROUP_SPACING=PRESERVED');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('BOTTOM_GLYPH_OVERFLOW=0');print('PDF='+str(pdf.relative_to(ROOT)))
