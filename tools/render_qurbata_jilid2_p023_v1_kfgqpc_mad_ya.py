#!/usr/bin/env python3
"""QURBATA Jilid 2 P023 — K2 U02 mad ya using Uthmani sukun and final dammah."""
...
UTHMANI_SUKUN=chr(0x06E1)
ROUND_SUKUN=chr(0x0652)
MARKS=set(chr(c) for c in range(0x064B,0x0660))|{UTHMANI_SUKUN,'ـ'}
...
def has_mad_ya(word):
    chars=list(word)
    return any(ch=='ي' and i>=1 and chars[i-1]=='ِ' and i+1<len(chars) and chars[i+1]==UTHMANI_SUKUN for i,ch in enumerate(chars))
for r in lex:
    n=len(bases(r['word']))
    ...
    if ROUND_SUKUN in r['word']: raise ValueError('P023_ROUND_SUKUN_FORBIDDEN='+repr(r))
    if not has_mad_ya(r['word']): raise ValueError('P023_UTHMANI_SUKUN_REQUIRED='+repr(r))
    if not r['word'].endswith('ُ'): raise ValueError('P023_FINAL_DAMMA_REQUIRED='+repr(r))
...
def build(debug):
    ...
    short='بِ'; unit='بِي'+UTHMANI_SUKUN; word='بِي'+UTHMANI_SUKUN+'عُ'
    pres=f'''<section class="presentation"><div class="presentation-object-wrap"><div class="presentation-object"><span class="arabic-part" lang="ar">{word}</span><span class="arrow">←</span><span class="arabic-part mad-unit" lang="ar">{unit}</span><span class="arrow">←</span><span class="arabic-part" lang="ar">{short}</span></div></div></section>'''
    h=h[:s]+pres+h[e:]
...
async def render(h,out,debug):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P023-V3.json'; png=out/'png'; png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        if ROUND_SUKUN in text: raise RuntimeError('P023_RENDERED_ROUND_SUKUN_FORBIDDEN')
        if UTHMANI_SUKUN not in text: raise RuntimeError('P023_RENDERED_UTHMANI_SUKUN_MISSING')
        ...
        pdf=out/'QURBATA-JILID-2-P023-V3-KFGQPC-MAD-YA-UTHMANI-SUKUN.pdf'; await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'}); await browser.close()
    return metrics,report,pdf
p001.render=render
def main():
    lengths={3:0,4:0,5:0}
    for r in lex: lengths[len(bases(r['word']))]+=1
    rc=v22.main(); print('JILID2_P023_RENDERER_V3=PASS'); print('PAGE=23'); print('K_SEQUENCE=K2'); print('UK_SEQUENCE=J2.K2.U02'); print('SUKUN_CODEPOINT=U+06E1'); print('SUKUN_STYLE=DOTLESS_HEAD_OF_KHAH'); print('ROUND_SUKUN_U+0652=FORBIDDEN'); print('FINAL_HARAKAT=DAMMA'); print('CURRENT_OBJECTS=32'); print('MUROJAAH_OBJECTS=0'); print('WORD_LENGTH_3_COUNT='+str(lengths[3])); print('WORD_LENGTH_4_COUNT='+str(lengths[4])); print('WORD_LENGTH_5_COUNT='+str(lengths[5])); print('ARABIC_FONT_PRIMARY=QURBATA KFGQPC Uthman Taha Naskh'); print('FONT_BINDING_GATE=PASS'); print('STATUS=P023_UTHMANI_SUKUN_CANDIDATE_NOT_FROZEN'); return rc
