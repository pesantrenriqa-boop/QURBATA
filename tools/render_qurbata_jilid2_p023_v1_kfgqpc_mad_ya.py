#!/usr/bin/env python3
"""QURBATA Jilid 2 P023 — K2 U02 mad ya, native KFGQPC shaping, final dammah."""
...
for r in lex:
    ...
    if ROUND_SUKUN in r['word'] or UTHMANI_HEAD in r['word']: raise ValueError('P023_SYNTHETIC_SUKUN_FORBIDDEN='+repr(r))
...
async def render(h,out,debug):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P023-V4.json'; png=out/'png'; png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        if ROUND_SUKUN in text or UTHMANI_HEAD in text: raise RuntimeError('P023_SYNTHETIC_SUKUN_RENDERED')
        ...
        pdf=out/'QURBATA-JILID-2-P023-V4-KFGQPC-MAD-YA-NATIVE.pdf'; await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'}); await browser.close()
    return metrics,report,pdf
p001.render=render
def main():
    lengths={3:0,4:0,5:0}
    for r in lex: lengths[len(bases(r['word']))]+=1
    rc=v22.main(); print('JILID2_P023_RENDERER_V4=PASS'); print('PAGE=23'); print('K_SEQUENCE=K2'); print('UK_SEQUENCE=J2.K2.U02'); print('MAD_YA_ORTHOGRAPHY=KASRA_PLUS_BARE_YA'); print('SYNTHETIC_SUKUN=FORBIDDEN'); print('ROUND_SUKUN_U+0652=FORBIDDEN'); print('UTHMANI_HEAD_U+06E1=FORBIDDEN'); print('FINAL_HARAKAT=DAMMA'); print('CURRENT_OBJECTS=32'); print('MUROJAAH_OBJECTS=0'); print('ARABIC_FONT_PRIMARY=QURBATA KFGQPC Uthman Taha Naskh'); print('FONT_BINDING_GATE=PASS'); print('STATUS=P023_NATIVE_KFGQPC_RECOVERY_CANDIDATE_NOT_FROZEN'); return rc
