#!/usr/bin/env python3
"""QURBATA Jilid 2 P023 — third mad-alif page, diverse 3–5 letter transfer."""
...
for r in lex:
    ...
    if r['function']!='CURRENT': raise ValueError('P023_NON_FOCUS_OBJECT_FORBIDDEN='+repr(r))
    if not has_mad_alif(r['word']): raise ValueError('P023_MAD_ALIF_REQUIRED='+repr(r))
    if has_future_mad_ya(r['word']) or has_future_mad_waw(r['word']): raise ValueError('P023_FUTURE_MAD_LEAKAGE='+repr(r))
...
def build(debug):
    ...
    pres='''<section class="presentation"><div class="presentation-object-wrap"><div class="presentation-object"><span class="arabic-part" lang="ar">بَابُ</span><span class="arrow">←</span><span class="arabic-part mad-unit" lang="ar">بَا</span><span class="arrow">←</span><span class="arabic-part" lang="ar">بَ</span></div></div></section>'''
    h=h[:s]+pres+h[e:]
...
async def render(h,out,debug):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P023-MAD-ALIF-V1.json'; png=out/'png'; png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        pdf=out/'QURBATA-JILID-2-P023-V1-KFGQPC-MAD-ALIF-DIVERSE.pdf'; await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'}); await browser.close()
    return metrics,report,pdf
p001.render=render
def main():
    lengths={3:0,4:0,5:0}
    for r in lex: lengths[len(bases(r['word']))]+=1
    rc=v22.main(); print('JILID2_P023_MAD_ALIF_RENDERER_V1=PASS'); print('PAGE=23'); print('K_SEQUENCE=K2'); print('UK_SEQUENCE=J2.K2.U01C'); print('HEADER_SEQUENCE=بَ|بَا|بَابُ'); print('FOCUS=MAD_ALIF_THIRD_PAGE'); print('MAD_PATTERN=FATHA_PLUS_ALIF'); print('MAD_YA=DEFERRED'); print('MAD_WAW=DEFERRED'); print('CURRENT_OBJECTS=32'); print('WORD_LENGTH_3_COUNT='+str(lengths[3])); print('WORD_LENGTH_4_COUNT='+str(lengths[4])); print('WORD_LENGTH_5_COUNT='+str(lengths[5])); print('UPPER_COMPETENCY_MARKS=SUKUN|TANWIN|SHADDA_FORBIDDEN'); print('ARABIC_FONT_PRIMARY=QURBATA KFGQPC Uthman Taha Naskh'); print('FONT_BINDING_GATE=PASS'); print('STATUS=P023_THIRD_MAD_ALIF_CANDIDATE_NOT_FROZEN'); return rc
