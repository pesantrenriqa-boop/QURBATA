...
INJECT_JS='''({items,label})=>{
 ...
 grid.style.height='132mm';grid.style.maxHeight='132mm';grid.style.boxSizing='border-box';grid.style.marginBottom='0';grid.style.rowGap='1.7mm';
 ...
 Object.assign(box.style,{position:'absolute',left:'11mm',right:'11mm',bottom:'18mm',height:'11.5mm',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'.3mm 2.2mm .35mm',borderTop:'.28mm solid #111',borderBottom:'.18mm solid #bbb',boxSizing:'border-box',background:'#fff',textAlign:'center',overflow:'hidden',zIndex:'20'});
 const lab=document.createElement('div');lab.className='micro-label';lab.textContent=label;Object.assign(lab.style,{fontFamily:'Arial,sans-serif',fontSize:'5.2pt',fontWeight:'700',margin:'0 0 .2mm',lineHeight:'1'});box.appendChild(lab);
 const run=document.createElement('div');run.className='awail-run';Object.assign(run.style,{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-evenly',fontFamily:'KFGQPC Uthman Taha Naskh, Amiri Quran, Amiri, serif',fontSize:'25pt',lineHeight:'.98',direction:'rtl',unicodeBidi:'isolate',whiteSpace:'nowrap',letterSpacing:'0',boxSizing:'border-box'});
 for(const t of items){const sp=document.createElement('span');sp.className='awail-item';sp.textContent=t;Object.assign(sp.style,{display:'inline-block',margin:'0 .5mm',flex:'0 0 auto'});run.appendChild(sp)}
 box.appendChild(run);page.appendChild(box);return {ok:true,pageClass:page.className};
}'''

async def _write_pdf(page,out:Path):
 names=[out/'QURBATA-JILID-2-P009-V9-KAF-LAM-AWAILUSSURAR-DOM-INJECTED.pdf']+[out/f'QURBATA-JILID-2-P009-V9-KAF-LAM-AWAILUSSURAR-DOM-INJECTED-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
 last=None
 for idx,p in enumerate(names):
  try:
   await page.pdf(path=str(p),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});return p,('DIRECT_P009_V9' if idx==0 else f'LOCK_FALLBACK_P009_V9_{idx:02d}')
  except PermissionError as e:last=e
 raise RuntimeError('P009_NO_AVAILABLE_PDF_NAME') from last

async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P009-V9.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  await page.screenshot(path=str(png/'page-009-v9.png'),full_page=True);pdf,mode=await _write_pdf(page,out);await b.close()
...
def main():
 ...
 rc=v22.main();print('JILID2_P009_RENDERER_V9_DOM_INJECTED=PASS');print('ACQUISITION_LETTERS=ك|ل');print('PRACTICE_MODE=JOINING_FORM_DRILL');print('CORE_DRILL_LENGTH=3_LETTERS_ONLY');print(f'FORM_KAF_OBJECTS={kaf}');print(f'FORM_LAM_OBJECTS={lam}');print('SHORT_VOWELS=FATHAH|KASRAH|DAMMAH');print('TITLE_VISUAL_RIGHT_TO_LEFT=كَ←كُتِبَ|لَ←لَبِثَ');print(f'BOTTOM_ROW_ITEM_COUNT={len(items)}');print('BOTTOM_ROW_FONT_SIZE_PT=25');print('BOTTOM_ROW_LAYOUT=DOM_INJECTED_PAGE_CHILD');print('PDF_WRITE_POLICY=INCREMENTAL_LOCK_SAFE_01_99');return rc
