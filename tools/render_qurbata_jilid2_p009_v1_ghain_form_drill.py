...
.j2-grid{grid-template-rows:repeat(7,minmax(0,1fr))!important;height:132mm!important;max-height:132mm!important;box-sizing:border-box!important;margin-bottom:0!important}
.p009-enrichment-row{position:absolute!important;left:11mm!important;right:11mm!important;bottom:14.5mm!important;height:13.5mm!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;padding:.5mm 2.5mm .7mm!important;border-top:.28mm solid #111!important;border-bottom:.18mm solid #bbb!important;box-sizing:border-box!important;background:#fff!important;text-align:center!important;overflow:visible!important;z-index:5!important}
.p009-enrichment-row .micro-label{font-family:Arial,sans-serif!important;font-size:5.5pt!important;font-weight:700!important;margin:0 0 .4mm!important;line-height:1!important}
.p009-enrichment-row .awail-run{width:100%!important;display:flex!important;align-items:center!important;justify-content:space-evenly!important;font-family:'KFGQPC Uthman Taha Naskh','Amiri Quran','Amiri',serif!important;font-size:26pt!important;line-height:1.15!important;direction:rtl!important;unicode-bidi:isolate!important;white-space:nowrap!important;letter-spacing:0!important;padding-top:.4mm!important;padding-bottom:.4mm!important;box-sizing:border-box!important}
.p009-enrichment-row .awail-item{display:inline-block!important;margin:0 .7mm!important;flex:0 0 auto!important;overflow:visible!important}
'''
...
def build(debug):
 ...
 page_end=h.rfind('</div>')
 if page_end<0: raise ValueError('P009_PAGE_CONTAINER_NOT_FOUND')
 return h[:page_end]+en+h[page_end:]
p001.build_page_html=build
async def _write_pdf(page,out:Path):
 names=[out/'QURBATA-JILID-2-P009-V6-KAF-LAM-AWAILUSSURAR-DEDICATED-SAFEZONE.pdf']+[out/f'QURBATA-JILID-2-P009-V6-KAF-LAM-AWAILUSSURAR-DEDICATED-SAFEZONE-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
 last=None
 for idx,p in enumerate(names):
  try:
   await page.pdf(path=str(p),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});return p,('DIRECT_P009_V6' if idx==0 else f'LOCK_FALLBACK_P009_V6_{idx:02d}')
  except PermissionError as e:last=e
 raise RuntimeError('P009_NO_AVAILABLE_PDF_NAME') from last
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P009-V6.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  await page.screenshot(path=str(png/'page-009-v6.png'),full_page=True);pdf,mode=await _write_pdf(page,out);await b.close()
...
def main():
 ...
 rc=v22.main();print('JILID2_P009_RENDERER_V6_DEDICATED_SAFEZONE=PASS');print('ACQUISITION_LETTERS=ك|ل');print('PRACTICE_MODE=JOINING_FORM_DRILL');print('CORE_DRILL_LENGTH=3_LETTERS_ONLY');print(f'FORM_KAF_OBJECTS={kaf}');print(f'FORM_LAM_OBJECTS={lam}');print('SHORT_VOWELS=FATHAH|KASRAH|DAMMAH');print('TITLE_VISUAL_RIGHT_TO_LEFT=كَ←كُتِبَ|لَ←لَبِثَ');print('PREMATURE_MADD_OR_HAMZAH=0');print('PREMATURE_MARK_LEAKAGE=0');print('FUTURE_LETTER_LEAKAGE=0');print(f'BOTTOM_ROW_ITEM_COUNT={len(items)}');print('BOTTOM_ROW_FONT_SIZE_PT=26');print('CORE_GRID_HEIGHT_MM=132');print('BOTTOM_ROW_SAFEZONE_HEIGHT_MM=13.5');print('BOTTOM_ROW_BOTTOM_OFFSET_MM=14.5');print('BOTTOM_ROW_LAYOUT=ABSOLUTE_DEDICATED_SAFEZONE');print('PDF_WRITE_POLICY=INCREMENTAL_LOCK_SAFE_01_99');return rc
