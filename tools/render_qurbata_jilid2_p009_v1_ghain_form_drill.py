...
def build(debug):
 ...
 nums=''.join(f'<span class="eg">{x}</span>' for x in '٠١٢٣٤٥٦٧٨٩');njs=''.join(f'<span class="eg">{x}</span>' for x in 'ادذرزو');en=f'''<div class="p009-enrichment-row"><div class="micro"><div class="micro-label">ANGKA ARAB</div><div class="glyph-run num">{nums}</div></div><div class="micro"><div class="micro-label">PEMUTUS SAMBUNGAN</div><div class="glyph-run nonjoin">{njs}</div></div></div>''';g=h.find('<section class="j2-grid">');ge=h.find('</section>',g);return h[:ge]+en+h[ge:]
p001.build_page_html=build
async def _write_pdf(page,out:Path):
 names=[out/'QURBATA-JILID-2-P009-V2-KAF-LAM-MIXED-VOWELS.pdf']+[out/f'QURBATA-JILID-2-P009-V2-KAF-LAM-MIXED-VOWELS-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
 last=None
 for idx,p in enumerate(names):
  try:
   await page.pdf(path=str(p),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});return p,('DIRECT_P009_V2' if idx==0 else f'LOCK_FALLBACK_P009_V2_{idx:02d}')
  except PermissionError as e:last=e
 raise RuntimeError('P009_NO_AVAILABLE_PDF_NAME') from last
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P009-V2.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  if issues:raise RuntimeError('P009_LAYOUT_ISSUES='+repr(issues))
  await page.screenshot(path=str(png/'page-009-v2.png'),full_page=True);pdf,mode=await _write_pdf(page,out);await b.close()
