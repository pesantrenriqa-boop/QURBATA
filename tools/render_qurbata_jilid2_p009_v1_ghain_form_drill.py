...
p001.P001_CSS+=r'''.presentation-object{font-size:34pt!important;direction:ltr!important;flex-direction:row-reverse!important;gap:1.7mm!important}.presentation-object .arabic-part{direction:rtl!important;line-height:1.15!important}.presentation-object .arrow{font-size:15pt!important}.j2-glyph{font-size:39pt!important}.j2-grid{grid-template-rows:repeat(8,minmax(0,1fr))!important}.p009-enrichment-row{grid-column:1/-1!important;grid-row:8!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;padding:.55mm 6mm .35mm!important;border-top:.28mm solid #111!important;box-sizing:border-box!important;background:#fff!important;text-align:center!important}.p009-enrichment-row .micro-label{font-family:Arial,sans-serif!important;font-size:6.2pt!important;font-weight:700!important;margin:0 0 .8mm!important}.p009-enrichment-row .awail-run{font-family:'KFGQPC Uthman Taha Naskh','Amiri Quran','Amiri',serif!important;font-size:24pt!important;line-height:1.05!important;direction:rtl!important;unicode-bidi:isolate!important;white-space:nowrap!important;letter-spacing:0!important}.p009-enrichment-row .awail-item{display:inline-block!important;margin:0 4mm!important}'''
...
def build(debug):
 ...
 awail=''.join(f'<span class="awail-item">{p001.arabic_html(x.strip())}</span>' for x in enrich['Content'].split('|'))
 en=f'''<div class="p009-enrichment-row" data-enrichment-step="{enrich['StepCode']}"><div class="micro-label">{enrich['Label']}</div><div class="awail-run">{awail}</div></div>''';g=h.find('<section class="j2-grid">');ge=h.find('</section>',g);return h[:ge]+en+h[ge:]
p001.build_page_html=build
async def _write_pdf(page,out:Path):
 names=[out/'QURBATA-JILID-2-P009-V3-KAF-LAM-MIXED-VOWELS-AWAILUSSURAR.pdf']+[out/f'QURBATA-JILID-2-P009-V3-KAF-LAM-MIXED-VOWELS-AWAILUSSURAR-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
 last=None
 for idx,p in enumerate(names):
  try:
   await page.pdf(path=str(p),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});return p,('DIRECT_P009_V3' if idx==0 else f'LOCK_FALLBACK_P009_V3_{idx:02d}')
  except PermissionError as e:last=e
 raise RuntimeError('P009_NO_AVAILABLE_PDF_NAME') from last
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P009-V3.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  b=await pw.chromium.launch();page=await b.new_page(viewport={'width':1120,'height':1584},device_scale_factor=2);await page.goto(h.resolve().as_uri(),wait_until='networkidle');await page.evaluate('document.fonts.ready');metrics,issues=await p001.fit_and_inspect(page);extra=await page.evaluate('''()=>{const e=document.querySelector('.p009-enrichment-row'),g=document.querySelector('.j2-grid'),out=[];if(!e||!g)return[{kind:'P009_ENRICHMENT_MISSING'}];const er=e.getBoundingClientRect(),gr=g.getBoundingClientRect();if(er.left<gr.left-1||er.right>gr.right+1||er.bottom>gr.bottom+1)out.push({kind:'P009_ENRICHMENT_OUTSIDE_GRID'});return out}''');all_issues=[*issues,*extra];report.write_text(json.dumps(all_issues,ensure_ascii=False,indent=2),encoding='utf-8')
  if all_issues:raise RuntimeError('P009_LAYOUT_ISSUES='+repr(all_issues))
  await page.screenshot(path=str(png/'page-009-v3.png'),full_page=True);pdf,mode=await _write_pdf(page,out);await b.close()
...
def main():
 ...
 rc=v22.main();print('JILID2_P009_RENDERER_V3_KAF_LAM_MIXED_VOWELS=PASS');print('ACQUISITION_LETTERS=ك|ل');print('PRACTICE_MODE=JOINING_FORM_DRILL');print('CORE_DRILL_LENGTH=3_LETTERS_ONLY');print(f'FORM_KAF_OBJECTS={kaf}');print(f'FORM_LAM_OBJECTS={lam}');print('SHORT_VOWELS=FATHAH|KASRAH|DAMMAH');print('TITLE_VISUAL_RIGHT_TO_LEFT=كَ←كُتِبَ|لَ←لَبِثَ');print('PREMATURE_MADD_OR_HAMZAH=0');print('PREMATURE_MARK_LEAKAGE=0');print('FUTURE_LETTER_LEAKAGE=0');print(f'BOTTOM_ROW_ENRICHMENT_STEP={enrich["StepCode"]}');print(f'BOTTOM_ROW_ENRICHMENT_CATEGORY={enrich["Category"]}');print(f'BOTTOM_ROW_ENRICHMENT_LABEL={enrich["Label"]}');print('BOTTOM_ROW_ROTATION=ENABLED');print('PDF_WRITE_POLICY=INCREMENTAL_LOCK_SAFE_01_99');return rc
