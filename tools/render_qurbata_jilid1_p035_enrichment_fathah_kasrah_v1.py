...
# Enrichment: no new harakat. Mixed fathah+kasrah with broad alphabet coverage.
EX=['بَ تِ ثَ','جِ حَ خِ','دَ ذِ رَ','زِ سَ شِ','صَ ضِ طَ','ظِ عَ غِ','فَ قِ كَ','لِ مَ نِ',
    f'{HA_F} وِ يَ',f'{HA_K} بَ تِ','ثَ جِ حَ','خِ دَ ذِ','رَ زِ سَ','شِ صَ ضِ','طَ ظِ عَ','غِ فَ قِ','كَ لِ مَ','نِ وَ يِ',
    'ءَ إِ بَ','تِ ثَ جِ','حَ خِ دَ','ذِ رَ زِ','سَ شِ صَ']
...
def audit():
 ...
 if len(ts)!=69:raise RuntimeError(f'P035_TOKEN_COUNT_FAIL {len(ts)}')
 if abs(fath-kas)>3:raise RuntimeError(f'P035_HARAKAT_IMBALANCE fath={fath} kas={kas}')
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P035-PENGAYAAN-FATHAH-KASRAH-01-CANDIDATE-V1.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P035-PENGAYAAN-FATHAH-KASRAH-01-CANDIDATE-V1.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P035_ENRICHMENT=PASS');print('PAGE_TYPE=FATHAH_KASRAH_ENRICHMENT');print('HARAKAT_NEW=NONE');print('PATTERN=THREE_LETTER_MIXED');print('HA_GLYPH=TWO_HOLE_ONLY');print('PDF='+str(pdf.relative_to(ROOT)))
