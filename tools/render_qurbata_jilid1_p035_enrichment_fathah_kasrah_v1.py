...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P035-PENGAYAAN-FATHAH-KASRAH-01-CANDIDATE-V4-LARGE-BALANCED.pdf'
 async with async_playwright() as pw:
  ...
  if g['left']<8 or g['right']<8 or g['minVisualGap']<10 or g['rows']!=7:raise RuntimeError('P035_LARGE_SPACING_SAFE_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P035-PENGAYAAN-FATHAH-KASRAH-01-CANDIDATE-V4-LARGE-BALANCED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P035_ENRICHMENT=PASS');print('PRACTICE_FONT_PT=34');print('NAME_FONT_PT=38');print('TYPE_SCALE=NEAR_MATCH');print('INTRA_GROUP_SPACING_MM=3.2');print('VISUAL_SPACING_AUDIT=PASS');print('HA_GLYPH=TWO_HOLE_ONLY');print('PDF='+str(pdf.relative_to(ROOT)))
