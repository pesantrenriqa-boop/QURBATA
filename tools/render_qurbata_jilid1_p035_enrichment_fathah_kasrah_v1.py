...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P035-PENGAYAAN-FATHAH-KASRAH-01-CANDIDATE-V3-SPACED.pdf'
 async with async_playwright() as pw:
  ...
  if g['left']<12 or g['right']<12 or g['minGroupGap']<20 or g['rows']!=7:raise RuntimeError('P035_SPACING_SAFE_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P035-PENGAYAAN-FATHAH-KASRAH-01-CANDIDATE-V3-SPACED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P035_ENRICHMENT=PASS');print('LAYOUT=7_ROWS_X_3_GROUPS_FIXED');print('GROUP_CELL_WIDTH_MM=34');print('GROUP_SPACING_MM=7');print('INTRA_GROUP_SPACING_MM=3.8');print('PRACTICE_FONT_PT=30');print('SPACING_AUDIT=PASS');print('HA_GLYPH=TWO_HOLE_ONLY');print('PDF='+str(pdf.relative_to(ROOT)))
