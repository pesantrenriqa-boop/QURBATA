...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P019';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha';HA='ﮪَ'
LETTERS={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ','لَ','مَ','نَ',HA,'وَ','يَ'}
EX=['بَ ثَ جَ','تَ جَ حَ','ثَ حَ خَ','جَ خَ دَ','حَ دَ ذَ','خَ ذَ رَ','دَ رَ زَ','ذَ زَ سَ','رَ سَ شَ','زَ شَ صَ','سَ صَ ضَ','شَ ضَ طَ','صَ طَ ظَ','ضَ ظَ عَ','طَ عَ غَ','ظَ غَ فَ','عَ فَ قَ','غَ قَ كَ','فَ كَ لَ','قَ لَ مَ','كَ مَ نَ','لَ نَ '+HA,'مَ '+HA+' وَ','نَ وَ يَ']
# Stage 2 cumulative letter-name row: previous ا ب ت ث + new ج ح خ, repeated in order to fill width.
NAMES=['ا','ب','ت','ث','ج','ح','خ','ا','ب','ت','ث','ج','ح','خ']
...
def audit():
 if len(EX)!=24 or any(len(x.split())!=3 for x in EX):raise RuntimeError('P019_THREE_LETTER_STRUCTURE_FAIL')
 ts=[t for e in EX for t in e.split()]
 if any(t not in LETTERS for t in ts):raise RuntimeError('P019_OUTSIDE_SCOPE')
 if NAMES[:7]!=['ا','ب','ت','ث','ج','ح','خ']:raise RuntimeError('P019_NAME_ROW_ORDER_FAIL')
 if any(any(ch in n for ch in 'ًٌٍَُِّْ') for n in NAMES):raise RuntimeError('P019_NAME_ROW_MUST_BE_UNVOWELLED')
 if any(n not in UNVOWELLED_ALLOWED for n in NAMES):raise RuntimeError('P019_NAME_ROW_OUTSIDE_ALLOWED_SET')
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">19</div></header><section class="presentation">PENGAYAAN · KOMBINASI 3 HURUF</section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free(p):
 ...
 raise RuntimeError('P019_NO_FREE_OUTPUT')
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P019-PENGAYAAN-3-HURUF-NAMA-HURUF-02-CANDIDATE-V1.pdf');png=free(OUT/'QURBATA-JILID-1-P019-PENGAYAAN-3-HURUF-NAMA-HURUF-02-CANDIDATE-V1.png')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P019-PENGAYAAN-3-HURUF-NAMA-HURUF-02-CANDIDATE-V1.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P019_ENRICHMENT_NAMES=PASS');print('MAIN_MATERIAL=THREE_LETTER_ENRICHMENT');print('NAME_ROW=UNVOWELLED_FULL_WIDTH_CUMULATIVE');print('NAME_ROW_BASE_SEQUENCE=ا|ب|ت|ث|ج|ح|خ');print('NAME_ROW_NEW=ج|ح|خ');print('NAME_ROW_REVIEW=ا|ب|ت|ث');print('NAME_ROW_COUNT='+str(len(NAMES)));print('NAME_ROW_FONT_PT=39');print('NAME_ROW_LAYOUT=FULL_WIDTH_SPACE_BETWEEN');print('NAME_ROW_SEPARATOR=THIN_LINE');print('PDF='+str(pdf.relative_to(ROOT)))
