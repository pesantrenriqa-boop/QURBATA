#!/usr/bin/env python3
# QURBATA Jilid 1 P016 — Pengayaan I: langsung kombinasi 3 huruf, seluruh kompetensi P001-P015.
...
def audit():
 if len(EX)!=24 or any(len(x.split())!=3 for x in EX):raise RuntimeError('P016_THREE_LETTER_STRUCTURE_FAIL')
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">16</div></header><section class="presentation">PENGAYAAN · KOMBINASI 3 HURUF</section><section class="grid">{rows()}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free(p):
...
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P016-PENGAYAAN-3-HURUF-CANDIDATE-V2.pdf');png=free(OUT/'QURBATA-JILID-1-P016-PENGAYAAN-3-HURUF-CANDIDATE-V2.png')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P016-PENGAYAAN-3-HURUF-CANDIDATE-V2.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P016_ENRICHMENT=PASS');print('NEW_MATERIAL=NONE');print('COMPETENCY_SCOPE=P001-P015');print('ENRICHMENT_FOCUS=THREE_LETTER_MASTERY');print('LETTER_COVERAGE=29_OF_29');print('PDF='+str(pdf.relative_to(ROOT)))
