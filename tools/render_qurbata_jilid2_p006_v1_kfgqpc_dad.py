#!/usr/bin/env python3
"""QURBATA Jilid 2 P006 — acquisition of ض with cumulative P001-P005 review."""
...
if any(r['competency_status']!='ALLOWED' for r in lex): raise ValueError('P006_LEXICAL_COMPETENCY_NOT_ALLOWED')
...
if semantic_issues: raise ValueError('P006_THREE_LETTER_SEMANTIC_GATE_FAIL='+repr(semantic_issues))

# Through P006 acquired joining letters are ب ت ث ج ح خ س ش ص ض; nonjoiners ا د ذ ر ز و are review.
p001.MICRO=MICRO
...
async def render_p006(h:Path,out:Path,debug:bool):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P006-V1.json'; png=out/'png';png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        if count!=32: raise RuntimeError(f'P006_OBJECT_COUNT_INVALID actual={count} expected=32')
        ...
        if layout_issues:
            ...
            raise RuntimeError('P006_LAYOUT_ISSUES='+str(len(layout_issues))+' TYPES='+','.join(f'{k}:{v}' for k,v in sorted(kinds.items()))+f' REPORT={report}')
        await page.screenshot(path=str(png/'page-006.png'),full_page=True)
        pdf=out/'QURBATA-JILID-2-P006-V1-KFGQPC-DAD-CUMULATIVE.pdf'
...
def main():
    ...
    missing=[r['word'] for r in current if 'ض' not in base_letters(r['word'])]
    if missing: raise ValueError('P006_CURRENT_OBJECT_MISSING_DAD='+repr(missing))
    ...
    if missing_harakat: raise ValueError('P006_CUMULATIVE_HARAKAT_MISSING='+repr(missing_harakat))
    rc=v22.main()
    ...
    print('JILID2_P006_RENDERER_V1=PASS'); print('PAGE=6')
    ...
    print('ACQUISITION_LETTERS=ض'); print('REVIEW_LETTERS=ب|ت|ث|ج|ح|خ|س|ش|ص|ا|د|ذ|ر|ز|و')
    ...
    print('CUMULATIVE_COMPETENCY_P001_P005=PRESERVED')
    ...
    print('COMPETENCY_LEAKAGE=0'); print('ARABIC_FONT_PRIMARY=KFGQPC Uthman Taha Naskh'); print('PRACTICE_FONT_SIZE=42PT'); print('PRESENTATION_FONT_SIZE=46PT'); print('STATUS=P006_CUMULATIVE_CANDIDATE_NOT_FROZEN')
    return rc
