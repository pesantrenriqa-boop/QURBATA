...
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P006-V4.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  await page.screenshot(path=str(png/'page-006-v4.png'),full_page=True);pdf=out/'QURBATA-JILID-2-P006-V4-TA-ZA-FORM-DRILL.pdf';await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return metrics,report,pdf,'DIRECT_P006_V4'
p001.render=render
def main():
 core=lex[:28];ta=sum('ط' in base(r['word']) for r in core);za=sum('ظ' in base(r['word']) for r in core)
 ...
 rc=v22.main();print('JILID2_P006_RENDERER_V4_FORM_DRILL=PASS');print('ACQUISITION_LETTERS=ط|ظ');print('PRACTICE_MODE=JOINING_FORM_DRILL');print(f'FORM_TA_OBJECTS={ta}');print(f'FORM_ZA_OBJECTS={za}');print('TITLE_VISUAL_RIGHT_TO_LEFT=طَ←طَرَبَ|ظَ←حَظَرَ');print('PREMATURE_MADD_OR_HAMZAH=0');print('FUTURE_LETTER_LEAKAGE=0');return rc
