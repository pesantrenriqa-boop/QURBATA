...
P004 keeps seven practice rows (28 on-page lexical objects), while row 8 is a
native micro-enrichment row inside .j2-grid. Arabic spelled-out letter names are
intentionally excluded; enrichment uses Arabic-Indic numerals and non-joining
letter recognition only. Core typography stays 52pt presentation / 39pt practice.
"""
...
.cumulative-grid-row{
  ...
  gap:2mm!important;direction:ltr!important;
  border-top:.28mm solid #111!important;
  padding:.55mm 1mm 0!important;box-sizing:border-box!important;
...
.cumulative-grid-row .micro-ar{font-family:'KFGQPC Uthman Taha Naskh','Amiri Quran','Amiri',serif!important;font-size:16pt!important;line-height:1!important;direction:rtl!important;white-space:nowrap!important;}
.cumulative-grid-row .micro-num{font-family:'KFGQPC Uthman Taha Naskh','Amiri Quran','Amiri',serif!important;font-size:18pt!important;line-height:1!important;direction:rtl!important;white-space:nowrap!important;}
'''
...
async def render_p004_v5(h:Path,out:Path,debug:bool):
    ...
    async with async_playwright() as pw:
        ...
        if await page.locator('.cumulative-grid-row .micro').count()!=2:
        ...
        extra=await page.evaluate('''()=>{
          ...
          if(er.left<gr.left-1||er.right>gr.right+1||er.top<gr.top-1||er.bottom>gr.bottom+1)out.push({kind:'ENRICHMENT_OUTSIDE_GRID'});
          if(objs.length){const lastBottom=Math.max(...objs.map(x=>x.getBoundingClientRect().bottom));if(lastBottom>er.top-4)out.push({kind:'ROW7_ROW8_CLEARANCE_TOO_SMALL',clearance:er.top-lastBottom,required:4});}
          const f=document.querySelector('.footer');if(f){const fr=f.getBoundingClientRect();if(er.bottom>fr.top-2)out.push({kind:'ENRICHMENT_FOOTER_COLLISION'});}
          return out;
        }''')
...
def main():
    ...
    print('ENRICHMENT_CATEGORY=E02|E06')
    print('ENRICHMENT_ITEM=ARABIC_INDIC_NUMERALS_0_9|NON_JOINERS')
