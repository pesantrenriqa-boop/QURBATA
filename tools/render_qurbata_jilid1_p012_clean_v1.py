#!/usr/bin/env python3
# QURBATA Jilid 1 P012 — كَ لَ. Same frozen pedagogical/layout baseline as P011.
...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P012';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'كَ','لَ'};REVIEW={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ'}
EXERCISES=['كَ ءَ','لَ أَ','كَ بَ','لَ تَ','ثَ كَ','جَ لَ','حَ كَ','خَ لَ','كَ دَ كَ','لَ ذَ لَ','كَ رَ كَ','لَ زَ لَ','كَ سَ كَ','لَ شَ لَ','كَ صَ كَ','لَ ضَ لَ','طَ كَ ظَ','عَ لَ غَ','فَ كَ قَ','ءَ لَ أَ','بَ كَ تَ','ثَ لَ جَ','حَ كَ خَ','رَ لَ زَ']
...
def rows():
 ...
 return ''.join(o)
def audit():
 if len(EXERCISES)!=24:raise RuntimeError('P012_SOURCE_COUNT_FAIL')
 for e in EXERCISES[:8]:
  t=e.split()
  if len(t)!=2 or len(set(t)&FOCUS)!=1 or len(set(t)&REVIEW)!=1:raise RuntimeError(f'P012_FIRST_TWO_ROWS_PATTERN_FAIL={e}')
 rendered=EXERCISES[:-1];ts=[x for e in rendered for x in e.split()]
 if any(x not in FOCUS|REVIEW for x in ts):raise RuntimeError('P012_WHITELIST_FAIL')
 if any(not(set(e.split())&FOCUS) for e in rendered):raise RuntimeError('P012_EXAMPLE_WITHOUT_FOCUS')
 f=sum(x in FOCUS for x in ts);r=len(ts)-f
 if (len(ts),f,r)!=(61,31,30):raise RuntimeError(f'P012_BALANCE_FAIL tokens={len(ts)} focus={f} review={r}')
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">12</div></header><section class="presentation"><span>كَ</span><span>لَ</span></section><section class="grid">{rows()}</section><div class="bottom-safe-spacer"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free(b):
 ...
 raise RuntimeError('P012_NO_FREE_OUTPUT')
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P012-KAF-LAM-CANDIDATE-V1-CUMULATIVE.pdf');png=free(OUT/'QURBATA-JILID-1-P012-KAF-LAM-CANDIDATE-V1-CUMULATIVE.png');expected=EXERCISES[:-1]
 async with async_playwright() as pw:
  ...
  if visible!=expected:raise RuntimeError('P012_RENDERED_CONTENT_MISMATCH')
  for e in visible[:8]:
   t=e.split()
   if len(t)!=2 or len(set(t)&FOCUS)!=1 or len(set(t)&REVIEW)!=1:raise RuntimeError(f'P012_VISUAL_FIRST_TWO_ROWS_PATTERN_FAIL={e}')
  ...
  if not g['ok'] or g['count']!=23:raise RuntimeError(f'P012_SAFE_AREA_FAIL {g}')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P012-KAF-LAM-CANDIDATE-V1-CUMULATIVE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P012_KAF_LAM_CUMULATIVE=PASS');print('MATERIAL_NEW=كَ|لَ');print('ROWS_1_2=NEW_PLUS_PREVIOUS_VERIFIED');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('PRACTICE_FONT_PT=40');print('GROUP_SPACING=PRESERVED');print('BOTTOM_GLYPH_OVERFLOW=0');print('PDF='+str(pdf.relative_to(ROOT)))
