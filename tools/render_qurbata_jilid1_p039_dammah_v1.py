...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P039';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'دُ','ذُ','رُ','زُ'}
EX=['دُ بَ','ذُ بِ','رُ تَ','زُ تِ','دُ ثَ','ذُ ثِ','رُ جَ','زُ جِ',
    'دُ حَ ذُ','ذُ خِ رُ','رُ دَ زُ','زُ ذِ دُ','دُ رَ ذُ','ذُ زِ رُ','رُ سَ زُ','زُ شِ دُ',
    'دُ صَ ضِ','ذُ طَ ظِ','رُ عَ غِ','زُ فَ قِ','دُ كَ لِ','ذُ مَ نِ','رُ وَ يِ']
HARAKAT=['fathah','kasrah','dammah','fathah','kasrah','dammah'];NAME_REVIEW=['ذ','ر','ز','س']
...
def audit():
 ...
 if len(ts)!=61:raise RuntimeError(f'P039_TOKEN_COUNT_FAIL {len(ts)}')
 if (focus,review)!=(31,30):raise RuntimeError(f'P039_BALANCE_FAIL focus={focus} review={review}')
 if any(x in {'ه','هَ','هِ','هُ'} for x in ts+NAME_REVIEW):raise RuntimeError('P039_ONE_HOLE_HA_FORBIDDEN')
 if any(HARAKAT.count(k)!=2 for k in ('fathah','kasrah','dammah')):raise RuntimeError('P039_HARAKAT_REVIEW_NOT_BALANCED')
def rows():
 o=[]
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">39</div></header><section class="presentation"><span>دُ</span><span>ذُ</span><span>رُ</span><span>زُ</span></section><section class="grid">{rows()}</section><div class="name-gap"></div>{bottom_strip()}<div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P039-DAMMAH-03-CANDIDATE-V1-COMPACT-HARAKAT.pdf'
 async with async_playwright() as pw:
  ...
  if g['lastRowBottom']>g['gridBottom']+1:raise RuntimeError('P039_GRID_OVERFLOW '+str(g))
  if g['lastGlyphToStrip']<5:raise RuntimeError('P039_HARAKAT_STRIP_COLLISION '+str(g))
  if g['stripBottom']>g['footerTop'] or g['footerBottom']>g['pageBottom']+1:raise RuntimeError('P039_BOTTOM_FLOW_FAIL '+str(g))
  if any('◌' in x['text'] for x in g['items']):raise RuntimeError('P039_DOTTED_CIRCLE_FORBIDDEN')
  for x in g['items']:
   if x['k']=='kasrah' and x['anchorY']<=x['lineBottom']+2:raise RuntimeError('P039_KASRAH_ANCHOR_NOT_BELOW_LINE '+str(x))
   if x['k'] in ('fathah','dammah') and x['anchorY']>=x['lineTop']-2:raise RuntimeError('P039_UPPER_HARAKAT_ANCHOR_NOT_ABOVE_LINE '+str(x))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P039-DAMMAH-03-CANDIDATE-V1-COMPACT-HARAKAT.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P039_DAMMAH=PASS');print('DAMMAH_BLOCK_PAGE=3_OF_9');print('MATERIAL_NEW=دُ|ذُ|رُ|زُ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('PRACTICE_FONT_PT=39');print('HARAKAT_FONT_PT=32');print('FATHAH_POSITION=ABOVE_LINE');print('KASRAH_POSITION=BELOW_LINE');print('DAMMAH_POSITION=ABOVE_LINE');print('NAME_REVIEW=ذ|ر|ز|س');print('BOTTOM_FLOW=SAFE');print('PDF='+str(pdf.relative_to(ROOT)))
