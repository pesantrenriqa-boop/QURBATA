...
MAP=ROOT/'content/qwo/registry/JILID-2-P005-COMPETENCY-MAP-V1.csv'
MICRO=ROOT/'content/qwo/registry/JILID-2-MICRO-COMPETENCY-P005-V1.csv'
LEX=ROOT/'content/qwo/registry/JILID-2-P005-LEXICAL-FOUNDATION-V1.csv'
...
if len(stairs)!=10: raise ValueError('P005_MICRO_STAIRS_INVALID')
if len(lex)!=32: raise ValueError('P005_LEXICAL_COUNT_INVALID')
if any(r['competency_status']!='ALLOWED' for r in lex): raise ValueError('P005_LEXICAL_COMPETENCY_NOT_ALLOWED')
...
for r in lex:
    ...
    if (r.get('lexical_status') or '').strip() not in {'CURATED','VERIFIED','MEANING_VERIFIED'}: issues.append((r['slot'],r['word'],'LEXICAL_STATUS_NOT_ACCEPTED'))
...
async def render_p005(h:Path,out:Path,debug:bool):
    ...
    async with async_playwright() as pw:
        ...
        if count!=32: raise RuntimeError(f'P005_OBJECT_COUNT_INVALID actual={count} expected=32')
        ...
        pdf=out/'QURBATA-JILID-2-P005-V1-KFGQPC-LEXICAL.pdf'
