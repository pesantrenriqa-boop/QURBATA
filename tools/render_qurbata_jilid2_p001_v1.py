...
def arabic_html(s):
    """Keep native Arabic joining but expose vowel marks for micro vertical tuning."""
    out=[]
    for ch in str(s or ''):
...
def build_page_html(debug:bool)->str:
    ...
    for ri,row in enumerate(P001_ROWS,1):
        ...
        for obj in row:
            cells.append(f'<div class="j2-object {cls}" data-slot="{slot}" data-row="{ri}"><span class="j2-glyph">{arabic_html(obj)}</span></div>');slot+=1
    presentation=f'''<div class="presentation-object"><span class="arabic-part">{arabic_html('بَتِثُ')}</span><span class="arrow">←</span><span class="arabic-part">{arabic_html('ثُ')}</span><span class="arrow">←</span><span class="arabic-part">{arabic_html('تِ')}</span><span class="arrow">←</span><span class="arabic-part">{arabic_html('بَ')}</span></div>'''
