...
DEFAULT_OUT=ROOT/'dist/jilid-2-p001-candidate-v6'
...
.brand-block{display:flex;align-items:flex-start;justify-content:flex-start;min-width:0}.brand-logo{width:23mm;height:16.5mm;object-fit:contain;display:block}
.heading{min-width:0;height:100%;display:flex;align-items:center;justify-content:center;text-align:center;padding:0}.learning-header-title{color:#064d37;white-space:nowrap;font-family:"Segoe UI Semibold","Trebuchet MS",sans-serif;font-size:8.4pt;font-weight:700;line-height:1;letter-spacing:.11em;font-variant:small-caps}
...
def build_page_html(debug:bool)->str:
    ...
    presentation='''<div class="presentation-object"><span class="arabic-part">بَتِثُ</span><span class="arrow">←</span><span class="arabic-part">ثُ</span><span class="arrow">←</span><span class="arabic-part">تِ</span><span class="arrow">←</span><span class="arabic-part">بَ</span></div>'''
    return f'''<!doctype html><html{debug_attr}><head><meta charset="utf-8"><style>{P001_CSS}</style></head><body><main class="page with-presentation"><header class="header"><div class="brand-block"><img class="brand-logo" src="{LOGO.resolve().as_uri()}" alt="Logo QURBATA"></div><div class="heading"><div class="learning-header-title">QURBATA • JILID 2</div></div><div class="page-number">01</div></header><section class="presentation"><div class="presentation-object-wrap">{presentation}</div></section><section class="j2-grid">{''.join(cells)}</section><section class="targets"><div class="target-item"><span>Kompetensi</span><strong>Membaca huruf hijaiyah bersambung</strong></div><div class="target-item"><span>Subkompetensi</span><strong>Konsep dasar sambungan</strong></div><div class="target-item"><span>Tangga</span><strong>2 huruf → 3 huruf → pemutus sambungan</strong></div><div class="target-item"><span>NIDOM</span><strong>Registry Jilid 2 belum diikat</strong></div></section><footer class="footer"><div class="field"><strong>Nama Guru</strong><span class="line"></span></div><div class="field"><strong>Tanggal</strong><span class="line"></span></div><div class="field"><strong>Nilai</strong><span class="line"></span></div></footer><div class="bottom-band"></div></main></body></html>'''

async def fit_and_inspect(page):
    metrics=await base.base.fit_joined(page)
    # The foundation fitter intentionally lets Arabic ink breathe beyond the text box.
    # Validate against each logical row cell, not the raw fitted glyph box, to avoid false positives.
...
async def render(html_path:Path,out:Path,debug:bool):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P001-V5.json';png_dir=out/'png';png_dir.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as p:
        ...
        await page.screenshot(path=str(png_dir/'page-001.png'),full_page=True);pdf=out/'QURBATA-JILID-2-P001-CANDIDATE-V5.pdf';await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await browser.close()
