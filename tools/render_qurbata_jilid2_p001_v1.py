...
DEFAULT_OUT=ROOT/'dist/jilid-2-p001-candidate-v16'
...
/* V11: preserve readable mark positions but create visibly larger inter-row clearance. */
.q-mark{position:relative;display:inline;line-height:0;font:inherit}.q-fatha{top:.34em}.q-damma{top:.39em}.q-kasra{top:-.21em}
...
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P001-V11.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as p:
  ...
  await page.screenshot(path=str(png/'page-001.png'),full_page=True);pdf=out/'QURBATA-JILID-2-P001-CANDIDATE-V11.pdf';await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await browser.close()
...
def main():
 ...
 print('JILID2_P001_RENDERER_V11=PASS');print('PAGE=1');print('HARAKAT_FATHA_OFFSET_EM=0.34');print('HARAKAT_DAMMA_OFFSET_EM=0.39');print('HARAKAT_KASRA_OFFSET_EM=-0.21');print('ROW_GAP_MM=4.6');print('GLYPH_LINE_HEIGHT=1.04');print('VERTICAL_VALIDATION=MINIMUM_8PX_INTER_ROW_CLEARANCE');print('PRACTICE_OBJECTS=26');print('COMPETENCY_LEAKAGE=0');print('LAYOUT_OVERFLOW=0');print(f'OVERFLOW_REPORT={report.relative_to(ROOT)}');print(f'PDF={pdf.relative_to(ROOT)}');return 0
