...
function Cells([string]$l){$s=$l.Trim();if($s.StartsWith('|')){$s=$s.Substring(1)};if($s.EndsWith('|')){$s=$s.Substring(0,$s.Length-1)};,@($s -split '\|'|%{$_.Trim()})}
function H([string]$s){(($s.ToLowerInvariant()-replace '[`*_]',''-replace '\s+',' ').Trim())}
...
function ParseRows([string]$t){
  ...
  for($i=0;$i-lt$ls.Count-1;$i++){
    ...
    $hdr=@(Cells $ls[$i]);$norm=@($hdr|%{H $_});$mat=-1;$no=-1;$id=-1;$typ=-1
    ...
    for($r=$i+2;$r-lt$ls.Count;$r++){
      ...
      if($raw-match '^(\d{1,2})\s*[–-]\s*(\d{1,2})$'){
        $a=[int]$Matches[1];$b=[int]$Matches[2];$parts=@($reading -split '\s*[·•]\s*'|?{$_});if($parts.Count-eq($b-$a+1)){for($k=0;$k-lt$parts.Count;$k++){$rows+=[pscustomobject]@{No=$a+$k;Text=$parts[$k].Trim();ExerciseID='';Type=$kind}}};continue
      }
    ...
    $u=@($rows|Group-Object No|%{$_.Group[0]}|Sort-Object No);if($u.Count){$tables+=,[pscustomobject]@{Rows=$u;Count=$u.Count}}
  }
...
function Cap([string]$s){$tokens=@($s-split '\s+'|?{$_}).Count;if($tokens-eq2){4}elseif($tokens-eq3){3}else{3}}
...
Write-Host 'QURBATA InDesign V3 export complete';Write-Host "Pages found          : $($records.Count)";Write-Host "24-reading pages     : $(($audit|?{$_.ReadingCount-eq24}).Count)";Write-Host "Source conflicts     : $(($audit|?{$_.SourceConflict}).Count)";Write-Host "Ready content merge  : $(($audit|?{$_.ReadyForContentMerge}).Count)";Write-Host "Output               : $OutputDir"
