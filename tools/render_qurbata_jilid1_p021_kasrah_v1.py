...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P021-KASRAH-01-CANDIDATE-V1.pdf'
...
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P021-KASRAH-01-CANDIDATE-V1.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P021_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=ءِ|إِ');print('NAME_ROW=BOTTOM_ONLY');print('PDF='+str(pdf.relative_to(ROOT)))
