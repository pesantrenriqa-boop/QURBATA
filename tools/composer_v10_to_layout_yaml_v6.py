...
def main():
 ...
 reading=read(inp/'JILID-1-READING-OBJECTS-V10.csv');meta=read(inp/'JILID-1-PAGE-METADATA-V10.csv');inj=read(inp/'JILID-1-INJECTION-CONTENT-V10.csv');nidom_rows=read(NIDOM_REGISTRY);rb=defaultdict(list);ib=defaultdict(list)
 for r in reading:rb[int(r['Page'])].append(r)
 for r in inj:ib[int(r['Page'])].append(r)
 nidom={int(r['Page']):r for r in nidom_rows}
 ...
 for page in range(1,41):
  ...
  nidom_row=nidom[page]
  nidom_sub=nidom_row['NidomSubcompetency']
  d={'schema_version':8,'book':'QURBATA','volume':1,'page':page,'status':'v10-unit-review-candidate','layout':'canonical-j1-composition-v5','page_role':'LETTER_NAMES' if page in SPECIAL else 'READING','identity':{'title':'QURBATA','subtitle':f'JILID 1 • HALAMAN {page:02d}'},'targets':{'competency_unit':competency_unit or m.get('CompetencyDescriptions',''),'memorization_item':memorization_item,'arabic_subcompetency':arabic_sub,'nidom_code':nidom_row['NidomCode'],'nidom_subcompetency':nidom_sub,'material_progress':f'{page:02d} / 40','competency_codes':m.get('CompetencyCodes',''),'competency_descriptions':m.get('CompetencyDescriptions',''),'memorization_code':m.get('MemorizationCode',''),'memorization':m.get('MemorizationDescription',''),'memorization_stage':m.get('MemorizationStage',''),'arabic_code':m.get('ArabicCode',''),'arabic_language':m.get('ArabicDescription',''),'harakat_stage':m.get('HarakatStage',''),'new_letters':m.get('NewLetters',''),'active_letters':m.get('ActiveLetters','')},'new_material':{'label':m.get('NewMaterialLabel',''),'bases':m.get('NewMaterialBases',''),'tokens':focus_tokens},'special_injection':m.get('SpecialInjection','NONE'),'footer':{'profile':m.get('FooterProfile','J1_STANDARD_V2'),'teacher_label':'Nama Guru','date_label':'Tanggal','score_label':'Nilai'}}
  if page in SPECIAL:
 ...
 print('PAGES_WRITTEN=40');print(f'READING_OBJECTS_WRITTEN={total}');print(f'LETTER_NAMES_WRITTEN={names}');print('REVIEW_ATOM=LETTER_PLUS_HARAKAT');print('BOX_WIDTH_POLICY=FIXED_EQUAL_COLUMNS');print('UNIT_ALIGNMENT=EDGE_LOCKED_RTL');print('TARGET_POSITION=BOTTOM_COMPACT');print('HEH_DISPLAY=ISOLATED_CANONICAL');print('NIDOM_REGISTRY=JILID-1-NIDOM-REGISTRY-V1');print('NIDOM_RULE=N+1|N+2|N+4|N+8');print('LAYOUT_ADAPTER_V6=PASS');return 0
