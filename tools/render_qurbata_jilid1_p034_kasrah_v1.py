...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P034';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha';HA='ﮪ';HA_F='ﮪَ';HA_K='ﮪِ'
FOCUS={HA_K,'وِ','يِ'}
OLD_KASRAH={'ءِ','إِ','بِ','تِ','ثِ','جِ','حِ','خِ','دِ','ذِ','رِ','زِ','سِ','شِ','صِ','ضِ','طِ','ظِ','عِ','غِ','فِ','قِ','كِ','لِ','مِ','نِ'}
FATHAH_POOL={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ','لَ','مَ','نَ',HA_F,'وَ','يَ'}
# Final kasrah page: all three remaining forms are introduced together.
# 61 tokens = 31 focus + 30 review = 15 old kasrah + 15 fathah.
EX=[f'{HA_K} لَ','وِ مَ','يِ نَ',f'{HA_K} '+HA_F,'وِ وَ','يِ يَ',f'{HA_K} ءَ','وِ أَ',
    f'يِ مِ {HA_K}','وِ نِ يِ',f'{HA_K} كِ وِ','يِ لِ '+HA_K,'وِ فِ يِ',f'{HA_K} قِ وِ','يِ عِ '+HA_K,'وِ غِ يِ',
    f'{HA_K} طِ بَ','وِ ظِ تَ','يِ صِ ثَ',f'{HA_K} ضِ جَ','وِ سِ حَ','يِ شِ خَ',f'{HA_K} رِ دَ']
NAMES=['ا','ب','ج','ح','خ','د','ر','س','ع','غ',HA,'و']
...
def audit():
 ...
 if (len(ts),f,old+fa)!=(61,31,30):raise RuntimeError(f'P034_BALANCE_FAIL tokens={len(ts)} focus={f} review={old+fa}')
 if (old,fa)!=(15,15):raise RuntimeError(f'P034_REVIEW_SPLIT_FAIL kasrah={old} fathah={fa}')
 if not all(any(x==z for x in ts) for z in FOCUS):raise RuntimeError('P034_REMAINING_KASRAH_INCOMPLETE')
 if any(x in {'ه','هَ','هِ'} for x in ts+NAMES):raise RuntimeError('P034_ONE_HOLE_HA_FORBIDDEN')
 if HA not in NAMES or HA_K not in FOCUS:raise RuntimeError('P034_TWO_HOLE_HA_REQUIRED')
def rows():
 o=[]
...
async def render(h):
 ...
 async with async_playwright() as pw:
  ...
  if names!=NAMES:raise RuntimeError('P034_NAME_ROW_MISMATCH')
  ...
  if g['left']<8 or g['right']<8 or g['top']<0 or g['bottom']<0:raise RuntimeError('P034_NAME_SAFEAREA_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P034-KASRAH-FINAL-HA-WAW-YA-CANDIDATE-V2-BALANCED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P034_KASRAH_FINAL=PASS');print('MAIN_COMPETENCY=KASRAH_FINAL');print('MATERIAL_NEW=ﮪِ|وِ|يِ');print('KASRAH_ALPHABET_COMPLETION=YES');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('REVIEW_SPLIT=15_OLD_KASRAH|15_FATHAH');print('HA_GLYPH=TWO_HOLE_ONLY');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
