...
EX=[f'{HA_K} لَ','وِ مَ',f'{HA_K} نَ','وِ '+HA_F,f'{HA_K} وَ','وِ يَ',f'{HA_K} ءَ','وِ أَ',
    f'{HA_K} مِ {HA_K}','وِ نِ وِ',f'{HA_K} كِ {HA_K}','وِ لِ وِ',f'{HA_K} فِ {HA_K}','وِ قِ وِ',f'{HA_K} عِ {HA_K}','وِ غِ وِ',
    f'{HA_K} طِ بَ','وِ ظِ تَ',f'{HA_K} صِ ثَ','وِ ضِ جَ',f'{HA_K} سِ حَ','وِ شِ خَ',f'{HA_K} رِ دَ']
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">34</div></header><section class="presentation"><span>{HA_K}</span><span>وِ</span></section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P034-KASRAH-14-CANDIDATE-V1-BALANCED.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P034-KASRAH-14-CANDIDATE-V1-BALANCED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P034_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=ﮪِ|وِ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('REVIEW_SPLIT=15_OLD_KASRAH|15_FATHAH');print('HA_GLYPH=TWO_HOLE_ONLY');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
