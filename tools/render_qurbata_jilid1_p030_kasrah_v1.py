...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P030';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha';HA='ﮪ'
FOCUS={'عِ','غِ'}
EX=['عِ طِ','غِ ظِ','عِ صِ','غِ ضِ','عِ سِ','غِ شِ','عِ رِ','غِ زِ','عِ دِ عِ','غِ ذِ غِ','عِ حِ طِ','غِ خِ ظِ','عِ ثِ صِ','غِ جِ ضِ','عِ بِ سِ','غِ تِ شِ','عِ ءِ رِ','غِ إِ زِ','عِ عَ دِ','غِ غَ ذِ','عِ فَ بِ','غِ قَ تِ','عِ كَ ءِ']
NAMES=['ب','ج','ح','خ','د','ر','س','ص','ع','غ','ن',HA]
def run(s):return '<span class="run">'+''.join(f'<span>{x}</span>' for x in s.split())+'</span>'
def audit():
 ts=[x for e in EX for x in e.split()]
 if sum(x in FOCUS for x in ts)<23:raise RuntimeError('P030_FOCUS_TOO_LOW')
 if len(NAMES)!=12 or 'ن' not in NAMES or HA not in NAMES:raise RuntimeError('P030_NAME_POLICY_FAIL')
 if 'ه' in NAMES:raise RuntimeError('P030_ONE_HOLE_HA_FORBIDDEN')
def rows():
 o=[]
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">30</div></header><section class="presentation"><span>عِ</span><span>غِ</span></section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P030-KASRAH-10-CANDIDATE-V2-HA2.pdf'
 async with async_playwright() as pw:
  ...
  if names!=NAMES:raise RuntimeError('P030_NAME_ROW_MISMATCH')
  ...
  if g['left']<8 or g['right']<8 or g['top']<0 or g['bottom']<0:raise RuntimeError('P030_NAME_SAFEAREA_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P030-KASRAH-10-CANDIDATE-V2-HA2.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P030_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=عِ|غِ');print('NAME_ROW_NEW=ن|'+HA);print('HA_GLYPH=TWO_HOLE_ONLY');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
