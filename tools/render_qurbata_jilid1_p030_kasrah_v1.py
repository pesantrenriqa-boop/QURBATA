...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P030';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'عِ','غِ'}
...
def audit():
 ...
 if len(NAMES)!=12 or not {'ن','ه'}.issubset(NAMES):raise RuntimeError('P030_NAME_POLICY_FAIL')
def rows():
 o=[]
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P030-KASRAH-10-CANDIDATE-V1-SAFE.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P030-KASRAH-10-CANDIDATE-V1-SAFE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P030_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=عِ|غِ');print('NAME_ROW=BOTTOM_ONLY_ROTATING_CUMULATIVE');print('NAME_ROW_NEW=ن|ه');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
