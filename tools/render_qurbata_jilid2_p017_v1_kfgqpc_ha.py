...
p001.P001_CSS += r'''.presentation-object{font-size:46pt}.j2-glyph{font-size:42pt}.ha-initial{display:inline-block;direction:rtl;unicode-bidi:isolate;font-family:"KFGQPC Uthman Taha Naskh",serif;font-feature-settings:"mark" 1,"mkmk" 1}'''
...
def build(debug):
    ...
    # U+FEEB ARABIC LETTER HEH INITIAL FORM: pedagogical standalone display of the
    # form used when ه begins a connected Arabic word. It is intentionally detached
    # from neighbouring spans while preserving the KFGQPC initial-form glyph.
    initial_ha='ﻫَ'
    pres=f'''<section class="presentation"><div class="presentation-object-wrap"><div class="presentation-object"><span class="arabic-part" lang="ar">{p001.arabic_html('هَجَرَ')}</span><span class="arrow">←</span><span class="arabic-part" lang="ar">{p001.arabic_html('جَ')}</span><span class="arrow">←</span><span class="arabic-part ha-initial" lang="ar">{initial_ha}</span></div></div></section>'''
    h=h[:s]+pres+h[e:]
...
async def render(h,out,debug):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P017-V4.json'; png=out/'png'; png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        glyph_count=await page.locator('.ha-initial').count()
        if glyph_count!=1: raise RuntimeError(f'P017_INITIAL_HA_GATE_FAIL actual={glyph_count} expected=1')
        glyph_text=await page.locator('.ha-initial').inner_text()
        if glyph_text!='ﻫَ': raise RuntimeError('P017_INITIAL_HA_TEXT_FAIL actual='+repr(glyph_text))
        ...
        pdf=out/'QURBATA-JILID-2-P017-V4-KFGQPC-HA-INITIAL-CUMULATIVE.pdf'
...
def main():
    ...
    rc=v22.main()
    print('JILID2_P017_RENDERER_V4=PASS'); print('PAGE=17'); print('PAGE_IDENTITY_GATE=17'); print('PRESENTATION_HA_FORM=INITIAL'); print('PRESENTATION_HA_CODEPOINT=U+FEEB'); print('PRESENTATION_HA_PEDAGOGICAL_ROLE=WORD_INITIAL_CONNECTED_FORM'); print('PRESENTATION_HA_SOURCE=KFGQPC_GLYPH'); print('MANUAL_GLYPH_DRAWING=DISABLED'); print(f"COMPETENCY={meta['CompetencyCode']}|{meta['Competency']}"); print(f"UNIT_COMPETENCY={meta['UnitCompetencyCode']}|{meta['UnitCompetency']}"); print(f"UNIT_MUROJAAH={meta['UnitMurojaahCode']}|{meta['UnitMurojaah']}"); print(f"STAIR_RANGE={stairs[0]['StairCode']}-{stairs[-1]['StairCode']}"); print('ACQUISITION_LETTERS=ه'); print('CUMULATIVE_HARAKAT=FATHA|KASRA|DAMMA'); print('HARAKAT_FATHA_COUNT='+str(counts['FATHA'])); print('HARAKAT_KASRA_COUNT='+str(counts['KASRA'])); print('HARAKAT_DAMMA_COUNT='+str(counts['DAMMA'])); print('HARAKAT_BALANCE_GATE=KASRA>=10|DAMMA>=10'); print('CUMULATIVE_COMPETENCY_P001_P016=PRESERVED'); print('PRACTICE_OBJECTS=32'); print('THREE_LETTER_WITH_MEANING=32'); print('COMPETENCY_LEAKAGE=0'); print('STATUS=P017_INITIAL_HA_VISUAL_CANDIDATE_NOT_FROZEN'); return rc
