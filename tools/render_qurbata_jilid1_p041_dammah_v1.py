...
FOCUS={'طُ','ظُ','عُ','غُ'}
EX=['طُ بَ','ظُ بِ','عُ تَ','غُ تِ','طُ ثَ','ظُ ثِ','عُ جَ','غُ جِ','طُ حَ ظُ','ظُ خِ عُ','عُ دَ غُ','غُ ذِ طُ','طُ رَ ظُ','ظُ زِ عُ','عُ سَ غُ','غُ شِ طُ','طُ صَ ضِ','ظُ طَ ظِ','عُ فَ قِ','غُ كَ لِ','طُ مَ نِ','ظُ وَ يِ','عُ ءَ بِ']
ARABIC_NUMERALS=['١','٢','٣','٤','٥','٦','٧','٨','٩','١٠'];NAME_REVIEW=['ط','ظ','ع','غ']
...
def audit():
 ...
 if ARABIC_NUMERALS!=['١','٢','٣','٤','٥','٦','٧','٨','٩','١٠']:raise RuntimeError('P041_ARABIC_NUMERAL_SEQUENCE_FAIL')
def rows():
 o=[]
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P041-DAMMAH-05-CANDIDATE-V3-ARABIC-NUMERALS.pdf'
 async with async_playwright() as pw:
  ...
  if g['nums']!=ARABIC_NUMERALS:raise RuntimeError('P041_ARABIC_NUMERAL_RENDER_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P041-DAMMAH-05-CANDIDATE-V3-ARABIC-NUMERALS.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P041_DAMMAH=PASS');print('MATERIAL_NEW=طُ|ظُ|عُ|غُ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('BOTTOM_STRIP_PRIMARY=ARABIC_NUMERALS');print('ARABIC_NUMERALS=١|٢|٣|٤|٥|٦|٧|٨|٩|١٠');print('NUMBER_FONT_PT=25');print('NAME_REVIEW=ط|ظ|ع|غ');print('BOTTOM_FLOW=SAFE');print('PDF='+str(pdf.relative_to(ROOT)))
