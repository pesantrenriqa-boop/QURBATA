...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P041';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'طُ','ظُ','عُ','غُ'}
# 61 tokens total. First 8 pairs = 8 focus + 8 review.
# Next 8 triples contain 2 focus + 1 review; final 7 triples contain 1 focus + 2 review.
# Total = 31 focus + 30 review.
EX=['طُ بَ','ظُ بِ','عُ تَ','غُ تِ','طُ ثَ','ظُ ثِ','عُ جَ','غُ جِ',
    'طُ حَ ظُ','ظُ خِ عُ','عُ دَ غُ','غُ ذِ طُ','طُ رَ ظُ','ظُ زِ عُ','عُ سَ غُ','غُ شِ طُ',
    'طُ صَ ضِ','ظُ طَ ظِ','عُ فَ قِ','غُ كَ لِ','طُ مَ نِ','ظُ وَ يِ','عُ ءَ بِ']
HARAKAT=['fathah','kasrah','dammah','fathah','kasrah','dammah'];NAME_REVIEW=['ط','ظ','ع','غ']
...
def audit():
 ...
 if len(ts)!=61:raise RuntimeError(f'P041_TOKEN_COUNT_FAIL {len(ts)}')
 if (focus,review)!=(31,30):raise RuntimeError(f'P041_BALANCE_FAIL focus={focus} review={review}')
 if 'إُ' in ts:raise RuntimeError('P041_HAMZAH_BELOW_ALIF_FORBIDDEN')
 if any(x in {'ه','هَ','هِ','هُ'} for x in ts+NAME_REVIEW):raise RuntimeError('P041_ONE_HOLE_HA_FORBIDDEN')
 if any(HARAKAT.count(k)!=2 for k in ('fathah','kasrah','dammah')):raise RuntimeError('P041_HARAKAT_REVIEW_NOT_BALANCED')
def rows():
 o=[]
...
async def render(h):
 ...
 async with async_playwright() as pw:
  ...
  if g['lastRowBottom']>g['gridBottom']+1:raise RuntimeError('P041_GRID_OVERFLOW '+str(g))
  if g['lastGlyphToStrip']<5:raise RuntimeError('P041_STRIP_COLLISION '+str(g))
  if g['stripBottom']>g['footerTop'] or g['footerBottom']>g['pageBottom']+1:raise RuntimeError('P041_BOTTOM_FLOW_FAIL '+str(g))
  if any('◌' in x['text'] for x in g['items']):raise RuntimeError('P041_DOTTED_CIRCLE_FORBIDDEN')
  for x in g['items']:
   if x['k']=='kasrah' and x['anchorY']<=x['lineBottom']+2:raise RuntimeError('P041_KASRAH_POSITION_FAIL '+str(x))
   if x['k'] in ('fathah','dammah') and x['anchorY']>=x['lineTop']-2:raise RuntimeError('P041_UPPER_HARAKAT_POSITION_FAIL '+str(x))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P041-DAMMAH-05-CANDIDATE-V2-BALANCED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P041_DAMMAH=PASS');print('DAMMAH_COMPACT_PAGE=1_OF_4');print('MATERIAL_NEW=طُ|ظُ|عُ|غُ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('PRACTICE_FONT_PT=39');print('HARAKAT_FONT_PT=32');print('KASRAH_POSITION=BELOW_LINE');print('JILID1_TARGET_END=P045');print('BOTTOM_FLOW=SAFE');print('PDF='+str(pdf.relative_to(ROOT)))
