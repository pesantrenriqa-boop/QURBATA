#!/usr/bin/env python3
# QURBATA J1 P023 — Kasrah stage 3: ثِ جِ; cumulative kasrah + fathah review; letter-name strand continues.
...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P023';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'ثِ','جِ'}
EX=['ثِ بِ','جِ تِ','ثِ ءِ','جِ إِ','ثِ بَ','جِ تَ','ثِ ثَ','جِ جَ','ثِ حَ ثِ','جِ خَ جِ','ثِ دَ بِ','جِ ذَ تِ','ثِ رَ ءِ','جِ زَ إِ','ثِ سَ ثِ','جِ شَ جِ','ثِ صَ بِ','جِ ضَ تِ','ثِ طَ ءِ','جِ ظَ إِ','ثِ عَ ثِ','جِ غَ جِ','ثِ فَ بِ']
NAMES=['ا','ب','ت','ث','ج','ح','خ','د','ذ','ر','ز','ا']
def run(s):return '<span class="run">'+''.join(f'<span>{x}</span>' for x in s.split())+'</span>'
def audit():
 ts=[x for e in EX for x in e.split()]
 if sum(x in FOCUS for x in ts)<23:raise RuntimeError('P023_FOCUS_TOO_LOW')
 if NAMES[:11]!=['ا','ب','ت','ث','ج','ح','خ','د','ذ','ر','ز']:raise RuntimeError('P023_NAME_LADDER_FAIL')
def rows():
 o=[]
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">23</div></header><section class="presentation"><span>ثِ</span><span>جِ</span></section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P023-KASRAH-03-CANDIDATE-V1.pdf'
 async with async_playwright() as pw:
  ...
  if names!=NAMES:raise RuntimeError('P023_NAME_ROW_MISMATCH')
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P023-KASRAH-03-CANDIDATE-V1.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P023_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=ثِ|جِ');print('KASRAH_REVIEW=ءِ|إِ|بِ|تِ');print('NAME_ROW=BOTTOM_ONLY_STAGED_CUMULATIVE');print('NAME_ROW_NEW=ر|ز');print('NAME_ROW_REVIEW=ا|ب|ت|ث|ج|ح|خ|د|ذ');print('PDF='+str(pdf.relative_to(ROOT)))
