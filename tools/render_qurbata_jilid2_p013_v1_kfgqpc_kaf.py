#!/usr/bin/env python3
"""QURBATA Jilid 2 P013 — acquisition of ك with cumulative P001-P012 review."""
...
MAP=ROOT/'content/qwo/registry/JILID-2-P013-COMPETENCY-MAP-V1.csv'; MICRO=ROOT/'content/qwo/registry/JILID-2-MICRO-COMPETENCY-P013-V1.csv'; LEX=ROOT/'content/qwo/registry/JILID-2-P013-LEXICAL-FOUNDATION-V1.csv'
...
if len(stairs)!=10 or len(lex)!=32: raise ValueError('P013_REGISTRY_INVALID')
...
for r in lex:
    if len(bases(r['word']))!=3 or not r['meaning_id'].strip() or r['lexical_status']!='CURATED' or r['competency_status']!='ALLOWED': raise ValueError('P013_SEMANTIC_GATE_FAIL='+repr(r))
p001.MICRO=MICRO
p001.P001_BANNED_JOINING=set('ظلمنهي')
...
async def render(h,out,debug):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P013-V1.json'; png=out/'png'; png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        if count!=32: raise RuntimeError(f'P013_OBJECT_COUNT_INVALID actual={count} expected=32')
        ...
        if issues: raise RuntimeError('P013_LAYOUT_ISSUES='+str(len(issues))+' REPORT='+str(report))
        page_no=await page.locator('.page-number').inner_text()
        if page_no.strip()!='13': raise RuntimeError('P013_PAGE_IDENTITY_FAIL actual='+repr(page_no))
        await page.screenshot(path=str(png/'page-013.png'),full_page=True)
        pdf=out/'QURBATA-JILID-2-P013-V1-KFGQPC-KAF-CUMULATIVE.pdf'
...
def main():
    ...
    if leaks: raise ValueError('P013_COMPETENCY_LEAKAGE='+repr(leaks))
    current=[r for r in lex if r['function']=='CURRENT']; missing=[r['word'] for r in current if 'ك' not in bases(r['word'])]
    if missing: raise ValueError('P013_CURRENT_OBJECT_MISSING_KAF='+repr(missing))
    text=''.join(r['word'] for r in lex); counts={'FATHA':text.count('َ'),'KASRA':text.count('ِ'),'DAMMA':text.count('ُ')}
    if counts['KASRA']<10 or counts['DAMMA']<10: raise ValueError('P013_HARAKAT_BALANCE_FAIL='+repr(counts))
    rc=v22.main()
    print('JILID2_P013_RENDERER_V1=PASS'); print('PAGE=13'); print('PAGE_IDENTITY_GATE=13'); print(f"COMPETENCY={meta['CompetencyCode']}|{meta['Competency']}"); print(f"UNIT_COMPETENCY={meta['UnitCompetencyCode']}|{meta['UnitCompetency']}"); print(f"UNIT_MUROJAAH={meta['UnitMurojaahCode']}|{meta['UnitMurojaah']}"); print(f"STAIR_RANGE={stairs[0]['StairCode']}-{stairs[-1]['StairCode']}"); print('ACQUISITION_LETTERS=ك'); print('CUMULATIVE_HARAKAT=FATHA|KASRA|DAMMA'); print('HARAKAT_FATHA_COUNT='+str(counts['FATHA'])); print('HARAKAT_KASRA_COUNT='+str(counts['KASRA'])); print('HARAKAT_DAMMA_COUNT='+str(counts['DAMMA'])); print('HARAKAT_BALANCE_GATE=KASRA>=10|DAMMA>=10'); print('CUMULATIVE_COMPETENCY_P001_P012=PRESERVED'); print('PRACTICE_OBJECTS=32'); print('CURRENT_LEXICAL_OBJECTS='+str(len(current))); print('MUROJAAH_LEXICAL_OBJECTS='+str(32-len(current))); print('THREE_LETTER_WITH_MEANING=32'); print('MEANINGLESS_THREE_LETTER_OBJECTS=0'); print('COMPETENCY_LEAKAGE=0'); print('ARABIC_FONT_PRIMARY=KFGQPC Uthman Taha Naskh'); print('STATUS=P013_CUMULATIVE_CANDIDATE_NOT_FROZEN'); return rc
