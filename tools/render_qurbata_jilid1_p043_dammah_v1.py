...
EX=['مُ بَ','نُ بِ','وُ تَ','يُ تِ','مُ ثَ','نُ ثِ','وُ جَ','يُ جِ',
    ...
    'مُ صَ ضِ','نُ طَ ظِ','وُ عَ غِ','يُ فَ قِ','مُ كَ لِ','نُ ءَ أِ','وُ بَ تِ']
...
def audit():
 ...
 if 'إُ' in ts:raise RuntimeError('P043_HAMZAH_BELOW_ALIF_FORBIDDEN')
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P043-DAMMAH-07-CANDIDATE-V1-ARABIC-NUMERALS.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P043-DAMMAH-07-CANDIDATE-V1-ARABIC-NUMERALS.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P043_DAMMAH=PASS');print('DAMMAH_COMPACT_PAGE=3_OF_4');print('MATERIAL_NEW=مُ|نُ|وُ|يُ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('BOTTOM_STRIP_PRIMARY=ARABIC_NUMERALS');print('ARABIC_NUMERALS=١|٢|٣|٤|٥|٦|٧|٨|٩|١٠');print('NUMBER_FONT_PT=25');print('NAME_REVIEW=م|ن|و|ي');print('JILID1_TARGET_END=P045');print('BOTTOM_FLOW=SAFE');print('PDF='+str(pdf.relative_to(ROOT)))
