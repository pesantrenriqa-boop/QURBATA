#!/usr/bin/env python3
"""QURBATA Jilid 2 P016 — acquisition of ن with cumulative P001-P015 review."""
...
MAP=ROOT/'content/qwo/registry/JILID-2-P016-COMPETENCY-MAP-V1.csv'; MICRO=ROOT/'content/qwo/registry/JILID-2-MICRO-COMPETENCY-P016-V1.csv'; LEX=ROOT/'content/qwo/registry/JILID-2-P016-LEXICAL-FOUNDATION-V1.csv'
...
if len(stairs)!=10 or len(lex)!=32: raise ValueError('P016_REGISTRY_INVALID')
...
for r in lex:
    if len(bases(r['word']))!=3 or not r['meaning_id'].strip() or r['lexical_status']!='CURATED' or r['competency_status']!='ALLOWED': raise ValueError('P016_SEMANTIC_GATE_FAIL='+repr(r))
p001.MICRO=MICRO
# Future letters after nun: ه ي; ظ remains not yet acquired.
p001.P001_BANNED_JOINING=set('ظهي')
...
async def render(h,out,debug):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P016-V1.json'; png=out/'png'; png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        if count!=32: raise RuntimeError(f'P016_OBJECT_COUNT_INVALID actual={count} expected=32')
        ...
        if issues: raise RuntimeError('P016_LAYOUT_ISSUES='+str(len(issues))+' REPORT='+str(report))
        page_no=(await page.locator('.page-number').inner_text()).strip()
        if page_no!='16': raise RuntimeError('P016_PAGE_IDENTITY_FAIL actual='+repr(page_no))
        await page.screenshot(path=str(png/'page-016.png'),full_page=True)
        pdf=out/'QURBATA-JILID-2-P016-V1-KFGQPC-NUN-CUMULATIVE.pdf'
...
def main():
    ...
    if leaks: raise ValueError('P016_COMPETENCY_LEAKAGE='+repr(leaks))
    current=[r for r in lex if r['function']=='CURRENT']; missing=[r['word'] for r in current if 'ن' not in bases(r['word'])]
    if missing: raise ValueError('P016_CURRENT_OBJECT_MISSING_NUN='+repr(missing))
    text=''.join(r['word'] for r in lex); counts={'FATHA':text.count('َ'),'KASRA':text.count('ِ'),'DAMMA':text.count('ُ')}
    if counts['KASRA']<10 or counts['DAMMA']<10: raise ValueError('P016_HARAKAT_BALANCE_FAIL='+repr(counts))
    rc=v22.main()
    print('JILID2_P016_RENDERER_V1=PASS'); print('PAGE=16'); print('PAGE_IDENTITY_GATE=16'); print(f"COMPETENCY={meta['CompetencyCode']}|{meta['Competency']}"); print(f"UNIT_COMPETENCY={meta['UnitCompetencyCode']}|{meta['UnitCompetency']}"); print(f"UNIT_MUROJAAH={meta['UnitMurojaahCode']}|{meta['UnitMurojaah']}"); print(f"STAIR_RANGE={stairs[0]['StairCode']}-{stairs[-1]['StairCode']}"); print('ACQUISITION_LETTERS=ن'); print('CUMULATIVE_HARAKAT=FATHA|KASRA|DAMMA'); print('HARAKAT_FATHA_COUNT='+str(counts['FATHA'])); print('HARAKAT_KASRA_COUNT='+str(counts['KASRA'])); print('HARAKAT_DAMMA_COUNT='+str(counts['DAMMA'])); print('HARAKAT_BALANCE_GATE=KASRA>=10|DAMMA>=10'); print('CUMULATIVE_COMPETENCY_P001_P015=PRESERVED'); print('PRACTICE_OBJECTS=32'); print('CURRENT_LEXICAL_OBJECTS='+str(len(current))); print('MUROJAAH_LEXICAL_OBJECTS='+str(32-len(current))); print('THREE_LETTER_WITH_MEANING=32'); print('MEANINGLESS_THREE_LETTER_OBJECTS=0'); print('COMPETENCY_LEAKAGE=0'); print('ARABIC_FONT_PRIMARY=KFGQPC Uthman Taha Naskh'); print('STATUS=P016_CUMULATIVE_CANDIDATE_NOT_FROZEN'); return rc
