#!/usr/bin/env python3
"""QURBATA Jilid 2 P007 — acquisition of ع with continued bottom-row enrichment."""
...
p001.MICRO=MICRO
# ع is active; ظ and all later joining families remain forbidden.
...
.p007-enrichment-row{grid-column:1/-1!important;grid-row:8!important;display:grid!important;grid-template-columns:1fr 1fr!important;gap:10mm!important;padding:.55mm 6mm .35mm!important;border-top:.28mm solid #111!important;box-sizing:border-box!important;background:#fff!important;}
.p007-enrichment-row .micro{display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;text-align:center!important;min-width:0!important;}
.p007-enrichment-row .micro-label{font-family:Arial,sans-serif!important;font-size:6.2pt!important;font-weight:700!important;margin:0 0 .7mm!important;white-space:nowrap!important;}
...
def build_p007(debug:bool):
    ...
    pres=f'''<section class="presentation"><div class="presentation-object-wrap"><div class="presentation-object" dir="ltr"><span class="arabic-part" lang="ar" dir="rtl">{p001.arabic_html('عَ')}</span><span class="arrow" dir="ltr">←</span><span class="arabic-part" lang="ar" dir="rtl">{p001.arabic_html('عَبَرَ')}</span></div></div></section>'''
    ...
    enrichment=f'''<div class="p007-enrichment-row" data-enrichment-row="8"><div class="micro"><div class="micro-label">ANGKA ARAB</div><div class="glyph-run num">{nums}</div></div><div class="micro"><div class="micro-label">PEMUTUS SAMBUNGAN</div><div class="glyph-run nonjoin">{njs}</div></div></div>'''
...
async def _write_pdf(page,out:Path):
    names=[out/'QURBATA-JILID-2-P007-V1-AIN-BOTTOMROW.pdf']+[out/f'QURBATA-JILID-2-P007-V1-AIN-BOTTOMROW-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
    last=None
    for idx,p in enumerate(names):
        try:
            ...
            return p,('DIRECT_P007' if idx==0 else f'LOCK_FALLBACK_P007_{idx:02d}')
...
async def render_p007(h:Path,out:Path,debug:bool):
    ...
    async with async_playwright() as pw:
        ...
        await page.screenshot(path=str(png/'page-007-v1.png'),full_page=True)
...
def main():
    ...
    current_ain=[r for r in current if 'ع' in base_letters(r['word'])]
    ...
    rc=v22.main()
    print('JILID2_P007_RENDERER_V1=PASS');print('PAGE=7');print('ACQUISITION_LETTERS=ع');print('TITLE_VISUAL_RIGHT_TO_LEFT=عَ←عَبَرَ')
    print('LEXICAL_ACQUISITION_GATE=MIN_8_CURRENT_PER_NEW_LETTER');print(f'CURRENT_AIN_OBJECTS={len(current_ain)}');print('ZA_STATUS=DEFERRED_UNTIL_CORPUS_GATE_PASS')
