#!/usr/bin/env python3
# QURBATA J1 P026 — Kasrah stage 6: رِ زِ; cumulative kasrah + fathah review; safe one-row letter-name strand.
...
# One-row letter-name review: new letters mandatory, older letters rotate.
NAMES=['ا','ب','ت','ث','ج','ح','خ','ر','ز','ص','ط','ظ']
...
def audit():
 ...
 if len(NAMES)!=12 or 'ط' not in NAMES or 'ظ' not in NAMES:raise RuntimeError('P026_NAME_ROW_POLICY_FAIL')
def rows():
 o=[]
...
def doc(u):
 ...
 css=f'''@page{{size:A5;margin:0}}*{{box-sizing:border-box}}@font-face{{font-family:"{FONT}";src:url("{u}")}}html,body{{margin:0}}.page{{width:148mm;height:210mm;padding:3.6mm 8mm 2.5mm;display:flex;flex-direction:column;overflow:hidden}}.header{{height:17mm;position:relative;display:flex;align-items:center;justify-content:center}}.logo{{position:absolute;left:0;width:32mm;height:17mm}}.title{{color:#064d37;font:700 6.2pt Georgia;letter-spacing:.16em}}.pageno{{position:absolute;right:0;top:0;width:12mm;background:#064d37;color:white;border-bottom:1mm solid #b98a2f;text-align:center;font:700 12pt Arial;padding:2.2mm 1mm 3mm;border-radius:0 0 3mm 3mm}}.presentation{{height:18mm;display:flex;align-items:center;justify-content:center;gap:18mm;font:46pt "{FONT}";direction:rtl}}.grid{{height:124mm;display:grid;grid-template-rows:repeat(7,1fr);row-gap:.8mm}}.row{{display:flex;direction:rtl;align-items:center;justify-content:center}}.r2{{gap:10mm}}.r3{{gap:11mm}}.practice{{display:flex;justify-content:center;font:39pt/1 "{FONT}";white-space:nowrap}}.l2{{width:23mm}}.l3{{width:35mm}}.run{{display:inline-flex;direction:rtl}}.l2 .run{{gap:2.8mm}}.l3 .run{{gap:2.4mm}}.name-strip{{height:21mm;flex:0 0 21mm;margin-top:1mm;border-top:.35mm solid #b9b9b9;display:flex;align-items:center;justify-content:space-between;direction:rtl;font-family:"{FONT}";font-size:39pt;line-height:1.28;padding:1.1mm 4mm 1.8mm;overflow:visible}}.name-strip>span{{display:inline-block;line-height:1.28;overflow:visible}}.safe{{height:4mm;flex:0 0 4mm}}.footer{{height:12mm;display:flex;justify-content:space-between;align-items:center;padding-bottom:2.2mm;color:#173a2d}}.ar{{font:10.3pt "{FONT}";direction:rtl}}'''
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P026-KASRAH-06-CANDIDATE-V3-DESCENDER-SAFE.pdf'
 async with async_playwright() as pw:
  ...
  geom=await p.evaluate("()=>{let n=document.querySelector('.name-strip').getBoundingClientRect(),a=[...document.querySelectorAll('.name-strip>span')].map(e=>({t:e.textContent.trim(),r:e.getBoundingClientRect()}));let left=Math.min(...a.map(x=>x.r.left))-n.left,right=n.right-Math.max(...a.map(x=>x.r.right)),top=Math.min(...a.map(x=>x.r.top))-n.top,bottom=n.bottom-Math.max(...a.map(x=>x.r.bottom));let desc=a.filter(x=>['ج','ح','خ'].includes(x.t));return {left,right,top,bottom,descBottom:desc.map(x=>n.bottom-x.r.bottom),descTop:desc.map(x=>x.r.top-n.top)}}")
  if geom['left']<8 or geom['right']<8:raise RuntimeError('P026_NAME_SAFEWIDTH_FAIL '+str(geom))
  if geom['bottom']<2 or any(x<2 for x in geom['descBottom']):raise RuntimeError('P026_DESCENDER_CLIP_FAIL '+str(geom))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,geom
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P026-KASRAH-06-CANDIDATE-V3-DESCENDER-SAFE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P026_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=رِ|زِ');print('KASRAH_REVIEW=ءِ|إِ|بِ|تِ|ثِ|جِ|حِ|خِ|دِ|ذِ');print('NAME_ROW=BOTTOM_ONLY_ROTATING_CUMULATIVE');print('NAME_ROW_COUNT=12');print('NAME_ROW_NEW=ط|ظ');print('NAME_ROW_SAFEWIDTH=PASS');print('DESCENDER_GLYPHS=ج|ح|خ');print('DESCENDER_SAFE=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
