...
# One-row letter-name review must remain inside A5 safe width. New letters are mandatory; older letters rotate.
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P026-KASRAH-06-CANDIDATE-V2-SAFE-NAMES.pdf'
 async with async_playwright() as pw:
  ...
  geom=await p.evaluate("()=>{let n=document.querySelector('.name-strip').getBoundingClientRect(),a=[...document.querySelectorAll('.name-strip>span')].map(e=>e.getBoundingClientRect());return {left:Math.min(...a.map(x=>x.left))-n.left,right:n.right-Math.max(...a.map(x=>x.right))}}")
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P026-KASRAH-06-CANDIDATE-V2-SAFE-NAMES.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P026_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=رِ|زِ');print('KASRAH_REVIEW=ءِ|إِ|بِ|تِ|ثِ|جِ|حِ|خِ|دِ|ذِ');print('NAME_ROW=BOTTOM_ONLY_ROTATING_CUMULATIVE');print('NAME_ROW_COUNT=12');print('NAME_ROW_NEW=ط|ظ');print('NAME_ROW_SAFEWIDTH=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
