...
- row 1: one full-width current-material focus strip
- row 2: 4 x L2 from the exact row-1 focus bases
- rows 3-6: 12 x L3 in a 3-column x 4-row grid
- independent Arabic units; never shape L2/L3 as connected words
- canonical ه metadata may use a dedicated two-eye display glyph.
"""
...
def display_letter(letter:str)->str:
    return 'ﻫ' if letter=='ه' else letter
def main():
    ...
    for page in range(1,41):
        ...
        if page in SPECIAL:
            ...
            data['page_kind']='LETTER_NAMES';data['objects']=[];data['letter_names']=[{'sequence':int(r['Sequence']),'canonical_letter':r['Letter'],'letter':display_letter(r['Letter']),'letter_name_arabic':r['LetterNameArabic'],'status':r.get('Status','REVIEW_CANDIDATE')} for r in rows];names+=14
        else:
    ...
    print('PAGES_WRITTEN=40');print(f'READING_OBJECTS_WRITTEN={total}');print(f'LETTER_NAMES_WRITTEN={names}');print('PAGE_PATTERN=ROW1_FOCUS|ROW2_4xL2_FROM_ROW1|ROWS3_6_3x4_L3');print('RTL_ORDER=FIRST_AUTHORED_UNIT_ON_RIGHT');print('HEH_DISPLAY=TWO_EYE_PRESENTATION');print('LAYOUT_ADAPTER_V4=PASS');return 0
