#!/usr/bin/env python3
# QURBATA Jilid 1 P021 — main competency continues; randomized unvowelled letter names only in bottom reinforcement row.
...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P021';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha';HA='ﮪَ'
LETTERS={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ','لَ','مَ','نَ',HA,'وَ','يَ'}
EX=['ءَ جَ مَ','أَ حَ نَ','بَ خَ وَ','تَ دَ يَ','ثَ ذَ فَ','جَ رَ قَ','حَ زَ كَ','خَ سَ لَ','دَ شَ مَ','ذَ صَ نَ','رَ ضَ '+HA,'زَ طَ وَ','سَ ظَ يَ','شَ عَ فَ','صَ غَ قَ','ضَ فَ كَ','طَ قَ لَ','ظَ كَ مَ','عَ لَ نَ','غَ مَ '+HA,'فَ نَ وَ','قَ '+HA+' يَ','كَ وَ ءَ','لَ يَ أَ']
...
def audit():
 if len(EX)!=24 or any(len(x.split())!=3 for x in EX):raise RuntimeError('P021_THREE_LETTER_STRUCTURE_FAIL')
 ts=[t for e in EX for t in e.split()]
 if any(t not in LETTERS for t in ts):raise RuntimeError('P021_OUTSIDE_SCOPE')
 if any(any(ch in n for ch in 'ًٌٍَُِّْ') for n in NAMES):raise RuntimeError('P021_NAME_ROW_MUST_BE_UNVOWELLED')
 if any(n not in UNVOWELLED_ALLOWED for n in NAMES):raise RuntimeError('P021_NAME_ROW_OUTSIDE_ALLOWED_SET')
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">21</div></header><section class="presentation">LATIHAN · KOMBINASI 3 HURUF</section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free(p):
...
async def render(h):
 pdf=free(OUT/'QURBATA-JILID-1-P021-KOMPETENSI-3-HURUF-NAMA-HURUF-ACAK-CANDIDATE-V2.pdf');png=free(OUT/'QURBATA-JILID-1-P021-KOMPETENSI-3-HURUF-NAMA-HURUF-ACAK-CANDIDATE-V2.png')
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P021-KOMPETENSI-3-HURUF-NAMA-HURUF-ACAK-CANDIDATE-V2.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P021_MAIN_COMPETENCY=PASS');print('MAIN_MATERIAL=THREE_LETTER_COMPETENCY');print('NAME_ROW=UNVOWELLED_RANDOMIZED_REINFORCEMENT');print('NAME_ROW_COUNT='+str(len(NAMES)));print('NAME_ROW_FONT_PT=39');print('NAME_ROW_POSITION=BOTTOM_ONLY');print('PDF='+str(pdf.relative_to(ROOT)))
