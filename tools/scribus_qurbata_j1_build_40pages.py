...
def _nowrap_arabic_drill(text):
    # Keep each drill on one line without altering Arabic source order.
    return text.replace(" ", "\u00A0")


def _apply_arabic_rtl(frame, align):
    try:
...
def add_tartil_grid(page_num, row, arabic):
    ...
    for rr in range(8):
        for cc in range(4):
            ...
            # Scribus 1.6.x is inconsistent with RIGHT paragraph alignment on
            # short RTL strings. Avoid it completely. A narrow text frame is
            # physically pinned to the right edge; text is centered inside it.
            # Thus its visual position is right-side by geometry, not bidi align.
            inner_w = CELL_W * 0.84
            right_pad = 0.8
            inner_x = x + CELL_W - right_pad - inner_w
            ...
            _apply_arabic_rtl(frame, scribus.ALIGN_CENTERED)
