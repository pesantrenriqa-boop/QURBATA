...
# Vertical rebalance: lower the presentation and use more of the lower body.
p001.P001_CSS+=r'''
.presentation{margin-top:3.2mm!important;margin-bottom:1.4mm!important;transform:translateY(1.6mm)!important}
.presentation-object-wrap{align-items:center!important}
.j2-grid{margin-top:1.2mm!important}
'''
...
INJECT_JS='''({items,label})=>{
 ...
 grid.style.height='130mm';grid.style.maxHeight='130mm';grid.style.boxSizing='border-box';grid.style.marginBottom='0';grid.style.rowGap='1.7mm';
 ...
 Object.assign(box.style,{position:'absolute',left:'11mm',right:'11mm',bottom:'16.5mm',height:'16.5mm',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'.8mm 3mm 2.1mm',borderTop:'.28mm solid #111',boxSizing:'border-box',background:'#fff',textAlign:'center',overflow:'visible',zIndex:'20'});
...
async def _write_pdf(page,out:Path):
 names=[out/'QURBATA-JILID-2-P010-V5-MIM-NUN-VERTICAL-BALANCE.pdf']+[out/f'QURBATA-JILID-2-P010-V5-MIM-NUN-VERTICAL-BALANCE-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
 last=None
 for idx,p in enumerate(names):
  try:
   await page.pdf(path=str(p),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});return p,('DIRECT_P010_V5' if idx==0 else f'LOCK_FALLBACK_P010_V5_{idx:02d}')
  except PermissionError as e:last=e
 raise RuntimeError('P010_NO_AVAILABLE_PDF_NAME') from last

async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P010-V5.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  extra=await page.evaluate('''()=>{const e=document.querySelector('.p010-enrichment-row'),r=document.querySelector('.awail-run'),g=document.querySelector('.j2-grid'),f=document.querySelector('.footer'),p=e?.parentElement,pr=document.querySelector('.presentation'),out=[];if(!e||!r||!g||!p||!pr)return[{kind:'P010_SAFEZONE_MISSING'}];const er=e.getBoundingClientRect(),rr=r.getBoundingClientRect(),gr=g.getBoundingClientRect(),pg=p.getBoundingClientRect(),prr=pr.getBoundingClientRect();if(rr.scrollWidth>rr.clientWidth+2)out.push({kind:'P010_AWAIL_ROW_OVERFLOW'});if(er.bottom>pg.bottom-12)out.push({kind:'P010_PAGE_BOTTOM_SAFEAREA_FAIL'});if(er.top<gr.bottom+4)out.push({kind:'P010_CORE_ENRICHMENT_COLLISION'});if(prr.top<pg.top+55)out.push({kind:'P010_PRESENTATION_TOO_HIGH',presentationTop:prr.top,pageTop:pg.top});if(f){const fr=f.getBoundingClientRect();if(er.bottom>fr.top-4)out.push({kind:'P010_ENRICHMENT_FOOTER_COLLISION'});}for(const it of document.querySelectorAll('.awail-item')){const ir=it.getBoundingClientRect();if(ir.bottom>er.bottom-3)out.push({kind:'P010_GLYPH_BOTTOM_CLIP_RISK',glyph:it.textContent});if(ir.top<er.top+3)out.push({kind:'P010_GLYPH_TOP_CLIP_RISK',glyph:it.textContent});}return out}''')
  ...
  await page.screenshot(path=str(png/'page-010-v5.png'),full_page=True);pdf,mode=await _write_pdf(page,out);await b.close()
...
def main():
 ...
 rc=p009.v22.main()
 print('JILID2_P010_RENDERER_V5_VERTICAL_BALANCE=PASS');print('PAGE=10');print('ACQUISITION_LETTERS=م|ن');print('PRACTICE_MODE=JOINING_FORM_DRILL_MEANINGFUL');print(f'FORM_MIM_OBJECTS={mim}');print(f'FORM_NUN_OBJECTS={nun}');print('PRESENTATION_SHIFT_DOWN_MM=4.8_EFFECTIVE');print('CORE_GRID_HEIGHT_MM=130');print('BOTTOM_ROW_BOTTOM_OFFSET_MM=16.5');print('VERTICAL_COMPOSITION=REBALANCED_TOP_AND_BOTTOM');print('BOTTOM_ROW_STEP=E04');print('BOTTOM_ROW_ITEMS=6');print('BOTTOM_ROW_GRID=6_EQUAL_COLUMNS');print('MICRO_BINDING=PATH_NOT_PARSED_LIST');return rc
