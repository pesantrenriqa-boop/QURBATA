...
# P010 is independent from P009. Bind its own curriculum and page data.
p001.MICRO=MICRO
p001.P001_ROWS=[core[i:i+4] for i in range(0,28,4)]
p001.P001_BANNED_JOINING=set('هي')
p001.P001_CSS+=r'''
.presentation{height:16mm!important;flex:0 0 16mm!important;margin:4.8mm 3mm 2.4mm!important;transform:none!important}
...
.j2-grid{margin-top:1.8mm!important}
...
INJECT_JS='''({items,label})=>{
 ...
 grid.style.height='132mm';grid.style.maxHeight='132mm';grid.style.boxSizing='border-box';grid.style.marginBottom='0';grid.style.rowGap='3.0mm';
...
async def _write_pdf(page,out):
 names=[out/'QURBATA-JILID-2-P010-V10-MIM-NUN-PAGE10-VERTICAL-FILL.pdf']+[out/f'QURBATA-JILID-2-P010-V10-MIM-NUN-PAGE10-VERTICAL-FILL-LOCK-SAFE-{i:02d}.pdf' for i in range(1,100)]
 for idx,p in enumerate(names):
  try:
   await page.pdf(path=str(p),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});return p,('DIRECT_P010_V10' if idx==0 else f'LOCK_FALLBACK_P010_V10_{idx:02d}')
  except PermissionError:pass
 raise RuntimeError('P010_NO_AVAILABLE_PDF_NAME')

async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P010-V10.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  extra=await page.evaluate('''()=>{const pg=document.querySelector('.page'),num=document.querySelector('.page-number'),g=document.querySelector('.j2-grid'),e=document.querySelector('.p010-enrichment-row'),f=document.querySelector('.footer'),out=[];if(!pg||!num||!g||!e)return[{kind:'P010_REQUIRED_ELEMENT_MISSING'}];if(num.textContent.trim()!=='10')out.push({kind:'P010_PAGE_NUMBER_WRONG',actual:num.textContent.trim()});const gr=g.getBoundingClientRect(),er=e.getBoundingClientRect();const gap=er.top-gr.bottom;if(gap>28)out.push({kind:'P010_EXCESSIVE_BOTTOM_WHITESPACE',gap});if(gap<5)out.push({kind:'P010_CORE_ENRICHMENT_COLLISION',gap});if(f){const fr=f.getBoundingClientRect();if(er.bottom>fr.top-3)out.push({kind:'P010_ENRICHMENT_FOOTER_COLLISION'});}return out}''')
  ...
  await page.screenshot(path=str(png/'page-010-v10.png'),full_page=True);pdf,mode=await _write_pdf(page,out);await b.close()
...
def main():
 ...
 rc=v22.main()
 print('JILID2_P010_RENDERER_V10_PAGE10_VERTICAL_FILL=PASS');print('PAGE=10');print('P009_RENDERER_INHERITANCE=REMOVED');print('ACQUISITION_LETTERS=م|ن');print('PRESENTATION_TOP_MARGIN_MM=4.8');print('PRACTICE_FONT_SIZE_PT=33');print('CORE_GRID_HEIGHT_MM=132');print('CORE_ROW_GAP_MM=3.0');print('EXCESSIVE_BOTTOM_WHITESPACE_GUARD=ENABLED');print('BOTTOM_ROW_STEP=E04');return rc
