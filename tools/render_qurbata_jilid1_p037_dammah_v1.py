...
def audit():
 ...
 if set(HARAKAT)!={'َ','ِ','ُ'}:raise RuntimeError('P037_HARAKAT_REVIEW_INCOMPLETE')
...
def doc(u):
 css=f'''@page{{size:A5;margin:0}}*{{box-sizing:border-box}}@font-face{{font-family:"{FONT}";src:url("{u}")}}html,body{{margin:0}}.page{{width:148mm;height:210mm;padding:3.6mm 8mm 2.5mm;display:grid;grid-template-rows:17mm 18mm 1fr 5mm 22mm 2mm 12mm;overflow:hidden}}.header{{position:relative;display:flex;align-items:center;justify-content:center}}.logo{{position:absolute;left:0;width:32mm;height:17mm}}.title{{color:#064d37;font:700 6.2pt Georgia;letter-spacing:.16em}}.pageno{{position:absolute;right:0;top:0;width:12mm;background:#064d37;color:white;border-bottom:1mm solid #b98a2f;text-align:center;font:700 12pt Arial;padding:2.2mm 1mm 3mm;border-radius:0 0 3mm 3mm}}.presentation{{display:flex;align-items:center;justify-content:center;gap:14mm;font:44pt/1.3 "{FONT}";direction:rtl;overflow:visible}}.grid{{min-height:0;display:grid;grid-template-rows:repeat(7,minmax(0,1fr));row-gap:2.2mm;padding:2.2mm 0 2.8mm;overflow:visible}}.row{{display:flex;direction:rtl;align-items:center;justify-content:center;overflow:visible;min-height:0}}.r2{{gap:10mm}}.r3{{gap:11mm}}.practice{{display:flex;align-items:center;justify-content:center;font:36pt/1.28 "{FONT}";white-space:nowrap;overflow:visible;padding:.4mm 0}}.glyph{{display:inline-block;overflow:visible}}.l2{{width:23mm}}.l3{{width:35mm}}.run{{display:inline-flex;direction:rtl;align-items:center;overflow:visible}}.l2 .run{{gap:2.8mm}}.l3 .run{{gap:2.4mm}}.name-gap{{min-height:5mm}}.learning-strip{{border-top:.35mm solid #b9b9b9;display:grid;grid-template-columns:1fr 1px 36mm;align-items:center;column-gap:4mm;padding:1.2mm 2.5mm 1.4mm;overflow:visible}}.harakat-review{{display:flex;direction:rtl;align-items:center;justify-content:space-between;gap:2.5mm;height:100%}}.harakat-item{{width:9mm;height:14mm;display:flex;align-items:flex-start;justify-content:center;border-bottom:.55mm solid #222;overflow:visible}}.mark{{font:32pt/1.05 "{FONT}";display:inline-block;min-width:7mm;text-align:center;transform:translateY(-1.4mm);overflow:visible}}.strip-divider{{height:15mm;background:#c9c9c9}}.name-review{{display:flex;direction:rtl;align-items:center;justify-content:space-between;gap:3mm;font:30pt/1.3 "{FONT}";overflow:visible}}.name-review-item{{display:flex;align-items:center;justify-content:center;min-width:6mm;overflow:visible}}.safe{{min-height:2mm}}.footer{{display:flex;justify-content:space-between;align-items:center;padding-bottom:1mm;color:#173a2d}}.ar{{font:10.3pt "{FONT}";direction:rtl}}'''
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V8-HARAKAT-STRIP.pdf'
 async with async_playwright() as pw:
  ...
  if g['minMarkWidth']<25:raise RuntimeError('P037_HARAKAT_UNDERLINE_TOO_SHORT '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V8-HARAKAT-STRIP.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P037_DAMMAH=PASS');print('MATERIAL_NEW=ءُ|أُ|بُ|تُ');print('HAMZAH_BELOW_ALIF=FORBIDDEN');print('UNWANTED_BUTA_READING=0');print('BOTTOM_STRIP_PRIMARY=HARAKAT');print('HARAKAT_REVIEW=FATHAH|KASRAH|DAMMAH');print('HARAKAT_UNDERLINE=YES');print('NAME_REVIEW_COUNT=4');print('NAME_REVIEW=ا|ب|ت|ث');print('BOTTOM_FLOW=SAFE');print('PDF='+str(pdf.relative_to(ROOT)))
