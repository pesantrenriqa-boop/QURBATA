...
FOCUS={'ءُ','أُ','إُ','بُ'}
# First dammah page introduces four forms at once; review deliberately spans both mastered harakat.
EX=['ءُ بَ','أُ بِ','إُ تَ','بُ تِ','ءُ ثَ','أُ ثِ','إُ جَ','بُ جِ',
    'ءُ حَ بُ','أُ خِ بُ','إُ دَ ءُ','بُ ذِ أُ','ءُ رَ إُ','أُ زِ بُ','إُ سَ ءُ','بُ شِ أُ',
    'ءُ صَ بُ','أُ ضِ إُ','إُ طَ بُ','بُ ظِ ءُ','ءُ عَ أُ','أُ غِ بُ','إُ فَ ءُ']
...
def audit():
 ...
 if sum(x in FOCUS for x in ts)!=31:raise RuntimeError('P037_FOCUS_BALANCE_FAIL')
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V1.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V1.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P037_DAMMAH=PASS');print('DAMMAH_BLOCK_PAGE=1_OF_9');print('MAIN_COMPETENCY=DAMMAH');print('MATERIAL_NEW=ءُ|أُ|إُ|بُ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('REVIEW_SCOPE=FATHAH_PLUS_KASRAH');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
