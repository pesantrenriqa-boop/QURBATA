...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V4-FULL-VERTICAL-SAFE.pdf'
 async with async_playwright() as pw:
  ...
  g=await p.evaluate("()=>{let page=document.querySelector('.page').getBoundingClientRect(),footer=document.querySelector('.footer').getBoundingClientRect(),name=document.querySelector('.name-strip').getBoundingClientRect(),rows=[...document.querySelectorAll('.row')].map(e=>e.getBoundingClientRect()),pres=document.querySelector('.presentation').getBoundingClientRect();let gaps=rows.slice(1).map((r,i)=>r.top-rows[i].bottom);return {pageBottom:page.bottom,footerBottom:footer.bottom,bottomClear:page.bottom-footer.bottom,nameFooterGap:footer.top-name.bottom,minRowGap:Math.min(...gaps),presentationBottom:pres.bottom,firstRowTop:rows[0].top,presentationRowGap:rows[0].top-pres.bottom}}")
  if g['footerBottom']>g['pageBottom'] or g['bottomClear']<0 or g['minRowGap']<0 or g['presentationRowGap']<0:raise RuntimeError('P037_VERTICAL_FIT_FAIL '+str(g))
  if g['bottomClear']>18:raise RuntimeError('P037_EXCESS_BOTTOM_WHITESPACE '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V4-FULL-VERTICAL-SAFE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P037_DAMMAH=PASS');print('DAMMAH_BLOCK_PAGE=1_OF_9');print('MAIN_COMPETENCY=DAMMAH');print('MATERIAL_NEW=ءُ|أُ|بُ|تُ');print('HAMZAH_BELOW_ALIF=FORBIDDEN');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('VERTICAL_GLYPH_CLEARANCE=PASS');print('BOTTOM_PAGE_FIT=PASS');print('PRACTICE_FONT_PT=37');print('GRID_HEIGHT_MM=132');print('ROW_GAP_MM=2.6');print('PDF='+str(pdf.relative_to(ROOT)))
