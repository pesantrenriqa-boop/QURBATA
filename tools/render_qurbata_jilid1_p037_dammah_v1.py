...
FOCUS={'ءُ','أُ','بُ','تُ'}
EX=['ءُ بَ','أُ بِ','بُ تَ','تُ تِ','ءُ ثَ','أُ ثِ','بُ جَ','تُ جِ','ءُ حَ خِ','أُ دَ ذِ','بُ رَ زِ','تُ سَ شِ','ءُ صَ ضِ','أُ طَ ظِ','بُ عَ غِ','تُ فَ ءُ','ءُ قِ أُ','أُ كَ بُ','بُ لِ تُ','تُ مَ ءُ','ءُ نِ أُ',f'أُ {HA_F} بُ','بُ وَ تُ']
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V6-GRID-FLOW-SAFE.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V6-GRID-FLOW-SAFE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P037_DAMMAH=PASS');print('MATERIAL_NEW=ءُ|أُ|بُ|تُ');print('HAMZAH_BELOW_ALIF=FORBIDDEN');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('LAYOUT_ENGINE=CSS_GRID_PAGE_FLOW');print('LAST_ROW_TO_NAME_STRIP=SAFE');print('BOTTOM_FLOW=SAFE');print('NAME_FONT_PT=35');print('PRACTICE_FONT_PT=36');print('PDF='+str(pdf.relative_to(ROOT)))
