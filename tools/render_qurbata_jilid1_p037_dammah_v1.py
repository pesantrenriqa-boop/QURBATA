...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V13-COMPACT-HARAKAT.pdf'
 async with async_playwright() as pw:
  ...
  if g['lastGlyphToStrip']<5:raise RuntimeError('P037_HARAKAT_STRIP_COLLISION '+str(g))
  ...
  for x in g['items']:
   if x['k']=='kasrah' and x['anchorY']<=x['lineBottom']+2:raise RuntimeError('P037_KASRAH_ANCHOR_NOT_BELOW_LINE '+str(x))
   if x['k'] in ('fathah','dammah') and x['anchorY']>=x['lineTop']-2:raise RuntimeError('P037_UPPER_HARAKAT_ANCHOR_NOT_ABOVE_LINE '+str(x))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V13-COMPACT-HARAKAT.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P037_DAMMAH=PASS');print('PRACTICE_FONT_PT=39');print('HARAKAT_STRIP_HEIGHT_MM=19');print('HARAKAT_ITEM_HEIGHT_MM=15');print('HARAKAT_FONT_PT=32');print('HARAKAT_TO_LINE=COMPACT');print('FATHAH_POSITION=ABOVE_LINE');print('KASRAH_POSITION=BELOW_LINE');print('DAMMAH_POSITION=ABOVE_LINE');print('BOTTOM_FLOW=SAFE');print('PDF='+str(pdf.relative_to(ROOT)))
