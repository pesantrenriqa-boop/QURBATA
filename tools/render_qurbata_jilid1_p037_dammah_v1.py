...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V5-SEPARATED-NAMES.pdf'
 async with async_playwright() as pw:
  ...
  g=await p.evaluate("()=>{let page=document.querySelector('.page').getBoundingClientRect(),footer=document.querySelector('.footer').getBoundingClientRect(),name=document.querySelector('.name-strip').getBoundingClientRect(),last=document.querySelector('.row:last-child').getBoundingClientRect(),lastGlyphs=[...document.querySelectorAll('.row:last-child .glyph')].map(e=>e.getBoundingClientRect()),names=[...document.querySelectorAll('.name-strip>span')].map(e=>e.getBoundingClientRect());let maxGlyphBottom=Math.max(...lastGlyphs.map(x=>x.bottom));let minNameTop=Math.min(...names.map(x=>x.top));return {bottomClear:page.bottom-footer.bottom,lastGlyphToRule:name.top-maxGlyphBottom,glyphToName:minNameTop-maxGlyphBottom,nameTop:name.top,lastRowBottom:last.bottom}}")
  if g['bottomClear']<0 or g['bottomClear']>22:raise RuntimeError('P037_PAGE_FIT_FAIL '+str(g))
  if g['lastGlyphToRule']<10 or g['glyphToName']<28:raise RuntimeError('P037_NAME_STRIP_COLLISION '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V5-SEPARATED-NAMES.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P037_DAMMAH=PASS');print('MATERIAL_NEW=ءُ|أُ|بُ|تُ');print('HAMZAH_BELOW_ALIF=FORBIDDEN');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('LAST_ROW_TO_NAME_STRIP=SAFE');print('NAME_STRIP_COLLISION=0');print('NAME_FONT_PT=35');print('PRACTICE_FONT_PT=37');print('PDF='+str(pdf.relative_to(ROOT)))
