...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P037';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha';HA='ﮪ';HA_F='ﮪَ';HA_K='ﮪِ'
FOCUS={'ءُ','أُ','إُ','بُ'}
# Exact 61-token policy: 31 new-dammah focus + 30 cumulative review.
# Rows 1-2: 8 x (1 focus + 1 review) = 8 focus / 8 review.
# Remaining 15 groups: first 7 x (1 focus + 2 review), last 8 x (2 focus + 1 review).
EX=['ءُ بَ','أُ بِ','إُ تَ','بُ تِ','ءُ ثَ','أُ ثِ','إُ جَ','بُ جِ',
    'ءُ حَ خِ','أُ دَ ذِ','إُ رَ زِ','بُ سَ شِ','ءُ صَ ضِ','أُ طَ ظِ','إُ عَ غِ',
    'بُ فَ ءُ','ءُ قِ أُ','أُ كَ إُ','إُ لِ بُ','بُ مَ ءُ','ءُ نِ أُ',f'أُ {HA_F} إُ','إُ وَ بُ']
...
async def render(h):
 ...
 async with async_playwright() as pw:
  ...
  if g['left']<8 or g['right']<8 or g['top']<0 or g['bottom']<0:raise RuntimeError('P037_NAME_SAFEAREA_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P037-DAMMAH-01-CANDIDATE-V2-BALANCED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P037_DAMMAH=PASS');print('DAMMAH_BLOCK_PAGE=1_OF_9');print('MAIN_COMPETENCY=DAMMAH');print('MATERIAL_NEW=ءُ|أُ|إُ|بُ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('REVIEW_SCOPE=FATHAH_PLUS_KASRAH');print('HA_GLYPH=TWO_HOLE_ONLY');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
