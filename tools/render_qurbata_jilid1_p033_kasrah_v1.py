...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P033';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha';HA='ﮪ';HA_F='ﮪَ'
FOCUS={'مِ','نِ'}
OLD_KASRAH={'ءِ','إِ','بِ','تِ','ثِ','جِ','حِ','خِ','دِ','ذِ','رِ','زِ','سِ','شِ','صِ','ضِ','طِ','ظِ','عِ','غِ','فِ','قِ','كِ','لِ'}
FATHAH_POOL={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ','لَ','مَ','نَ',HA_F,'وَ','يَ'}
# 61 tokens = 31 focus + 30 review; review split = 15 old kasrah + 15 fathah.
# This page rotates through middle/late fathah forms to keep cumulative review even.
EX=['مِ دَ','نِ ذَ','مِ رَ','نِ زَ','مِ سَ','نِ شَ','مِ صَ','نِ ضَ',
    'مِ ءِ مِ','نِ إِ نِ','مِ بِ مِ','نِ تِ نِ','مِ ثِ مِ','نِ جِ نِ','مِ حِ مِ','نِ خِ نِ',
    'مِ دِ طَ','نِ ذِ ظَ','مِ رِ عَ','نِ زِ غَ','مِ سِ فَ','نِ شِ قَ','مِ صِ كَ']
NAMES=['ا','ب','ج','ح','خ','د','ر','س','ص','ع',HA,'ي']
...
def audit():
 ...
 if (len(ts),f,old+fa)!=(61,31,30):raise RuntimeError(f'P033_BALANCE_FAIL tokens={len(ts)} focus={f} review={old+fa}')
 if (old,fa)!=(15,15):raise RuntimeError(f'P033_REVIEW_SPLIT_FAIL kasrah={old} fathah={fa}')
 required={'دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ'}
 if not required.issubset(ts):raise RuntimeError('P033_FATHAH_ROTATION_FAIL')
 if 'ه' in NAMES or 'هَ' in ts:raise RuntimeError('P033_ONE_HOLE_HA_FORBIDDEN')
 if HA not in NAMES:raise RuntimeError('P033_TWO_HOLE_HA_REQUIRED')
def rows():
 o=[]
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">33</div></header><section class="presentation"><span>مِ</span><span>نِ</span></section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P033-KASRAH-13-CANDIDATE-V1-BALANCED.pdf'
 async with async_playwright() as pw:
  ...
  if names!=NAMES:raise RuntimeError('P033_NAME_ROW_MISMATCH')
  ...
  if g['left']<8 or g['right']<8 or g['top']<0 or g['bottom']<0:raise RuntimeError('P033_NAME_SAFEAREA_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P033-KASRAH-13-CANDIDATE-V1-BALANCED.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P033_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=مِ|نِ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('REVIEW_SPLIT=15_OLD_KASRAH|15_FATHAH');print('FATHAH_ROTATION=دَ|ذَ|رَ|زَ|سَ|شَ|صَ|ضَ|طَ|ظَ|عَ|غَ|فَ|قَ|كَ');print('HA_GLYPH=TWO_HOLE_ONLY');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
