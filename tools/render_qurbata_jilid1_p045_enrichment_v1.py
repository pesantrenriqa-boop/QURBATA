...
EX=['يِ وُ نَ','مُ لِ كَ','قَ فُ غِ','عِ ظَ طُ','ضُ صِ شَ','سَ زُ رِ','ذِ دَ خُ',
    ...
    'ضَ طِ ظُ','عُ غَ فِ','قِ كَ لُ','مَ نِ وُ','يُ '+HA_F+' بِ',HA_K+' وُ تَ','ثِ جُ حَ']
...
def audit():
 ...
 bad=[(i+1,g,[hk(x) for x in g]) for i,g in enumerate(groups) if sorted(hk(x) for x in g)!=['d','f','k']]
 if bad:raise RuntimeError('P045_HARAKAT_MIX_FAIL '+str(bad))
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P045-PENGAYAAN-02-CANDIDATE-V2-MIX-FIX.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P045-PENGAYAAN-02-CANDIDATE-V2-MIX-FIX.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P045_ENRICHMENT=PASS');print('PAGE_TYPE=FINAL_FATHAH_KASRAH_DAMMAH_ENRICHMENT');print('HARAKAT_BALANCE=21|21|21');print('EVERY_GROUP_MIX=FATHAH|KASRAH|DAMMAH');print('KASRAH_HAMZAH=إِ');print('BOTTOM_STRIP_PRIMARY=ARABIC_NUMERALS');print('JILID1_TARGET_END=P045');print('PDF='+str(pdf.relative_to(ROOT)))
