...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P028';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'صِ','ضِ'}
EX=['صِ سِ','ضِ شِ','صِ رِ','ضِ زِ','صِ دِ','ضِ ذِ','صِ حِ','ضِ خِ','صِ ثِ صِ','ضِ جِ ضِ','صِ بِ سِ','ضِ تِ شِ','صِ ءِ رِ','ضِ إِ زِ','صِ صَ دِ','ضِ ضَ ذِ','صِ طَ حِ','ضِ ظَ خِ','صِ عَ صِ','ضِ غَ ضِ','صِ فَ بِ','ضِ قَ تِ','صِ كَ ءِ']
NAMES=['ب','ث','ج','ح','خ','د','ر','س','ع','غ','ف','ق']
def run(s):return '<span class="run">'+''.join(f'<span>{x}</span>' for x in s.split())+'</span>'
def audit():
 ts=[x for e in EX for x in e.split()]
 if sum(x in FOCUS for x in ts)<23:raise RuntimeError('P028_FOCUS_TOO_LOW')
 if len(NAMES)!=12 or 'ف' not in NAMES or 'ق' not in NAMES:raise RuntimeError('P028_NAME_POLICY_FAIL')
def rows():
 o=[]
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">28</div></header><section class="presentation"><span>صِ</span><span>ضِ</span></section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P028-KASRAH-08-CANDIDATE-V1-SAFE.pdf'
 async with async_playwright() as pw:
  ...
  if names!=NAMES:raise RuntimeError('P028_NAME_ROW_MISMATCH')
  ...
  if g['left']<8 or g['right']<8 or g['top']<0 or g['bottom']<0:raise RuntimeError('P028_NAME_SAFEAREA_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf,g
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P028-KASRAH-08-CANDIDATE-V1-SAFE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P028_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=صِ|ضِ');print('KASRAH_REVIEW=سِ|شِ|رِ|زِ|دِ|ذِ|حِ|خِ|ثِ|جِ|بِ|تِ|ءِ|إِ');print('NAME_ROW=BOTTOM_ONLY_ROTATING_CUMULATIVE');print('NAME_ROW_NEW=ف|ق');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
