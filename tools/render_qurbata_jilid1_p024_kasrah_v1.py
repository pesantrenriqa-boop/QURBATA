#!/usr/bin/env python3
# QURBATA J1 P024 — Kasrah stage 4: حِ خِ; cumulative kasrah + fathah review; staged letter-name strand.
...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P024';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'حِ','خِ'}
EX=['حِ ثِ','خِ جِ','حِ بِ','خِ تِ','حِ ءِ','خِ إِ','حِ حَ','خِ خَ','حِ دَ حِ','خِ ذَ خِ','حِ رَ ثِ','خِ زَ جِ','حِ سَ بِ','خِ شَ تِ','حِ صَ ءِ','خِ ضَ إِ','حِ طَ حِ','خِ ظَ خِ','حِ عَ ثِ','خِ غَ جِ','حِ فَ بِ','خِ قَ تِ','حِ كَ ءِ']
NAMES=['ا','ب','ت','ث','ج','ح','خ','د','ذ','ر','ز','س','ش','ا']
def run(s):return '<span class="run">'+''.join(f'<span>{x}</span>' for x in s.split())+'</span>'
def audit():
 ts=[x for e in EX for x in e.split()]
 if sum(x in FOCUS for x in ts)<23:raise RuntimeError('P024_FOCUS_TOO_LOW')
 if NAMES[:13]!=['ا','ب','ت','ث','ج','ح','خ','د','ذ','ر','ز','س','ش']:raise RuntimeError('P024_NAME_LADDER_FAIL')
def rows():
 o=[]
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="pageno">24</div></header><section class="presentation"><span>حِ</span><span>خِ</span></section><section class="grid">{rows()}</section><section class="name-strip">{names}</section><div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P024-KASRAH-04-CANDIDATE-V1.pdf'
 async with async_playwright() as pw:
  ...
  if names!=NAMES:raise RuntimeError('P024_NAME_ROW_MISMATCH')
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P024-KASRAH-04-CANDIDATE-V1.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P024_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=حِ|خِ');print('KASRAH_REVIEW=ءِ|إِ|بِ|تِ|ثِ|جِ');print('NAME_ROW=BOTTOM_ONLY_STAGED_CUMULATIVE');print('NAME_ROW_NEW=س|ش');print('NAME_ROW_REVIEW=ا|ب|ت|ث|ج|ح|خ|د|ذ|ر|ز');print('PDF='+str(pdf.relative_to(ROOT)))
