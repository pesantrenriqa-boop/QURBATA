#!/usr/bin/env python3
"""QURBATA Jilid 2 P025 — penguatan mad ya, tangga 3/4/5 huruf."""
...
ROWS=[
['دِينُ','فِيلُ','جِيدُ','عِيدُ'],['خِيرُ','سِيرُ','بِيرُ','زِيرُ'],
['رَحِيمُ','كَرِيمُ','حَكِيمُ','عَلِيمُ'],['عَظِيمُ','شَهِيدُ','وَكِيلُ','مَجِيدُ'],
['بَصِيرُ','خَبِيرُ','قَدِيرُ','سَمِيعُ'],['نَذِيرُ','بَشِيرُ','قَرِيبُ','مُنِيبُ'],
['مِيثَاقُ','مِيزَانُ','قِيلَتْ','زِينَةُ'],['سَبِيلُ','نَصِيرُ','حَمِيدُ','شَدِيدُ'],
]
...
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P025-V3.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  lesson=await p.evaluate("""()=>[...document.querySelectorAll('.j2-glyph,.presentation-object .arabic-part')].map(e=>e.textContent||'').join('|')""")
  # Correct semantic gate: P025 may contain legitimate consonant sukun (e.g. قِيلَتْ).
  # What is forbidden is an explicit sukun attached to the MAD YA itself: kasra + ya + sukun.
  bad_mad_ya=('ِيْ' in lesson) or ('ِيۡ' in lesson)
  if bad_mad_ya:raise RuntimeError('P025_EXPLICIT_SUKUN_ON_MAD_YA_FORBIDDEN='+repr(lesson))
  ...
  metrics,issues=await p001.fit_and_inspect(p);report.write_text(json.dumps({'layout_issues':issues,'stage':'MAD_YA_REINFORCEMENT_3_4_5','mad_ya_explicit_sukun':False,'consonant_sukun_allowed':True},ensure_ascii=False,indent=2),encoding='utf-8')
  ...
  pdf=out/'QURBATA-JILID-2-P025-V3-MAD-YA-REINFORCEMENT.pdf';await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return metrics,report,pdf
p001.render=render
def main():
 rc=v22.main();print('JILID2_P025_RENDERER_V3=PASS');print('PAGE=25');print('SEQUENCE=CONTINUE_MAD_YA');print('LADDER=3_4_5_LETTERS');print('EXPLICIT_SUKUN_ON_MAD_YA=NO');print('LEGITIMATE_CONSONANT_SUKUN=ALLOWED');print('FONT=KFGQPC_UTHMAN_TAHA');print('STATUS=P025_CANDIDATE_NOT_FROZEN');return rc
