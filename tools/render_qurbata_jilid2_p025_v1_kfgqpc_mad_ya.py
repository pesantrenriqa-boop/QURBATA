...
# P025 remains inside the mad-ya reinforcement block. No explicit sukun is drawn on ya madd.
ROWS=[
...
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P025-V1.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  ...
  if 'ۡ' in body or 'ْ' in body:raise RuntimeError('P025_EXPLICIT_SUKUN_FORBIDDEN')
  metrics,issues=await p001.fit_and_inspect(p);report.write_text(json.dumps({'layout_issues':issues,'stage':'MAD_YA_REINFORCEMENT_3_4_5'},ensure_ascii=False,indent=2),encoding='utf-8')
  ...
  pdf=out/'QURBATA-JILID-2-P025-V1-MAD-YA-REINFORCEMENT.pdf';await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return metrics,report,pdf
p001.render=render
def main():
 rc=v22.main();print('JILID2_P025_RENDERER_V1=PASS');print('PAGE=25');print('SEQUENCE=CONTINUE_MAD_YA');print('LADDER=3_4_5_LETTERS');print('EXPLICIT_SUKUN_ON_MAD_YA=NO');print('FONT=KFGQPC_UTHMAN_TAHA');print('STATUS=P025_CANDIDATE_NOT_FROZEN');return rc
