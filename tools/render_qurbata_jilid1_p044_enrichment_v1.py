...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P044';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha';HA='ﮪ';HA_F='ﮪَ';HA_K='ﮪِ';HA_D='ﮪُ'
# Enrichment only: every 3-letter group contains one fathah, one kasrah, one dammah.
EX=['بَ تِ ثُ','جُ حَ خِ','دِ ذُ رَ','زَ سُ شِ','صِ ضَ طُ','ظُ عِ غَ','فَ قُ كِ',
    'لِ مَ نُ','وُ يَ '+HA_K,'أُ إِ ءَ','ثَ جِ حُ','خُ دَ ذِ','رِ زُ سَ','شَ صُ ضِ',
    'طِ ظَ عُ','غُ فِ قَ','كَ لُ مِ','نِ وَ يُ',HA_D+' بِ تَ','ثِ جَ حُ','خَ دُ ذِ']
ARABIC_NUMERALS=['١','٢','٣','٤','٥','٦','٧','٨','٩','١٠'];NAME_REVIEW=['ه','و','ي','ء']
...
def audit():
 ...
 if len(EX)!=21 or any(len(g)!=3 for g in groups) or len(ts)!=63:raise RuntimeError('P044_ENRICHMENT_GRID_FAIL')
 if any(sorted(hk(x) for x in g)!=['d','f','k'] for g in groups):raise RuntimeError('P044_HARAKAT_MIX_FAIL')
 c={h:sum(hk(x)==h for x in ts) for h in ('f','k','d')}
 if c!={'f':21,'k':21,'d':21}:raise RuntimeError('P044_HARAKAT_BALANCE_FAIL '+str(c))
 if 'أِ' in ts:raise RuntimeError('P044_WRONG_KASRAH_HAMZAH_FORM')
 if 'إُ' in ts:raise RuntimeError('P044_WRONG_DAMMAH_HAMZAH_FORM')
 if any(x in {'ه','هَ','هِ','هُ'} for x in ts):raise RuntimeError('P044_ONE_HOLE_HA_FORBIDDEN')
 if HA_D not in ts:raise RuntimeError('P044_TWO_HOLE_HA_DAMMAH_REQUIRED')
def rows():return ''.join('<div class="row">'+''.join(f'<div class="practice">{run(x)}</div>' for x in EX[i:i+3])+'</div>' for i in range(0,21,3))
def bottom_strip():
 nums=''.join(f'<span class="number-item">{x}</span>' for x in ARABIC_NUMERALS)
 names=''.join(f'<span class="name-review-item">{(HA if x=="ه" else x)}</span>' for x in NAME_REVIEW)
...
def doc(u):
 ...
 return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1 · PENGAYAAN</div><div class="pageno">44</div></header><section class="presentation"><span>بَ</span><span>بِ</span><span>بُ</span></section><section class="grid">{rows()}</section><div></div>{bottom_strip()}<div class="safe"></div><footer class="footer"><span class="ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P044-PENGAYAAN-01-CANDIDATE-V1.pdf'
 async with async_playwright() as pw:
  ...
  if g['lastRowBottom']>g['gridBottom']+1 or g['lastGlyphToStrip']<5:raise RuntimeError('P044_LAYOUT_FAIL '+str(g))
  if g['stripBottom']>g['footerTop'] or g['footerBottom']>g['pageBottom']+1:raise RuntimeError('P044_BOTTOM_FLOW_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P044-PENGAYAAN-01-CANDIDATE-V1.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P044_ENRICHMENT=PASS');print('PAGE_TYPE=FATHAH_KASRAH_DAMMAH_ENRICHMENT');print('HARAKAT_BALANCE=21|21|21');print('HA_DAMMAH=TWO_HOLE_ONLY');print('KASRAH_HAMZAH=إِ');print('BOTTOM_STRIP_PRIMARY=ARABIC_NUMERALS');print('PDF='+str(pdf.relative_to(ROOT)))
