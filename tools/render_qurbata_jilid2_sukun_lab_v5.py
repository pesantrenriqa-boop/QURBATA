...
async def run(out,kfg,amiri,kfgsrc,sukun_y_shift):
    ...
    async with async_playwright() as pw:
        ...
        await p.screenshot(path=str(out/'SUKUN-LAB-V7-2.png'),full_page=True);await p.pdf(path=str(out/'SUKUN-LAB-V7-2.pdf'),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
    (out/'SUKUN-LAB-V7-2.json').write_text(json.dumps({'kfg_source':kfgsrc,'kfg_font':kfg.name,'amiri_font':amiri.name,'patch':meta,'book_pages_modified':False},ensure_ascii=False,indent=2),encoding='utf-8');print('SUKUN_LAB_V7_2=PASS');print('BASE_LETTERS=KFGQPC_UTHMAN_TAHA');print('RENDER_CODEPOINT=U+0652');print('VISUAL_OUTLINE=AMIRI_U+06E1');print(f'SUKUN_Y_SHIFT={sukun_y_shift}');print('POSITIONING=KFGQPC_U+0652_GPOS_PRESERVED');print('BOOK_PAGES_MODIFIED=NO')
