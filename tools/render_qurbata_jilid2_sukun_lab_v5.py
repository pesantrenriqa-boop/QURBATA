#!/usr/bin/env python3
"""QURBATA Jilid 2 — SUKUN LAB V7.2.
Keeps KFGQPC Arabic letters and U+0652 positioning, replacing only the sukun outline
with Amiri U+06E1. V7.2 lowers the replacement outline slightly while preserving the
...
def patch_font(kfg_path:Path,amiri_path:Path,dst:Path,sukun_y_shift:int):
    ...
    pen=TTGlyphPen(kgs)
    # TrueType Y grows upward, therefore a negative Y translation lowers the glyph.
...
async def run(out,kfg,amiri,kfgsrc,sukun_y_shift):
    ...
    async with async_playwright() as pw:
        ...
        await p.screenshot(path=str(out/'SUKUN-LAB-V7-2.png'),full_page=True);pdf_path=await write_pdf_with_fallback(p,out);await b.close()
    (out/'SUKUN-LAB-V7-2.json').write_text(json.dumps({'kfg_source':kfgsrc,'kfg_font':kfg.name,'amiri_font':amiri.name,'patch':meta,'pdf':pdf_path.name,'book_pages_modified':False},ensure_ascii=False,indent=2),encoding='utf-8');print('SUKUN_LAB_V7_2=PASS');print('BASE_LETTERS=KFGQPC_UTHMAN_TAHA');print('RENDER_CODEPOINT=U+0652');print('VISUAL_OUTLINE=AMIRI_U+06E1');print(f'SUKUN_Y_SHIFT={sukun_y_shift}');print('POSITIONING=KFGQPC_U+0652_GPOS_PRESERVED');print(f'PDF_OUTPUT={pdf_path}');print('BOOK_PAGES_MODIFIED=NO')

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output-dir',default='dist/jilid-2-sukun-lab-v7-2');ap.add_argument('--font-file');ap.add_argument('--font-zip');ap.add_argument('--amiri-font');ap.add_argument('--sukun-y-shift',type=int,default=DEFAULT_SUKUN_Y_SHIFT,help='Vertical shift in KFGQPC font units; negative values move the sukun downward (default: -120).');a=ap.parse_args();out=Path(a.output_dir);out=out if out.is_absolute() else ROOT/out;out.mkdir(parents=True,exist_ok=True);kfg,src=kfgloader.discover_font(a.font_file,a.font_zip,out);amiri=discover_amiri(a.amiri_font,out);asyncio.run(run(out,kfg,amiri,src,a.sukun_y_shift))
