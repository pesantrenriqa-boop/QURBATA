#!/usr/bin/env python3
"""QURBATA Jilid 2 — SUKUN LAB V7.6.
Keeps KFGQPC Arabic letters and U+0652 positioning, replacing only the sukun outline
with Amiri U+06E1. V7.6 lowers the replacement outline further while preserving
...
async def run(out,kfg,amiri,kfgsrc,sukun_y_shift):
    ...
    (out/'SUKUN-LAB-V7-6.json').write_text(json.dumps({'kfg_source':kfgsrc,'kfg_font':kfg.name,'amiri_font':amiri.name,'patch':meta,'pdf':pdf_path.name,'book_pages_modified':False},ensure_ascii=False,indent=2),encoding='utf-8');print('SUKUN_LAB_V7_6=PASS');print('BASE_LETTERS=KFGQPC_UTHMAN_TAHA');print('RENDER_CODEPOINT=U+0652');print('VISUAL_OUTLINE=AMIRI_U+06E1');print(f'SUKUN_Y_SHIFT={sukun_y_shift}');print('POSITIONING=KFGQPC_U+0652_GPOS_PRESERVED');print(f'PDF_OUTPUT={pdf_path}');print('BOOK_PAGES_MODIFIED=NO')

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output-dir',default='dist/jilid-2-sukun-lab-v7-6');ap.add_argument('--font-file');ap.add_argument('--font-zip');ap.add_argument('--amiri-font');ap.add_argument('--sukun-y-shift',type=int,default=DEFAULT_SUKUN_Y_SHIFT,help='Vertical shift in KFGQPC font units; negative values move the sukun downward (default: -1700).');a=ap.parse_args();out=Path(a.output_dir);out=out if out.is_absolute() else ROOT/out;out.mkdir(parents=True,exist_ok=True);kfg,src=kfgloader.discover_font(a.font_file,a.font_zip,out);amiri=discover_amiri(a.amiri_font,out);asyncio.run(run(out,kfg,amiri,src,a.sukun_y_shift))
