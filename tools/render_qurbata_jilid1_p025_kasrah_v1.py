...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P025';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'دِ','ذِ'}
EX=['دِ حِ','ذِ خِ','دِ ثِ','ذِ جِ','دِ بِ','ذِ تِ','دِ ءِ','ذِ إِ','دِ بَ دِ','ذِ تَ ذِ','دِ ثَ حِ','ذِ جَ خِ','دِ حَ بِ','ذِ خَ تِ','دِ دَ ءِ','ذِ ذَ إِ','دِ رَ دِ','ذِ زَ ذِ','دِ سَ حِ','ذِ شَ خِ','دِ صَ بِ','ذِ ضَ تِ','دِ طَ ءِ']
NAMES=['ا','ب','ت','ث','ج','ح','خ','د','ذ','ر','ز','س','ش','ص','ض']
def run(s):return '<span class="run">'+''.join(f'<span>{x}</span>' for x in s.split())+'</span>'
def audit():
 ts=[x for e in EX for x in e.split()]
 if sum(x in FOCUS for x in ts)<23:raise RuntimeError('P025_FOCUS_TOO_LOW')
 if NAMES[-2:]!=['ص','ض']:raise RuntimeError('P025_NAME_STAGE_FAIL')
def rows():
 o=[]
