#!/usr/bin/env python3
"""QURBATA Jilid 1 P001 — V7 proportional grouped practice spacing."""
...
def build_rows(objects):
    l2=[o for o in objects if len(tokens(o))==2];l3=[o for o in objects if len(tokens(o))==3]
    rows=[]
...
async def render(h,out):
    pdf=out/'QURBATA-JILID-1-P001-PRINT-ANCHOR-V7-PROPORTIONAL-GROUPS.pdf';png=out/'QURBATA-JILID-1-P001-PRINT-ANCHOR-V7-PROPORTIONAL-GROUPS.png'
    async with async_playwright() as pw:
        ...
        if not await p.evaluate(f"()=>document.fonts.check('43pt \\\"{FONT_FAMILY}\\\"','بَ تَ ثَ')"):raise RuntimeError('QJ1_P001_KFGQPC_FONT_BINDING_FAIL')
    ...
    return pdf,png
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output-dir',default=str(DEFAULT_OUT.relative_to(ROOT)));ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();out=Path(a.output_dir);out=out if out.is_absolute() else ROOT/out;out.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,out);objs,mode,removed=load_objects();h=out/'QURBATA-JILID-1-P001-PRINT-ANCHOR-V7-PROPORTIONAL-GROUPS.html';h.write_text(build(font.resolve().as_uri(),objs),encoding='utf-8');pdf,png=asyncio.run(render(h,out));print('QJ1_P001_PRINT_ANCHOR_V7=PASS');print('BASE_FONT=KFGQPC_UTHMAN_TAHA');print('PRACTICE_FONT_PT=43');print('TWO_LETTER_ROWS=2|GROUPS_PER_ROW=4|INTRA_GROUP_GAP=4.8MM|INTER_GROUP_GAP=12MM');print('THREE_LETTER_ROWS=5|GROUPS_PER_ROW=3|INTRA_GROUP_GAP=3.6MM|INTER_GROUP_GAP=15MM');print('GROUPING=EXPLICIT_ROW_CONTAINERS');print('REMOVED_ORPHAN_OBJECT='+removed);print(f'CONTENT_SOURCE_MODE={mode}');print(f'FONT_SOURCE={src}');print(f'PDF={pdf.relative_to(ROOT)}');print(f'PNG={png.relative_to(ROOT)}');return 0
