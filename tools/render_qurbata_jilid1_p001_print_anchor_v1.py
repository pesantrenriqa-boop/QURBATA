#!/usr/bin/env python3
"""QURBATA Jilid 1 P001 — clean global shell, proportional grouped practice."""
...
def build(font_uri,objects):
    ...
    css=f'''@page{{size:A5;margin:0}}*{{box-sizing:border-box}}@font-face{{font-family:"{FONT_FAMILY}";src:url("{font_uri}") format("truetype");font-display:block}}html,body{{margin:0;padding:0;background:#fff}}.page{{width:148mm;height:210mm;padding:3.6mm 8mm 2.5mm;display:flex;flex-direction:column;overflow:hidden;font-family:Arial,sans-serif}}.header{{height:17mm;flex:0 0 17mm;position:relative;display:flex;align-items:center;justify-content:center}}.brand-logo{{position:absolute;left:0;width:32mm;height:17mm;object-fit:contain}}.title{{position:absolute;left:50%;transform:translateX(-50%);text-align:center;color:#064d37;font-family:Georgia,"Times New Roman",serif;font-size:6.2pt;font-weight:700;letter-spacing:.16em;white-space:nowrap}}.page-number{{position:absolute;right:0;top:0;width:12mm;background:#064d37;color:#fff;border-bottom:1mm solid #b98a2f;text-align:center;font-weight:700;padding:2.2mm 1mm 3mm;border-radius:0 0 3mm 3mm;font-size:12pt}}.presentation{{height:20mm;flex:0 0 20mm;display:flex;align-items:center;justify-content:center;gap:13mm;font-family:"{FONT_FAMILY}",serif;font-size:46pt;line-height:1.05;direction:rtl;font-feature-settings:'mark' 1,'mkmk' 1}}.practice-grid{{height:158mm;flex:0 0 158mm;display:grid;grid-template-rows:repeat(7,minmax(0,1fr));row-gap:3.2mm;padding:.8mm 0 1mm;overflow:visible}}.practice-row{{display:flex;direction:rtl;align-items:center;justify-content:center;min-height:0;width:100%;overflow:visible}}.row-l2{{gap:10mm}}.row-l3{{gap:11mm}}.practice{{display:flex;align-items:center;justify-content:center;flex:0 0 auto;font-family:"{FONT_FAMILY}",serif;font-size:40pt;line-height:1.02;white-space:nowrap;border:0!important;background:transparent!important;box-shadow:none!important;font-feature-settings:'mark' 1,'mkmk' 1;text-rendering:optimizeLegibility;overflow:visible}}.l2{{width:23mm}}.l3{{width:35mm}}.token-run{{display:inline-flex;direction:rtl;align-items:center;justify-content:center}}.l2 .token-run{{gap:2.8mm}}.l3 .token-run{{gap:2.4mm}}.letter-token{{display:inline-block;direction:rtl;unicode-bidi:isolate}}.footer{{height:12mm;flex:0 0 12mm;display:flex;justify-content:space-between;align-items:center;padding:0 0 2.2mm;color:#173a2d}}.footer-ar{{font-family:"{FONT_FAMILY}",serif;font-size:10.3pt;line-height:1;direction:rtl;font-feature-settings:'mark' 1,'mkmk' 1}}'''
    return f'''<!doctype html><html><head><meta charset="utf-8"><style>{css}</style></head><body><main class="page"><header class="header"><img class="brand-logo" src="{LOGO.resolve().as_uri()}"><div class="title">QURBATA · JILID 1</div><div class="page-number">01</div></header><section class="presentation" lang="ar" dir="rtl"><span>بَ</span><span>تَ</span><span>ثَ</span></section><section class="practice-grid">{rows}</section><footer class="footer"><span class="footer-ar">قُرْآنٌ · لُغَةٌ · أَدَبٌ</span><span class="footer-ar">تَعَلَّمْ · اِعْمَلْ · عَلِّمْ</span></footer></main></body></html>'''
def free_path(base):
    ...
    raise RuntimeError('QJ1_P001_NO_FREE_OUTPUT_FILENAME')
async def render(h,out):
    pdf=free_path(out/'QURBATA-JILID-1-P001-PRODUCTION-CLEAN-V10.pdf');png=free_path(out/'QURBATA-JILID-1-P001-PRODUCTION-CLEAN-V10.png')
    async with async_playwright() as pw:
        ...
        if not await p.evaluate(f"()=>document.fonts.check('10.3pt \\\"{FONT_FAMILY}\\\"','قُرْآنٌ تَعَلَّمْ')"):raise RuntimeError('QJ1_P001_FOOTER_FONT_BINDING_FAIL')
    ...
    return pdf,png
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output-dir',default=str(DEFAULT_OUT.relative_to(ROOT)));ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();out=Path(a.output_dir);out=out if out.is_absolute() else ROOT/out;out.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,out);objs,mode,removed=load_objects();h=out/'QURBATA-JILID-1-P001-PRODUCTION-CLEAN-V10.html';h.write_text(build(font.resolve().as_uri(),objs),encoding='utf-8');pdf,png=asyncio.run(render(h,out));print('QJ1_P001_PRODUCTION_CLEAN_V10=PASS');print('LOCKED_OUTPUT_POLICY=AUTO_VERSIONED_FILENAME');print('HEADER_TITLE_ALIGNMENT=PAGE_CENTER_MATCHES_PRESENTATION_CENTER');print('FOOTER_BOTTOM_CLEARANCE=RAISED_2.2MM');print('GLOBAL_SHELL=QURBATA_CLEAN');print('BASE_FONT=KFGQPC_UTHMAN_TAHA');print(f'PDF={pdf.relative_to(ROOT)}');print(f'PNG={png.relative_to(ROOT)}');return 0
