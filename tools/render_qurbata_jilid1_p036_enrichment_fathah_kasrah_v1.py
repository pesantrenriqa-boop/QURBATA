...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P036-PENGAYAAN-FATHAH-KASRAH-02-CANDIDATE-V2-GRID.pdf'
 async with async_playwright() as pw:
  ...
  if g['left']<0 or g['right']<0 or g['rows']!=7:raise RuntimeError('P036_GRID_SAFE_FAIL '+str(g))
  await p.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return pdf
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P036-PENGAYAAN-FATHAH-KASRAH-02-CANDIDATE-V2-GRID.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf=asyncio.run(render(h));print('QJ1_P036_ENRICHMENT=PASS');print('LAYOUT=7_ROWS_X_3_GROUPS');print('GROUP_SIZE=3_LETTERS_ONLY');print('GROUP_SPACING=5MM');print('INTRA_GROUP_SPACING=2.8MM');print('HA_GLYPH=TWO_HOLE_ONLY');print('PDF='+str(pdf.relative_to(ROOT)))
