...
enrich=ladder['E05']
main_items=['طه','طسم','طس']
review_text='٠ ١ ٢ ٣ ٤ ٥ ٦ ٧ ٨ ٩   |   ألف دال ذال راء زاي واو'
...
def build(debug):
 ...
 # Force the acquisition Ha into medial contextual form with tatweel on both sides: ـهَـ
 pres=f'''<section class="presentation"><div class="presentation-object-wrap"><div class="presentation-object" dir="ltr"><span class="arabic-part">{p001.arabic_html('وَصَلَ')}</span><span class="arrow">←</span><span class="arabic-part">{p001.arabic_html('وَ')}</span><span class="arabic-part">{p001.arabic_html('هَبَطَ')}</span><span class="arrow">←</span><span class="arabic-part">{p001.arabic_html('ـهَـ')}</span></div></div></section>'''
 return h[:s]+pres+h[e:]
p001.build_page_html=build
INJECT_JS='''({items,label,review})=>{const page=document.querySelector('.page'),grid=document.querySelector('.j2-grid'),pr=document.querySelector('.presentation');if(!page||!grid||!pr)return {ok:false};page.style.position='relative';page.style.overflow='hidden';const box=document.createElement('div');box.className='p011-enrichment-row';Object.assign(box.style,{position:'absolute',left:'11mm',right:'11mm',bottom:'16mm',height:'25mm',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-start',padding:'0 3mm 1.2mm',boxSizing:'border-box',background:'#fff',textAlign:'center',overflow:'visible',zIndex:'20'});const line=document.createElement('div');Object.assign(line.style,{width:'100%',height:'0',borderTop:'.22mm solid #111',margin:'0 0 1.2mm'});box.appendChild(line);const lab=document.createElement('div');lab.textContent=label;Object.assign(lab.style,{fontFamily:'Arial,sans-serif',fontSize:'5.1pt',fontWeight:'700',margin:'0 0 .6mm',lineHeight:'1'});box.appendChild(lab);const run=document.createElement('div');Object.assign(run.style,{width:'88%',display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',alignItems:'center',justifyItems:'center',columnGap:'4mm',fontFamily:'KFGQPC Uthman Taha Naskh, Amiri Quran, Amiri, serif',fontSize:'33pt',lineHeight:'1.15',direction:'rtl',whiteSpace:'nowrap',overflow:'visible',padding:'.1mm 0 .4mm'});for(const t of items){const sp=document.createElement('span');sp.textContent=t;Object.assign(sp.style,{display:'inline-block',overflow:'visible',lineHeight:'1.15',padding:'0 .5mm .5mm'});run.appendChild(sp)}box.appendChild(run);const rev=document.createElement('div');rev.className='p011-review-strip';rev.textContent='مُرَاجَعَة: '+review;Object.assign(rev.style,{width:'100%',fontFamily:'KFGQPC Uthman Taha Naskh, Amiri Quran, Amiri, serif',fontSize:'11.5pt',lineHeight:'1.25',direction:'rtl',whiteSpace:'nowrap',marginTop:'.5mm',paddingTop:'.5mm',borderTop:'.15mm solid #999',overflow:'visible'});box.appendChild(rev);page.appendChild(box);const pg=page.getBoundingClientRect(),prr=pr.getBoundingClientRect(),er=box.getBoundingClientRect();const topPx=prr.bottom-pg.top+33,bottomPx=er.top-pg.top-6,heightPx=bottomPx-topPx;if(heightPx<275)return {ok:false,reason:'INSUFFICIENT_GRID_HEIGHT',heightPx};for(const [n,v] of [['position','absolute'],['left','11mm'],['right','11mm'],['top',topPx+'px'],['height',heightPx+'px'],['min-height','0px'],['max-height',heightPx+'px'],['margin','0'],['flex','none'],['box-sizing','border-box'],['row-gap','2.4mm'],['z-index','10']])grid.style.setProperty(n,v,'important');return {ok:true,topPx,bottomPx,heightPx};}'''
async def render(h,out,debug):
 report=out/'LAYOUT-OVERFLOW-REPORT-J2-P011-V3.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
 async with async_playwright() as pw:
  b=await pw.chromium.launch();page=await b.new_page(viewport={'width':1120,'height':1584},device_scale_factor=2);await page.goto(h.resolve().as_uri(),wait_until='networkidle');await page.evaluate('document.fonts.ready');inj=await page.evaluate(INJECT_JS,{'items':main_items,'label':enrich['Label'],'review':review_text})
  ...
  await page.screenshot(path=str(png/'page-011-v3.png'),full_page=True);pdf=out/'QURBATA-JILID-2-P011-V3-HA-MEDIAL-E05-REVIEW.pdf';await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await b.close()
 return metrics,report,pdf,'DIRECT_P011_V3'
p001.render=render
def main():
 ...
 rc=v22.main();print('JILID2_P011_RENDERER_V3_HA_MEDIAL_E05_REVIEW=PASS');print('PAGE=11');print('ACQUISITION_LETTERS=ه|و');print('HA_TITLE_FORM=FORCED_MEDIAL_ـهَـ');print('CORE_MATERIAL_SHIFT_DOWN=FURTHER');print('BOTTOM_NEW_COMPETENCY=E05_AWAILUSSURAR_3');print('BOTTOM_MAIN_ITEMS=طه|طسم|طس');print('BOTTOM_MAIN_FONT_PT=33');print('BOTTOM_REVIEW=ARABIC_NUMERALS_PLUS_NON_JOINER_NAMES');print('CORE_WORDS=28_MEANINGFUL');return rc
