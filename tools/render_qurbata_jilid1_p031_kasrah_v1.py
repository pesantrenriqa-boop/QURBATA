...
FOCUS={'فِ','قِ'}
EX=['فِ عِ','قِ غِ','فِ طِ','قِ ظِ','فِ صِ','قِ ضِ','فِ سِ','قِ شِ','فِ رِ فِ','قِ زِ قِ','فِ دِ عِ','قِ ذِ غِ','فِ حِ طِ','قِ خِ ظِ','فِ ثِ صِ','قِ جِ ضِ','فِ بِ سِ','قِ تِ شِ','فِ ءِ رِ','قِ إِ زِ','فِ فَ دِ','قِ قَ ذِ','فِ كَ بِ']
...
def audit():
 ...
 if sum(x in FOCUS for x in ts)<23:raise RuntimeError('P031_FOCUS_TOO_LOW')
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P031-KASRAH-11-CANDIDATE-V1-SAFE.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P031-KASRAH-11-CANDIDATE-V1-SAFE.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P031_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=فِ|قِ');print('NAME_ROW=BOTTOM_ONLY_ROTATING_CUMULATIVE');print('NAME_ROW_NEW=و|ي');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
