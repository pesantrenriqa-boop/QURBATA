...
OUT=ROOT/'dist/qurbata-print-ready/jilid-1/pages/P031';LOGO=ROOT/'books/shared/assets/qurbata-logo.svg';FONT='QURBATA KFGQPC Uthman Taha'
FOCUS={'فِ','قِ'}
OLD_KASRAH={'ءِ','إِ','بِ','تِ','ثِ','جِ','حِ','خِ','دِ','ذِ','رِ','زِ','سِ','شِ','صِ','ضِ','طِ','ظِ','عِ','غِ'}
FATHAH_POOL={'ءَ','أَ','بَ','تَ','ثَ','جَ','حَ','خَ','دَ','ذَ','رَ','زَ','سَ','شَ','صَ','ضَ','طَ','ظَ','عَ','غَ','فَ','قَ','كَ','لَ','مَ','نَ','هَ','وَ','يَ'}
# 61 tokens = 31 focus + 30 review. Review split = 15 old-kasrah + 15 fathah.
# Fathah rotation on this page intentionally covers late/underrepresented forms.
EX=['فِ وَ','قِ يَ','فِ نَ','قِ هَ','فِ مَ','قِ لَ','فِ كَ','قِ قَ',
...
async def render(h):
 pdf=OUT/'QURBATA-JILID-1-P031-KASRAH-11-CANDIDATE-V2-BALANCED-REVIEW.pdf'
...
if __name__=='__main__':
 audit();ap=argparse.ArgumentParser();ap.add_argument('--font-file');ap.add_argument('--font-zip');a=ap.parse_args();OUT.mkdir(parents=True,exist_ok=True);font,src=kfgloader.discover_font(a.font_file,a.font_zip,OUT);h=OUT/'QURBATA-JILID-1-P031-KASRAH-11-CANDIDATE-V2-BALANCED-REVIEW.html';h.write_text(doc(font.resolve().as_uri()),encoding='utf-8');pdf,g=asyncio.run(render(h));print('QJ1_P031_KASRAH=PASS');print('MAIN_COMPETENCY=KASRAH');print('MATERIAL_NEW=فِ|قِ');print('RENDER_BALANCE=31_FOCUS|30_REVIEW');print('REVIEW_SPLIT=15_OLD_KASRAH|15_FATHAH');print('FATHAH_ROTATION=وَ|يَ|نَ|هَ|مَ|لَ|كَ|قَ|فَ|غَ|عَ|ظَ|طَ|ضَ|صَ');print('NAME_ROW_NEW=و|ي');print('NAME_ROW_SAFEAREA=PASS');print('PDF='+str(pdf.relative_to(ROOT)))
