#!/usr/bin/env python3
"""QURBATA Jilid 2 P024 — K2 U02 mad ya: native inline Qur'anic open sukun, no overlays."""
...
# Inject the Qur'anic open sukun ONLY at render time. No SVG, no crop, no absolute positioning.
...
p001.P001_CSS += r'''
.presentation-object{font-size:48pt;font-family:'Amiri Quran','Amiri','QURBATA KFGQPC Uthman Taha Naskh',serif!important;font-feature-settings:'mark' 1,'mkmk' 1}
.j2-glyph{font-size:44pt;font-family:'Amiri Quran','Amiri','QURBATA KFGQPC Uthman Taha Naskh',serif!important;font-feature-settings:'mark' 1,'mkmk' 1;font-kerning:normal;text-rendering:optimizeLegibility}
.mad-unit{display:inline-block;direction:rtl;unicode-bidi:isolate;font-family:'Amiri Quran','Amiri','QURBATA KFGQPC Uthman Taha Naskh',serif!important;font-feature-settings:'mark' 1,'mkmk' 1}
'''
...
async def render(h,out,debug):
    report=out/'LAYOUT-OVERFLOW-REPORT-J2-P024-V10.json';png=out/'png';png.mkdir(parents=True,exist_ok=True)
    async with async_playwright() as pw:
        ...
        open_count=body.count(OPEN_SUKUN); round_count=body.count(ROUND_SUKUN)
        ...
        # No overlay artifacts are allowed anymore.
        overlay_count=await page.locator('[class*="sukun-"]').count()
        if overlay_count!=0: raise RuntimeError(f'P024_OVERLAY_ARTIFACT_FORBIDDEN actual={overlay_count}')
        ...
        pdf=out/'QURBATA-JILID-2-P024-V10-NATIVE-OPEN-SUKUN.pdf';await page.pdf(path=str(pdf),format='A5',print_background=True,margin={'top':'0','right':'0','bottom':'0','left':'0'});await browser.close()
    return metrics,report,pdf
p001.render=render
def main():
    rc=v22.main();print('JILID2_P024_RENDERER_V10=PASS');print('PAGE=24');print('HEADER_SEQUENCE=دِ|دِيۡ|دِيۡنُ');print('P024_STAGE=TWO_TO_THREE_LETTER_LADDER');print('TWO_LETTER_OBJECTS=16');print('THREE_LETTER_OBJECTS=16');print('SUKUN_CODEPOINT=U+06E1');print('SUKUN_STYLE=NATIVE_OPEN_DOTLESS_HEAD_OF_KHAH');print('SUKUN_RENDER_MODE=NATIVE_INLINE_OPENTYPE');print('SVG_OVERLAY=DISABLED');print('MANUAL_POSITIONING=DISABLED');print('ROUND_SUKUN_U+0652=FORBIDDEN');print('NATIVE_OPEN_SUKUN_COUNT=34');print('STATUS=P024_NATIVE_OPEN_SUKUN_CANDIDATE_NOT_FROZEN');return rc
