...
def review_stages_for(stage):
 return {"FATHAH":(),"KASRAH":("FATHAH",),"DHAMMAH":("FATHAH","KASRAH"),"MIXED":("FATHAH","KASRAH","DHAMMAH")}[stage]
...
def main():
 ...
 for page in range(1,41):
  ...
  stage=plan['HarakatStage'];page_competencies=[]
  for length in (1,2,3):
   generated=PGE.generate(plan['ActiveLetters'],plan['NewLetters'],stage,length,8,page,review_stages_for(stage));
   for offset,obj in enumerate(generated):
    ...
    code,desc=competency(obj.stage,length)
    pair=(code,desc)
    if pair not in page_competencies:page_competencies.append(pair)
    slot=(length-1)*8+offset+1
    reading.append({'Jilid':1,'Page':page,'Slot':slot,'RowBand':'ROWS_1_2_L1' if length==1 else ('ROWS_3_4_L2' if length==2 else 'ROWS_5_6_L3'),'ObjectID':f'J1V7-P{page:02d}-S{slot:02d}','ObjectOrigin':'PRACTICE_GENERATED','LearningState':obj.learning_state,'ReviewFromStage':obj.review_from_stage,'ArabicObject':obj.display_text,'Unit1':obj.units[0],'Unit2':obj.units[1] if length>=2 else '','Unit3':obj.units[2] if length>=3 else '','Base1':obj.bases[0],'Base2':obj.bases[1] if length>=2 else '','Base3':obj.bases[2] if length>=3 else '','UnitLength':length,'HarakatStage':obj.stage,'PageTargetHarakatStage':stage,'DisplayJoinPolicy':'DISCONNECTED_NO_SPACE','CompetencyCode':code,'CompetencyDescription':desc,'SourceRef':'PGE:JILID1','QuranQuotation':'NO','SpecialInjection':'NONE','Status':'MICRO_PROGRESSION_REVIEW_CANDIDATE_V7'})
  metadata.append({'Page':page,'HarakatStage':stage,'NewLetters':plan['NewLetters'],'ActiveLetters':plan['ActiveLetters'],'CompetencyCodes':' | '.join(x[0] for x in page_competencies),'CompetencyDescriptions':' | '.join(x[1] for x in page_competencies),'MemorizationCode':c['MemorizationCode'],'MemorizationDescription':c['MemorizationDescription'],'MemorizationStage':c['MemorizationStage'],'ArabicCode':c['ArabicCode'],'ArabicDescription':c['ArabicDescription'],'AkhlaqCode':c['AkhlaqCode'],'AkhlaqDescription':c['AkhlaqDescription'],'AssessmentCode':c['AssessmentCode'],'AssessmentDescription':c['AssessmentDescription'],'FooterProfile':c['FooterProfile'],'SpecialInjection':'NONE','Status':'MICRO_PROGRESSION_REVIEW_CANDIDATE_V7'})
 ...
 print('JILID1_COMPOSER_V7=PASS');print(f'READING_ROWS={len(reading)}');print('REVIEW_POLICY=50_CURRENT_COMPETENCY|50_CUMULATIVE_REVIEW');print('DISPLAY_JOIN_POLICY=DISCONNECTED_NO_SPACE');return 0
