...
def pair_units(combo,stage):
    mark=MARKS[stage] if stage in MARKS else ('َ' if stage=='MIXED' else 'َ')
...
def main()->int:
    ...
    for page in range(1,41):
        ...
        if new:
            ...
            focus_bases=stage_focus(active,stage,page);label={'KASRAH':'Kasrah','DHAMMAH':'Dhammah','MIXED':'Campuran fathah-kasrah-dhammah'}[stage];focus_tokens=[display_base(b)+(MARKS[stage] if stage in MARKS else MARKS['FATHAH']) for b in focus_bases]
        ...
        for i,combo in enumerate(pairs): reading.append(row_from(combo,pair_units(combo,stage),page=page,slot=i+1,band='ROW_2_L2_CURRENT',state='FOUNDATION' if page==1 else 'CURRENT',stage=stage,code=pair_code,desc=pair_desc))
        if page==1:
            ...
            # 19 objects cannot be exactly 50:50. Alternate 9:10 and 10:9 page totals.
